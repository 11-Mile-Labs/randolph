...
import { parseApproveDelegation, parseRejectDelegation, parseReviseDelegation, parseSaveDelegationPreset } from './delegation-validation.js';
...
if (!app.requestSingleInstanceLock()) { quitting = true; app.quit(); }
  ...
  async function start(): Promise<void> {
    try {
      ...
      command('randolph:approve-delegation', input => runtime!.approveDelegation(parseApproveDelegation(input)));
      command('randolph:save-delegation-preset', input => runtime!.saveDelegationPreset(parseSaveDelegationPreset(input)));
