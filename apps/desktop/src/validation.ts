...
function parseSelection(value: unknown): HarnessSelection {
  ...
  if (
    ...
    throw new Error('Invalid harness, model, or effort.');
...
export function parseProjectDefaults(value: unknown): SaveProjectDefaultsInput {
  ...
  if (input.enabledRoutes !== undefined) {
    if (!Array.isArray(input.enabledRoutes) || input.enabledRoutes.length > 32)
      throw new Error('Invalid enabled harness routes.');
    ...
    if (
      new Set(enabledRoutes.map((route) => `${route.harness}:${route.executable}`)).size !==
      enabledRoutes.length
    )
      throw new Error('Duplicate enabled route.');
  }
  return {
    projectId: parseId(input.projectId),
    ...(enabledRoutes === undefined ? {} : { enabledRoutes }),
    defaults: {
      ...parseSelection(input.defaults),
      ...(record(input.defaults).executable === undefined
        ? {}
        : { executable: parseExecutable(record(input.defaults).executable) }),
    },
...
export function parseConversationSelection(value: unknown): ConversationSelectionInput {
  const input = record(value);
  return {
    conversationId: parseId(input.conversationId),
    selection: input.selection === null ? null : parseSelection(input.selection),
  };
}
export function parseId(value: unknown): string {
  if (
    ...
    throw new Error('Invalid project, conversation, or run identifier.');
  return value;
}
export function parseSend(value: unknown): SendInput {
  ...
  if (input.harness === undefined && input.model === undefined && input.effort === undefined)
    return message;
  ...
  return {
    ...message,
    ...(input.harness === undefined ? {} : { harness: selection.harness }),
...
export function parseChatEvents(value: unknown): ChatEventsInput {
  const input = record(value);
  if (!Number.isSafeInteger(input.afterSequence) || Number(input.afterSequence) < 0)
    throw new Error('Invalid event cursor.');
  return {
    conversationId: parseId(input.conversationId),
    runId: parseId(input.runId),
...
export function parsePushApproval(value: unknown): ApprovePushInput {
  const input = record(value);
  if (typeof input.revision !== 'string' || !/^[0-9a-f]{64}$/.test(input.revision))
    throw new Error('Invalid push preview revision.');
...
export function parseAppSettings(value: unknown): SaveAppSettingsInput {
  const input = record(value),
    preferences = record(input.value),
    notifications = record(preferences.notifications);
  if (
    !['system', 'light', 'dark'].includes(String(preferences.theme)) ||
    typeof preferences.background !== 'boolean' ||
    ['completed', 'failures', 'approvals'].some((key) => typeof notifications[key] !== 'boolean')
  )
...
function parseExecutable(value: unknown): string | null {
  ...
  if (
    ...
    throw new Error('CLI executable must be a bounded absolute path.');
  return value;
}
export function parseHarnessRequest(value: unknown): {
  projectId?: string;
  executable?: string;
  harness?: HarnessId;
} {
  ...
  const input = record(value);
  return {
    ...(input.projectId === undefined ? {} : { projectId: parseId(input.projectId) }),
    ...(input.executable === undefined
      ? {}
      : { executable: parseExecutable(input.executable) ?? undefined }),
    ...(input.harness === undefined ? {} : { harness: parseHarnessId(input.harness) }),
  };
}

export function parseInspectProject(value: unknown): InspectProjectInput {
  const input = record(value);
  if (typeof input.brief !== 'string' || input.brief.length > 8000 || input.brief.includes('\0'))
...
export function parseSetupApproval(value: unknown): ApproveProjectSetupInput {
  const input = record(value);
  if (typeof input.proposalRevision !== 'string' || !/^[a-f0-9]{64}$/u.test(input.proposalRevision))
    throw new Error('Invalid setup proposal revision.');
  return {
    projectId: parseId(input.projectId),
    runId: parseId(input.runId),
    proposalRevision: input.proposalRevision,
    expectedContextRevision: parseRevision(input.expectedContextRevision),
    value: parseProjectContext(input.value),
  };
}
