  ...
  return (
    <aside className="activity-panel" aria-label="Live activity">
      <div className="activity-heading">
        ...
        {run ? (
          <span className={`status-dot ${execution?.status ?? run.status}`} aria-hidden="true" />
        ) : null}
      </div>
        ...
        <>
          <div className="status-card">
            ...
            <div className="status-row">
              ...
              <strong>
                {lastNativeEvent ? relativeTime(lastNativeEvent.at, now) : 'None recorded'}
              </strong>
            </div>
            <div className="status-row">
              <span>Model</span>
          ...
          {run.checkpointError ? (
            <p className="inline-error" role="status">
              Checkpoint unavailable: {run.checkpointError}
            </p>
          ) : run.checkpoints?.length ? (
            <p className="muted-copy">
              {run.checkpoints.length} recoverable checkpoints · Open Run history to restore
            </p>
          ) : null}
          {run.memory?.references.length ? (
            <details className="run-context">
              <summary>
                {run.memory.references.length} supplied lessons · ~{run.memory.estimatedTokens}{' '}
                tokens (estimate)
              </summary>
              <pre>{run.memory.text}</pre>
            </details>
          ) : null}
          {run.error ? (
          ...
          <div className="event-list" aria-label="Run events">
            {events.length === 0 ? (
              ...
              visibleEvents.toReversed().map((event) => (
          ...
          <details className="native-records">
            ...
            <pre>
              {events
                .map(
                  (event) =>
                    `${event.at} ${event.type} ${event.summary} ${JSON.stringify(event.data)}`,
                )
                .join('\n')}
            </pre>
          </details>
