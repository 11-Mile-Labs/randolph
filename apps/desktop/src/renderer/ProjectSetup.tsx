...
export default function ProjectSetup({ project, onClose, onChanged }: Props) {
  ...
  const proposalIdentity = latest?.proposal ? `${latest.run.id}:${latest.proposal.revision}` : undefined;
  const draftStale = Boolean(proposalIdentity && draftRevisionRef.current !== proposalIdentity);
  const blockedRun = Boolean(setup?.inspections.some(item => active(item.run.status) || item.run.cleanupUnconfirmed || item.run.status === 'stop-unconfirmed'));
  ...
  const harnessStatus = busy ? 'Checking the selected harness.' : !harness ? 'Could not determine harness availability.' : canInspect ? `${harness.harness === 'codex' ? 'Codex' : 'Grok'} is ready for read-only inspection.` : harness.reason ?? (!harness.available ? 'The selected CLI is unavailable. Install it or choose an installed copy.' : !harness.authenticated ? 'Sign in to the selected CLI before inspecting this project.' : !selectedModel ? 'Choose an available model before inspecting this project.' : !harness.executionModes?.includes('read-only') ? 'This harness is not verified for read-only inspection.' : 'Wait for the existing inspection to finish or verify its cleanup.');
  ...
  const load = async (resetDraft = false) => {
    try {
      ...
      const nextRevision = nextInspection?.proposal ? `${nextInspection.run.id}:${nextInspection.proposal.revision}` : undefined;
  ...
  const inspect = async () => {
    ...
    try { await window.randolph.inspectProject({ projectId: project.id, selection: { harness: harnessId, model, effort }, executable: executable || undefined, brief }); setBrief(''); await load(); await onChanged(); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Inspection could not start.'); }
    finally { setBusy(false); }
  };
  const approve = async () => {
    if (!latest?.proposal || !latest.canApprove || !currentRun?.projectContext || draftStale) return;
    setBusy(true); setError(undefined);
    ...
    catch (cause) { await load(); setError(cause instanceof Error ? cause.message : 'Approval is stale. Reload the proposal and try again.'); }
    finally { setBusy(false); }
  };
  ...
  const reconcileCleanup = async () => {
    setBusy(true); setError(undefined);
    try { await window.randolph.reconcileProjectSetupCleanup(project.id); await load(); await onChanged(); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not verify inspection cleanup.'); }
    finally { setBusy(false); }
  };
  ...
  return <dialog className="project-settings project-setup" ref={dialogRef} aria-labelledby="project-setup-title" onCancel={event => { event.preventDefault(); onClose(); }}>
    ...
    {selectedInspection ? <section className="setup-proposal"><h3>Latest proposal {latest?.approvedRevision === setup?.context.revision ? <span className="setup-ready">Approved</span> : latest?.canApprove ? <span className="setup-ready">Ready to approve</span> : <span>Needs another inspection</span>}</h3><label>Proposed purpose<input aria-label="Proposed purpose" value={draft.purpose} onChange={event => updateDraft({ ...draft, purpose: event.target.value })} /></label><label>Proposed instructions<textarea aria-label="Proposed instructions" value={draft.instructions} onChange={event => updateDraft({ ...draft, instructions: event.target.value })} rows={5} /></label><h4>Documents</h4>{draft.documents.map((doc, index) => <div className="setup-document" key={index}><input aria-label={`Document ${index + 1} path`} value={doc.path} onChange={event => { const documents = [...draft.documents]; documents[index] = { ...doc, path: event.target.value }; updateDraft({ ...draft, documents }); }} /><input aria-label={`Document ${index + 1} description`} value={doc.description} onChange={event => { const documents = [...draft.documents]; documents[index] = { ...doc, description: event.target.value }; updateDraft({ ...draft, documents }); }} /><button className="secondary-button" type="button" onClick={() => updateDraft({ ...draft, documents: draft.documents.filter((_, item) => item !== index) })}>Remove</button></div>)}<button className="secondary-button" type="button" onClick={() => updateDraft({ ...draft, documents: [...draft.documents, { path: '', description: '' }] })}>Add document</button><h4>Evidence</h4><ul>{selectedInspection.evidence.map(item => <li key={item}>{item}</li>)}</ul><h4>Questions</h4><ul>{selectedInspection.questions.length ? selectedInspection.questions.map(item => <li key={item}>{item}</li>) : <li>None recorded.</li>}</ul></section> : <p className="muted-copy">No inspection proposal yet.</p>}
    {draftStale ? <p className="inline-error" role="alert">A newer proposal is available. Reload before approving it.</p> : null}
    {setup?.cleanup ? <section className="setup-cleanup" aria-label="Inspection cleanup"><p className="inline-error" role="alert">{setup.cleanup.reason}</p><button className="secondary-button" type="button" onClick={() => void reconcileCleanup()} disabled={busy || !setup.cleanup.canReconcile}>Verify inspection cleanup</button></section> : null}
