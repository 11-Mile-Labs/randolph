...
test('project setup requires inspection and explicit approval, preserves edits, and stops without configuring the project', async ({
  ...
  const root = mkdtempSync(join(tmpdir(), 'randolph-setup-e2e-'));
  ...
  const proposal = {
    context: {
      purpose: 'Inspected project purpose',
      instructions: 'Proposed guidance',
      documents: [{ path: 'README.md', description: 'Project overview' }],
    },
    evidence: ['Read the existing README.'],
    questions: ['Which milestone comes first?'],
  };
  writeFileSync(
    join(bin, 'codex'),
    `#!${process.execPath}\n` +
...
`,
  ...
  try {
    ...
    await expect(
      dialog.locator('.setup-approved p').filter({ hasText: 'No approved purpose yet.' }),
    ).toContainText('No approved purpose yet.');
    await expect(
      dialog.getByRole('button', { name: 'Inspect project', exact: true }),
    ).toBeEnabled();
    expect(readFileSync(calls, 'utf8')).not.toContain('turn/start');
    await dialog
      .getByRole('textbox', { name: 'Setup idea or corrections', exact: true })
      .fill('Inspect this project.');
    await dialog.getByRole('button', { name: 'Inspect project', exact: true }).click();
    await expect(
      dialog.getByRole('textbox', { name: 'Proposed purpose', exact: true }),
    ).toHaveValue('Inspected project purpose');
    ...
    await dialog
      .getByRole('textbox', { name: 'Proposed instructions', exact: true })
      .fill('User correction\nKeep it small.');
    await dialog.getByRole('button', { name: 'Inspect project', exact: true }).click();
    await expect(
      dialog.getByText('A newer proposal is available. Reload before approving it.', {
        exact: true,
      }),
    ).toBeVisible();
    await expect(
      dialog.getByRole('textbox', { name: 'Proposed instructions', exact: true }),
    ).toHaveValue('User correction\nKeep it small.');
    await expect(
      dialog.getByRole('button', { name: 'Approve project context', exact: true }),
    ).toBeDisabled();
    await dialog.getByRole('button', { name: 'Reload', exact: true }).click();
    await expect(
      dialog.getByRole('textbox', { name: 'Proposed instructions', exact: true }),
    ).toHaveValue('Proposed guidance');
    await dialog
      .getByRole('textbox', { name: 'Proposed instructions', exact: true })
      .fill('User correction\nKeep it small.');
    await page.evaluate(async () => {
      ...
      await bridge.createConversation(snapshot.projects[0]!.id);
    });
    await expect(
      dialog.getByRole('textbox', { name: 'Proposed instructions', exact: true }),
    ).toHaveValue('User correction\nKeep it small.');
    const databasePath = join(root, 'data', 'app.sqlite');
    const receiptTrigger = new DatabaseSync(databasePath);
    receiptTrigger.exec(
      "CREATE TRIGGER fixture_receipt_failure BEFORE INSERT ON events WHEN json_extract(NEW.document, '$.type') = 'project-context.approved' BEGIN SELECT RAISE(FAIL, 'fixture receipt failure'); END",
    );
    receiptTrigger.close();
    try {
      const turnsBeforeApproval = readFileSync(calls, 'utf8')
        .split('\n')
      ...
      expect(readFileSync(join(project, 'config.project.yaml'), 'utf8')).toContain(
        'User correction',
      );
      await expect(
        dialog.getByText(
      ...
      await expect(
        dialog.locator('.setup-approved p').filter({ hasText: 'Inspected project purpose' }),
      ).toContainText('Inspected project purpose');
      await expect(
        dialog.getByRole('button', { name: 'Approve project context', exact: true }),
      ).toBeDisabled();
      expect(
      ...
      ).toHaveLength(turnsBeforeApproval);
      await dialog.evaluate((element) => element.scrollTo({ top: 0 }));
      const screenshotPath = testInfo.outputPath('project-setup-receipt-failure-top.png');
      await page.screenshot({ path: screenshotPath, fullPage: true });
      await testInfo.attach('project-setup-receipt-failure-top', {
        path: screenshotPath,
        contentType: 'image/png',
      });
      console.log(`Project Setup screenshot: ${screenshotPath}`);
      await dialog.evaluate((element) => element.scrollTo({ top: element.scrollHeight }));
      const lowerScreenshotPath = testInfo.outputPath('project-setup-receipt-failure-lower.png');
      await page.screenshot({ path: lowerScreenshotPath, fullPage: true });
      await testInfo.attach('project-setup-receipt-failure-lower', {
        path: lowerScreenshotPath,
        contentType: 'image/png',
      });
      console.log(`Project Setup screenshot: ${lowerScreenshotPath}`);
    } finally {
      const cleanup = new DatabaseSync(databasePath);
      cleanup.exec('DROP TRIGGER IF EXISTS fixture_receipt_failure');
      cleanup.close();
    }
    await dialog.getByRole('button', { name: 'Inspect project', exact: true }).click();
    await expect(
    ...
    await dialog
      .getByRole('textbox', { name: 'Proposed instructions', exact: true })
      .fill('Confirmed user correction.');
    await dialog.getByRole('button', { name: 'Approve project context', exact: true }).click();
    await expect
      .poll(() => readFileSync(join(project, 'config.project.yaml'), 'utf8'))
      .toContain('Confirmed user correction.');
    await expect(dialog.getByText('Approved', { exact: true })).toBeVisible();
    await expect(
      dialog.locator('.setup-approved p').filter({ hasText: 'Confirmed user correction.' }),
    ).toContainText('Confirmed user correction.');
    await expect(
      dialog.getByRole('button', { name: 'Approve project context', exact: true }),
    ).toBeDisabled();
    ...
    const turns = readFileSync(calls, 'utf8')
      .split('\n')
      .filter((line) => line.includes('turn/start')).length;
    await page
      .getByRole('navigation', { name: 'Project conversations', exact: true })
    ...
    await dialog.getByRole('button', { name: 'Inspect project', exact: true }).click();
    await expect(
      dialog.getByRole('button', { name: 'Stop inspection', exact: true }),
    ).toBeEnabled();
    await dialog.getByRole('button', { name: 'Stop inspection', exact: true }).click();
    await expect(dialog.getByRole('button', { name: 'Stop inspection', exact: true })).toHaveCount(
      0,
    );
    await expect(
      dialog.getByRole('button', { name: 'Approve project context', exact: true }),
