...
const gridColumns = (element: Element) => new Set([...element.querySelectorAll('label')].map(label => (label as HTMLElement).offsetLeft)).size;
...
function assignment(id: string, role: 'worker' | 'main-synthesis', source: 'run-basis' | `output:${string}`, dependencies: string[], producesSource = false) {
  return { id, task: `${id} task`, role, harness: 'codex' as const, executable: '', executableVersion: 'codex-cli 0.154.0', model: mainModel, effort: 'low', rationale: `${id} rationale`, dependencies, source, mode: 'read-only' as const, deliverables: [`${id} output`], completionCriteria: [`${id} complete`], ...(producesSource ? { producesSource: true } : {}) };
}

function plan(executable: string) {
  return { schemaVersion: 1 as const, id: 'fixture-plan', revision: 1, limits: { maxWorkers: 4, maxParallel: 2, maxAttempts: 2, activeMinutes: 30 }, assignments: [
...
function seed(retainedActivity: boolean | 'cleanup-only' = false) {
  ...
  const home = join(root, 'home'), project = join(root, 'fixture-project'), bin = join(root, 'bin'), data = join(root, 'data');
  ...
  const executable = join(bin, 'codex'); writeFixtureCli(executable);
  ...
  const run = { id: '11111111-1111-4111-8111-111111111111', projectId: registered.id, conversationId: conversation.id, status: 'completed' as const, cleanupUnconfirmed: false, harness: 'codex' as const, executable, executableVersion: 'codex-cli 0.154.0', enabledHarnessRoutes: [{ harness: 'codex' as const, executable }], harnessAuthorizationRevision: 'fixture-route', workspaceIdentity: workspaceIdentity(project), model: mainModel, effort: 'low', executionMode: 'read-only' as const, workspace: project, createdAt, updatedAt: createdAt, lastActivityAt: createdAt };
  ...
  const recorded = records.recordPlan({ runId: run.id, revision: 1, requestId: 'fixture-request', source: 'proposal', basis: retainedDelegationBasis(runtime.store, run), plan: plan(executable) });
  records.readyPlan({ runId: run.id, planId: recorded.id, digest: recorded.digest, basisDigest: recorded.basisDigest });
  if (retainedActivity === true) {
    const exact = { runId: run.id, planId: recorded.id, digest: recorded.digest, basisDigest: recorded.basisDigest };
    ...
    const controls = new DelegationControls(runtime.store); controls.create(run.id, authorization.id); controls.command(run.id, 1, 'pause');
    const operations = new NativeOperationRecords(runtime.store);
    operations.create({ id: 'retained-native', owner: { kind: 'run', id: run.id }, runId: run.id, harness: 'codex', purpose: 'model-turn', generation: 1, origin: {}, capacity: { role: 'main' } });
    operations.admit({ id: 'retained-native', expectedGeneration: 1 });
  }
  if (retainedActivity === 'cleanup-only') runtime.store.putRun({ ...run, status: 'interrupted', cleanupUnconfirmed: true });
  runtime.store.exportRun(runtime.store.runs().find(item => item.id === run.id)!);
  return { root, home, project, data, conversationId: conversation.id, runId: run.id, original: recorded, close: () => runtime.close() };
}

async function launch(seedValue: ReturnType<typeof seed>) {
  const env = { ...process.env, HOME: seedValue.home, PATH: `${join(seedValue.root, 'bin')}:${process.env.PATH}`, RANDOLPH_DATA_DIR: seedValue.data };
...
test('delegation keeps exact revisions, presets, and rejection separate from unavailable execution', async ({ browserName: _browserName }, testInfo) => {
  const fixture = seed();
  ...
  try {
    ...
    await page.screenshot({ path: testInfo.outputPath('delegation-panel-1360-top.png'), fullPage: true });
    await page.locator('.delegation-actions').scrollIntoViewIfNeeded();
    await page.screenshot({ path: testInfo.outputPath('delegation-panel-1360-bottom.png'), fullPage: true });
    ...
    await panel.evaluate(scrollToPanelTop);
    await page.screenshot({ path: testInfo.outputPath('delegation-panel-960-top.png'), fullPage: true });
    await page.locator('.delegation-actions').scrollIntoViewIfNeeded();
    await page.screenshot({ path: testInfo.outputPath('delegation-panel-960-bottom.png'), fullPage: true });
    ...
    await expect(page.getByText('Preset saved. Saving a preset does not authorize execution.')).toBeVisible();
    writeFileSync(join(fixture.project, 'config.delegation.yaml'), 'schemaVersion: 1\nrouting: balanced\ndefaultPresetId: null\npresets: []\n');
    ...
    await expect(page.evaluate(async input => { try { await window.randolph.rejectDelegation(input); return 'accepted'; } catch (error) { return error instanceof Error ? error.message : String(error); } }, { runId: fixture.runId, planId: fixture.original.id, digest: fixture.original.digest, basisDigest: fixture.original.basisDigest })).resolves.not.toBe('accepted');
    ...
    await app.close(); await fixture.close(); rmSync(fixture.root, { recursive: true, force: true });
  }
});
...
test('delegation retains an invalid saved graph as an editable draft', async () => {
  ...
  try {
    ...
    await expect(page.getByLabel('Approval blockers')).toContainText('depends on unknown assignment');
...
test('run activity shows retained cleanup and tasks without starting execution on reopen', async ({ browserName: _browserName }, testInfo) => {
  ...
  try {
    ...
    await app.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows()[0].setSize(960, 900));
    await expect(activity).toHaveJSProperty('scrollWidth', await activity.evaluate(element => element.clientWidth));
    await activity.scrollIntoViewIfNeeded();
    await page.screenshot({ path: testInfo.outputPath('run-activity-960.png'), fullPage: true });
    const retained = await page.evaluate(id => window.randolph.runExecutionSnapshot(id), fixture.runId);
    expect(retained.operations).toHaveLength(1);
    expect(retained.operations[0].id).toBe('retained-native');
  } finally { await app.close(); rmSync(fixture.root, { recursive: true, force: true }); }
});


test('ordinary cleanup remains visible with no task graph or native operation', async () => {
  const fixture = seed('cleanup-only'); await fixture.close();
  const { app, page } = await launch(fixture);
  try {
    const activity = page.getByRole('region', { name: 'Run activity', exact: true });
    await expect(activity).toBeVisible();
    await expect(activity.getByText('Run cleanup has not been confirmed.', { exact: true })).toBeVisible();
    const snapshot = await page.evaluate(id => window.randolph.runExecutionSnapshot(id), fixture.runId);
    expect(snapshot.cleanupRequired).toBe(true); expect(snapshot.tasks).toHaveLength(0); expect(snapshot.control).toBeUndefined();
  } finally { await app.close(); rmSync(fixture.root, { recursive: true, force: true }); }
});
