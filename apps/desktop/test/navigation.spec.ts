...
test('workspace and native Settings are discoverable before chat, persist preferences, and reopen from background', async () => {
  ...
  const home = join(root, 'home'), bin = join(root, 'bin'), project = join(root, 'project');
...
`, { mode: 0o700 });
  ...
  try {
    ...
    await page.getByRole('navigation', { name: 'Application', exact: true }).getByRole('button', { name: 'Settings', exact: true }).click();
    await expect(page.getByRole('heading', { name: 'Settings', exact: true })).toBeVisible();
    await page.getByRole('radio', { name: 'Dark', exact: true }).check();
    await page.getByRole('checkbox', { name: 'Keep app-managed work running when the window closes' }).check();
    ...
    await expect(page.getByRole('checkbox', { name: 'Automatically approve new global lessons' })).toBeChecked();
    ...
    const menu = await app.evaluate(({ Menu }) => Menu.getApplicationMenu()!.items[0]!.submenu!.items.map(item => ({ label: item.label, accelerator: item.accelerator })));
    ...
    await page.getByRole('navigation', { name: 'Application', exact: true }).getByRole('button', { name: 'Workspace', exact: true }).click();
    ...
    await app.evaluate(({ Menu }) => { const item = Menu.getApplicationMenu()!.items[0]!.submenu!.items.find(item => item.label === 'Settings…')!; Reflect.apply(item.click, item, [item, undefined, {}]); });
    page = await fresh;
    await expect(page.getByRole('heading', { name: 'Settings', exact: true })).toBeVisible();
    ...
    expect(readFileSync(join(root, 'data', 'config.memory.yaml'), 'utf8')).toContain('autoApprove: true');
    app = await launch();
