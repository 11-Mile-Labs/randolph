...
createInterface({input:process.stdin}).on('line',line=>{
 ...
 if(m.method==='account/read')send({id:m.id,result:{account:{type:'chatgpt'}}});
  ...
  send({method:'item/agentMessage/delta',params:{threadId:'fixture-thread',turnId:'fixture-turn',itemId:'answer',delta:'Retained recovery fixture response.'}});
  send({method:'turn/completed',params:{threadId:'fixture-thread',turn:{id:'fixture-turn',status:readFileSync(${JSON.stringify(turns)},'utf8').trim().split('\\n').length===1?'interrupted':'completed'}}});
