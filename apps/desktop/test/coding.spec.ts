...
`,
  ...
  try {
    ...
    await expect(
      page.getByText('Updated value.txt. Ready for review.', { exact: true }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole('button', { name: 'Approve commit and merge', exact: true }),
    ).toBeDisabled();
    ...
    await expect(
      page.getByRole('button', { name: 'Preview origin push', exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole('button', { name: 'Push approved commit', exact: true }),
    ).toHaveCount(0);
    await page.getByRole('button', { name: 'Preview origin push', exact: true }).click();
    await expect(
      page.getByRole('region', { name: 'Separate origin push' }).getByRole('alert'),
    ).toBeVisible();
    ...
    await expect(
      timeline.getByText('Approved commit merged into the parent branch.', { exact: true }),
    ).toBeVisible();
    await timeline.getByText('delivery.merged', { exact: true }).click();
    await expect(
      timeline.getByRole('listitem').filter({ hasText: 'delivery.merged' }).locator('pre'),
    ).toContainText(snapshot.reviews[0]!.commitOid!);
    await expect(
      history.getByRole('region', { name: 'Recorded delivery', exact: true }),
    ).toContainText('Merged to main');
    ...
    await history
      ...
      .getByRole('button', { name: 'Restore files to folder' })
      .click();
