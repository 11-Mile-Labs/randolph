...
test('memory and push IPC reject invalid scope identities, versions, and actions', () => {
  ...
  const reference = { lessonId: randomUUID(), version: 1 };
  assert.deepEqual(parseMemoryCommand({ projectId, action: 'approve', references: [reference] }), {
    projectId,
    action: 'approve',
    references: [reference],
  });
  for (const value of [
    { projectId, action: 'exec', command: 'touch file' },
    { projectId, action: 'approve', references: [{ ...reference, version: 0 }] },
    { projectId, action: 'pin', reference, pinned: 'yes' },
    { projectId, action: 'settings', scope: 'elsewhere', value: {}, expectedRevision: null },
  ])
    assert.throws(() => parseMemoryCommand(value));
