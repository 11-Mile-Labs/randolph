...
test('project CLI selection changes discovered models and dispatches only the selected executable', async () => {
  ...
  const home = join(root, 'home'), project = join(root, 'project'), first = join(root, 'first'), second = join(root, 'second');
  for (const path of [home, project, first, second]) mkdirSync(path);
  const calls = join(root, 'turns.log');
  for (const [directory, name] of [[first, 'first'], [second, 'second']] as const) writeFileSync(join(directory, 'codex'), `#!${process.execPath}\n` + `
...
`, { mode: 0o700 });
  const env = { ...process.env, HOME: home, PATH: `${first}:${second}:/usr/bin:/bin`, RANDOLPH_DATA_DIR: join(root, 'data') };
  ...
  try {
    ...
    await page.getByRole('combobox', { name: 'Codex CLI', exact: true }).selectOption(join(second, 'codex'));
    await expect(page.getByRole('combobox', { name: 'Default model', exact: true })).toHaveValue('second-model');
    ...
    expect(readFileSync(join(project, 'config.harness.yaml'), 'utf8')).toContain(join(second, 'codex'));
    await page.getByRole('button', { name: 'Close settings', exact: true }).click();
    await expect(page.getByRole('combobox', { name: 'Model', exact: true })).toHaveValue('second-model');
    ...
    await page.getByRole('navigation', { name: 'Project conversations', exact: true }).getByRole('button', { name: 'Project settings', exact: true }).click();
    await expect(page.getByRole('combobox', { name: 'Codex CLI', exact: true })).toHaveValue(join(second, 'codex'));
    await page.getByRole('combobox', { name: 'Codex CLI', exact: true }).selectOption('');
    await expect(page.getByRole('combobox', { name: 'Default model', exact: true })).toHaveValue('first-model');
    await page.getByRole('button', { name: 'Save project defaults', exact: true }).click();
    await expect(page.getByText('Project defaults saved.', { exact: true })).toBeVisible();
