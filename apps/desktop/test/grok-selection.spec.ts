...
`,
  ...
  try {
    ...
    await expect(
      page.getByText('Read-only; Code compatibility unverified', { exact: false }),
    ).toHaveCount(0);
    ...
    await expect(
      page.getByRole('combobox', { name: 'Conversation harness', exact: true }),
    ).toHaveValue('grok');
    ...
    await expect(
      page.getByRole('alert').filter({ hasText: 'Grok execution is unavailable' }),
    ).toBeVisible();
    ...
    await page
      .getByRole('combobox', { name: 'Conversation harness', exact: true })
      .selectOption('codex');
    await expect(
      page.getByRole('combobox', { name: 'Conversation harness', exact: true }),
    ).toHaveValue('codex');
