...
function fixture(mode = 'normal', version = 'grok 1.0.30 (04b7ffed98c6) [stable]') {
  ...
  writeFileSync(calls, '');
  writeFileSync(join(workspace, 'README.md'), 'Project fixture');
...
`,
  ...
  return {
    ...
    executable,
    calls,
    adapter: new GrokProtocol({ executable, readConfig: () => undefined }),
                  ...
                  availableModels: [
                    {
                      modelId: 'grok-4.6',
                      name: 'Grok 4.6',
                      _meta: { reasoningEfforts: [{ id: 'low', default: true }] },
                    },
                  ],
                },
              },
          ...
          JSON.stringify({
            ...
            result: {
              _meta: {
                ...
                subscription_tier: 'Test subscription',
              },
            },
          }) + '\n',
        );
...
test('native discovery requires cached subscription auth and exposes the actual model/effort catalog', async () => {
  const f = fixture();
  try {
    const info = await f.adapter.discover();
    assert.equal(info.authenticated, true);
    assert.equal(info.cleanupVerified, true);
    assert.equal(info.version, 'grok 1.0.30');
    assert.deepEqual(info.executionModes, []);
    assert.deepEqual(info.models[0].efforts, ['low', 'high']);
    assert.equal(info.executable, f.executable);
  } finally {
    f.close();
  }
  for (const mode of ['api', 'billed']) {
    const f = fixture(mode);
    try {
      const info = await f.adapter.discover();
      assert.equal(info.authenticated, false);
      await assert.rejects(f.adapter.run(input(f)), /subscription|API/i);
...
test('installation candidates are filesystem-only and do not launch version probes', async () => {
  ...
  try {
    let probes = 0;
    const adapter = new GrokProtocol({
    ...
    assert.ok(
      installations.every(
        (installation) =>
          typeof installation.executable === 'string' && installation.version === undefined,
      ),
    );
...
test('discovery cancellation before launch and during initialization leaves no authenticated catalog', async () => {
  ...
  try {
    ...
    assert.equal(readFileSync(before.calls, 'utf8'), '');
  } finally {
    before.close();
  }
  const f = fixture('delayed-init');
  try {
    const controller = new AbortController();
    const pending = f.adapter.discover(undefined, controller.signal);
    await waitForCall(f.calls, /initialize/);
    controller.abort();
    ...
    assert.match(calls, /initialize/);
    assert.doesNotMatch(calls, /authenticate/);
  } finally {
    f.close();
  }
});
...
test('repeated cancellation settles an owned discovery process once', async () => {
  ...
  try {
    ...
    const info = await pending;
    assert.equal(info.cleanupVerified, true);
    const terminations = readFileSync(f.calls, 'utf8')
      .trim()
      .split('\n')
      .filter(Boolean)
      .map(JSON.parse)
      .filter((entry) => entry.event === 'terminated');
...
test('ACP streams only this session assistant text and preserves separate tool activity', async () => {
  ...
  try {
    const events = [];
    const result = await f.adapter.run({ ...input(f), onEvent: (event) => events.push(event) });
    ...
    assert.ok(
      events.some((e) => e.type === 'session.started' && e.data.sessionId === 'native-session'),
    );
  } finally {
    f.close();
  }
...
test('cancellation addresses the native session and waits for owned process cleanup', async () => {
  ...
  try {
    ...
    const result = await f.adapter.run({
      ...
      onEvent: (event) => {
        if (event.type === 'message.delta') signal.abort();
      },
    });
    assert.equal(result.status, 'interrupted');
    assert.ok(readFileSync(f.calls, 'utf8').includes('session/cancel'));
  } finally {
    f.close();
  }
});
...
test('changed executable versions, unavailable models, and API overrides cannot dispatch', async () => {
  ...
  try {
    await assert.rejects(
      f.adapter.run({ ...input(f), executableVersion: 'old' }),
      /version changed/,
    );
    await assert.rejects(f.adapter.run({ ...input(f), model: 'other' }), /model|effort/);
    const overridden = new GrokProtocol({
      executable: f.executable,
      readConfig: () => '[model."grok-4.6"]\nbase_url="https://example.invalid"\n',
    });
...
test('only the verified Grok 1.0.30 build can dispatch', async () => {
  const f = fixture('normal', 'grok 1.0.31 (drifted) [stable]');
  try {
...
test('native effort mismatch cannot dispatch a prompt', async () => {
  const f = fixture('wrong-effort');
  try {
    await assert.rejects(f.adapter.run(input(f)), /confirm.*effort/);
    assert.equal(readFileSync(f.calls, 'utf8').includes('session/prompt'), false);
  } finally {
    f.close();
  }
...
for (const mode of ['missing-catalog', 'wrong-catalog-session', 'extra-tool', 'duplicate-tool'])
  test(`unverified native tool catalog (${mode}) prevents model dispatch`, async () => {
    const f = fixture(mode);
    try {
      const adapter = new GrokProtocol({
        executable: f.executable,
...
test('ACP advertises both filesystem handlers, stays bounded in the workspace, and denies writes without mutation', async () => {
  ...
  try {
    const events = [];
    assert.equal(
      (await f.adapter.run({ ...input(f), onEvent: (event) => events.push(event) })).status,
      'completed',
    );
    const replies = readFileSync(f.calls, 'utf8')
      .trim()
      .split('\n')
      .map(JSON.parse)
      .filter((m) => !m.method);
    const initialize = readFileSync(f.calls, 'utf8')
      .trim()
      .split('\n')
      .map(JSON.parse)
      .find((m) => m.method === 'initialize');
    assert.deepEqual(initialize.params.clientCapabilities.fs, {
      readTextFile: true,
      writeTextFile: true,
    });
    ...
    assert.ok(
      events.some(
        (event) => event.type === 'approval.denied' && event.data.method === 'fs/write_text_file',
      ),
    );
...
for (const mode of ['delayed-init', 'delayed-session'])
  test(`workspace redirection during ${mode} aborts before prompt`, async () => {
    const f = fixture(mode);
    try {
      ...
      writeFileSync(join(redirected, 'README.md'), 'REDIRECTED');
      rmSync(f.workspace, { recursive: true });
      symlinkSync(redirected, f.workspace);
...
test('Code writes edit and create files but cannot write outside scope or across sessions', async () => {
  const f = fixture('code-requests');
  try {
    const events = [];
    assert.equal(
      (await f.adapter.run({ ...input(f), executionMode: 'code', onEvent: (e) => events.push(e) }))
        .status,
      'completed',
    );
    const replies = readFileSync(f.calls, 'utf8')
...
test('late native tool expansion terminates the run before accepting further writes', async () => {
  const f = fixture('late-catalog');
  try {
    await assert.rejects(
      f.adapter.run({ ...input(f), executionMode: 'code' }),
