...
function fakeChild({
  ...
  threadResponse,
  notifications = [],
  beforeThreadResponse = () => {},
  beforeTurnResponse = () => {},
  commandResult = { exitCode: 0, stdout: '', stderr: '' },
  commandOutput = 'checks passed\n',
  commandDelay = 0,
  beforeResponse = () => {},
} = {}) {
      ...
      if (request.method === 'account/read')
        ...
        send({
          ...
          params: {
            ...
            stream: 'stdout',
            deltaBase64: Buffer.from(commandOutput).toString('base64'),
            capReached: false,
          },
        });
        setTimeout(() => {
          send({ id: request.id, result: commandResult });
        }, commandDelay);
      } else if (request.method === 'thread/start') {
        threadId = 'thread-1';
        beforeThreadResponse({ send, threadId });
        send({ id: request.id, result: { thread: { id: threadId }, ...threadResponse } });
        ...
        send({
          ...
          params: { threadId, turnId: 'turn-1', itemId: 'command-1', delta: 'secret command' },
        });
        for (const notification of notifications) send(notification);
        send({
          method: 'item/agentMessage/delta',
          params: { threadId, turnId: 'turn-1', itemId: 'message-1', delta: modelId },
        });
        send({
          method: 'turn/completed',
          params: { threadId, turn: { id: 'turn-1', status: turnStatus } },
        });
...
const runInput = (signal, events = [], model = 'gpt-test') => ({
...
test('discover returns native model efforts for ChatGPT account', async () => {
  ...
  assert.deepEqual(info.models, [
    { id: 'gpt-test', name: 'Test Model', efforts: ['low'], defaultEffort: 'low' },
  ]);
});
...
test('discovery cannot advertise an executable route when native cleanup is unconfirmed', async () => {
  ...
  assert.equal(info.cleanupVerified, false);
  assert.equal(info.authenticated, false);
  assert.deepEqual(info.models, []);
  assert.deepEqual(info.executionModes, []);
...
test('explicit discovery cancellation settles native cleanup without creating a model turn', async () => {
  const child = fakeChild({ delayInitialize: 40 }),
  ...
  assert.equal(info.cleanupVerified, true);
  assert.equal(info.authenticated, false);
  assert.match(info.reason, /cancelled/);
...
test('run emits only agent-message deltas and keeps each concurrent client isolated', async () => {
  ...
  const [a, b] = await Promise.all([
    adapter.run(runInput(new AbortController().signal, eventsA, 'first')),
    adapter.run(runInput(new AbortController().signal, eventsB, 'second')),
  ]);
  assert.equal(a.status, 'completed');
  assert.equal(b.status, 'completed');
  assert.deepEqual(
    eventsA.filter((event) => event.type === 'message.delta').map((event) => event.data),
    [{ messageId: 'message-1', text: 'first' }],
  );
  assert.deepEqual(
    eventsB.filter((event) => event.type === 'message.delta').map((event) => event.data),
    [{ messageId: 'message-1', text: 'second' }],
  );
...
test('run rejects foreign and identity-less turn notifications without leaking their evidence or completing the active turn', async () => {
      ...
      },
      {
        ...
        params: {
          ...
          item: {
            ...
            aggregatedOutput: 'foreign output',
          },
        },
      },
      {
        method: 'turn/diff/updated',
        params: { threadId: 'foreign-thread', turnId: 'turn-1', diff: 'foreign diff' },
      },
      {
        method: 'turn/completed',
        params: { threadId: 'foreign-thread', turn: { id: 'turn-1', status: 'failed' } },
      },
      {
        method: 'turn/completed',
        params: {
          threadId: 'thread-1',
          turnId: 'turn-1',
          turn: { id: 'foreign-turn', status: 'failed' },
        },
      },
      {
        method: 'turn/started',
        params: { threadId: 'thread-1', turnId: 'turn-1', turn: { id: 'foreign-turn' } },
      },
      {
        method: 'thread/started',
        params: { threadId: 'thread-1', thread: { id: 'foreign-thread' } },
      },
      {
        method: 'item/agentMessage/delta',
        params: { itemId: 'missing-message', delta: 'missing text' },
      },
      {
        method: 'item/started',
        params: {
          threadId: 'foreign-thread',
          turnId: 'turn-1',
          item: { type: 'commandExecution', command: 'foreign activity' },
        },
      },
      {
        method: 'item/started',
        params: { item: { type: 'commandExecution', command: 'missing activity' } },
      },
      {
        method: 'thread/status/changed',
        params: { threadId: 'foreign-thread', status: { type: 'idle' } },
      },
      {
        method: 'thread/tokenUsage/updated',
        params: { threadId: 'thread-1', turnId: 'foreign-turn', tokenUsage: {} },
      },
      {
        method: 'item/completed',
        params: {
          item: {
            type: 'fileChange',
            id: 'missing-file',
            changes: [{ path: 'missing.txt', diff: 'missing diff' }],
          },
        },
      },
  ...
  assert.deepEqual(
    events.filter((event) => event.type === 'message.delta').map((event) => event.data.text),
    ['gpt-test'],
  );
  assert.ok(events.every((event) => !/foreign|missing/.test(JSON.stringify(event))));
  assert.equal(
    events.some((event) => event.data?.method === 'thread/tokenUsage/updated'),
    false,
  );
  assert.equal(
    events.some((event) => event.data?.method === 'turn/started'),
    false,
  );
  assert.equal(
    events.some((event) => event.data?.method === 'thread/started'),
    false,
  );
});
...
test('run retains schema-shaped thread and turn lifecycle notifications delivered before their RPC acknowledgements', async () => {
  ...
  assert.deepEqual(
    events
      ...
      .map((event) => event.data.method)
      .filter((method) => method.endsWith('/started')),
  ...
  const child = fakeChild({
    beforeTurnResponse({ send, threadId, turnId }) {
      for (let index = 0; index <= 64; index++)
        send({
          method: 'item/agentMessage/delta',
          params: { threadId, turnId, itemId: `early-${index}`, delta: `early ${index}` },
        });
    },
  });
  await assert.rejects(
    adapterFor([child]).run(runInput(new AbortController().signal)),
    /buffer|notification/i,
  );
...
test('run retains identity-bearing native events delivered before turn start is acknowledged', async () => {
  const child = fakeChild({
    beforeTurnResponse({ send, threadId, turnId }) {
      send({
        method: 'item/agentMessage/delta',
        params: { threadId, turnId, itemId: 'early-message', delta: 'early text' },
      });
      send({
        method: 'item/completed',
        params: {
  ...
  assert.equal(
    events.find((event) => event.type === 'command.completed').data.output,
    'early output',
  );
...
test('run closes native event admission as soon as the caller cancels', async () => {
  ...
  const result = await adapterFor([child]).run({
    ...runInput(controller.signal, events),
    onEvent(event) {
      events.push(event);
  ...
  assert.equal(
    events.some((event) => event.type === 'command.completed'),
    false,
  );
  assert.equal(
    events
      .filter((event) => event.type === 'message.delta')
      .some((event) => event.data.text === 'gpt-test'),
    false,
  );
});
...
test('failed turns and malformed transport fail the run', async () => {
  await assert.rejects(
    adapterFor([fakeChild({ turnStatus: 'failed' })]).run(runInput(new AbortController().signal)),
    (error) => {
  ...
  await assert.rejects(
    adapterFor([fakeChild({ invalid: true })]).run(runInput(new AbortController().signal)),
    /invalid JSON|Expected property/,
  );
});
...
function codeFixture(t, policyOverrides = {}) {
  ...
  return {
    workspace,
    policy,
    threadResponse: { cwd: workspace, approvalPolicy: 'never', sandbox: policy },
  };
}
...
test('read-only execution rejects redirected workspaces before native launch', async (t) => {
  ...
  const child = fakeChild();
  await assert.rejects(
    adapterFor([child]).run({ ...runInput(new AbortController().signal), workspace: link }),
    /canonical|workspace/,
  );
  ...
  test(`read-only execution rejects directory replacement during ${boundary}`, async (t) => {
    ...
    const child = fakeChild({
      beforeResponse(method) {
        if (method === boundary) {
          renameSync(project, join(workspace, 'original'));
          mkdirSync(project);
        }
      },
    });
    await assert.rejects(
      adapterFor([child]).run({ ...runInput(new AbortController().signal), workspace: project }),
      /workspace.*changed/,
    );
    assert.equal(child.methods.includes('turn/start'), false);
    assert.equal(child.exitCode, 0);
...
test('code mode fails before turn dispatch for unverified version, missing or broader effective policy', async (t) => {
  ...
  for (const returned of [
    ...
    { ...threadResponse, cwd: '/' },
    { ...threadResponse, sandbox: { ...policy, networkAccess: true } },
    { ...threadResponse, sandbox: { ...policy, writableRoots: [workspace, '/tmp'] } },
    { ...threadResponse, sandbox: { ...policy, excludeSlashTmp: false } },
    { ...threadResponse, sandbox: { ...policy, excludeTmpdirEnvVar: false } },
    { ...threadResponse, sandbox: { type: 'dangerFullAccess' } },
  ]) {
    const child = fakeChild({ threadResponse: returned });
    await assert.rejects(
      adapterFor([child]).run({
        ...runInput(new AbortController().signal),
        workspace,
        executionMode: 'code',
      }),
      /policy|permissions|sandbox/i,
    );
    assert.equal(child.methods.includes('turn/start'), false);
  }
  const child = fakeChild({ threadResponse });
...
test('code mode declines escalations and captures bounded native verification and change evidence', async (t) => {
      ...
      { id: 94, method: 'item/unknown/requestApproval', params: {} },
      {
        ...
        params: {
          ...
          item: {
            ...
            command: 'node --test',
            exitCode: 1,
            aggregatedOutput: 'f'.repeat(100_000),
            status: 'completed',
            cwd: workspace,
          },
        },
      },
      {
        ...
        params: {
          ...
          item: {
            ...
            type: 'fileChange',
            status: 'completed',
            changes: [{ path: 'src/a.ts', kind: { type: 'update' }, diff: '+new' }],
          },
        },
      },
      {
  ...
  await adapterFor([child]).run({
    ...runInput(new AbortController().signal, events),
    workspace,
    executionMode: 'code',
  });
  assert.deepEqual(
    child.replies.map((reply) => reply.result),
    [
      { decision: 'decline' },
      { decision: 'decline' },
  ...
  assert.deepEqual(child.replies.at(-1).error, {
    code: -32601,
    message: 'Unsupported native request declined by Randolph.',
  });
  ...
  assert.ok(
    events
      .filter((event) => event.type === 'message.delta')
      .every((event) => !event.data.text.includes('secret command')),
  );
});
...
test('runCommand uses native argv execution with restricted policy, live output and no turn', async (t) => {
  ...
  const child = fakeChild({
    threadResponse,
    commandResult: { exitCode: 2, stdout: '', stderr: '' },
  });
  const output = [];
  const result = await adapterFor([child]).runCommand({
    workspace,
...
test('runCommand synchronously reports its native correlation token immediately before command RPC', async (t) => {
  ...
  const child = fakeChild({
    threadResponse,
    beforeResponse(method) {
      if (method === 'command/exec') order.push('rpc');
    },
  });
  const result = await adapterFor([child]).runCommand({
    workspace,
    ...
    signal: new AbortController().signal,
    onOutput() {},
    onDispatch({ processId }) {
      assert.match(processId, /^[0-9a-f-]{36}$/);
      order.push('dispatch');
    },
  });
  assert.equal(result.exitCode, 0);
  assert.deepEqual(order, ['dispatch', 'rpc']);
  assert.equal(child.methods.filter((method) => method === 'command/exec').length, 1);
});

test('runCommand dispatch callback errors, thenables, cancellation, and workspace replacement prevent command RPC', async (t) => {
  const { workspace, threadResponse } = codeFixture(t);
  for (const onDispatch of [
    () => {
      throw new Error('ledger rejected dispatch');
    },
    ...
    assert.match(result.error, /ledger rejected|synchronous/);
    assert.equal(child.methods.includes('command/exec'), false);
  }
  const controller = new AbortController(),
    cancelled = fakeChild({ threadResponse });
  const interrupted = await adapterFor([cancelled]).runCommand({
    workspace,
    command: ['/usr/bin/true'],
  ...
  assert.match(interrupted.error, /interrupt/i);
  assert.equal(cancelled.methods.includes('command/exec'), false);
  const replacement = codeFixture(t),
    original = `${replacement.workspace}-original`;
  t.after(() => rmSync(original, { recursive: true, force: true }));
  const replaced = fakeChild({
    threadResponse: replacement.threadResponse,
    beforeResponse(method) {
      if (method === 'thread/start') {
        renameSync(replacement.workspace, original);
        mkdirSync(replacement.workspace);
      }
    },
  });
  const stale = await adapterFor([replaced]).runCommand({
  ...
  assert.match(stale.error, /workspace.*changed/i);
  assert.equal(replaced.methods.includes('command/exec'), false);
  const mismatch = fakeChild({ threadResponse });
  const invalidIdentity = await adapterFor([mismatch]).runCommand({
    workspace,
    workspaceIdentity: { device: 0, inode: 0 },
    command: ['/usr/bin/true'],
    signal: new AbortController().signal,
    onOutput() {},
  });
  assert.equal(invalidIdentity.exitCode, null);
  assert.match(invalidIdentity.error, /workspace.*changed/i);
  assert.equal(mismatch.methods.includes('command/exec'), false);
});

test('runCommand consumes a rejecting async dispatch callback without issuing a command RPC', async (t) => {
  const { workspace, threadResponse } = codeFixture(t),
    child = fakeChild({ threadResponse });
  const result = await adapterFor([child]).runCommand({
    workspace,
    command: ['/usr/bin/true'],
    signal: new AbortController().signal,
...
test('runCommand snapshots mutable command input before the native handshake and rejects missing workspaces without launch', async (t) => {
  ...
  const absent = await missing.runCommand({
    workspace: join(workspace, 'gone'),
    command: ['/usr/bin/true'],
    signal: new AbortController().signal,
    onOutput() {},
  });
  assert.equal(absent.exitCode, null);
  assert.equal(absent.cleanupVerified, true);
...
test('runCommand rejects incompatible authentication/version and reports bounded output', async (t) => {
  ...
  for (const extra of [{ execFile: () => 'codex-cli 0.150.0' }, {}]) {
    const child = fakeChild({ accountType: 'apiKey', threadResponse });
    const result = await adapterFor([child], extra).runCommand({
      workspace,
      command: ['/usr/bin/true'],
      signal: new AbortController().signal,
      onOutput: () => {},
...
test('native normalization may omit the already writable cwd, but cannot add another root', async (t) => {
  ...
  const run = await adapterFor([codeChild]).run({
    ...runInput(new AbortController().signal),
    workspace,
    executionMode: 'code',
  });
  ...
  const command = await adapterFor([commandChild]).runCommand({
    workspace,
    command: ['/usr/bin/true'],
    signal: new AbortController().signal,
    onOutput: () => {},
...
test('GUI-hosted native execution has explicit installed toolchains while HOME stays in the workspace', async () => {
  ...
  await running;
  const setting = (name) =>
    JSON.parse(child.launch.args.find((arg) => arg.startsWith(`${name}=`)).slice(name.length + 1));
...
test('explicit CLI discovery and execution use the selected copy instead of the default', async () => {
  ...
  const adapter = new CodexAdapter({
    ...
    execFile: (file) => (file === '/selected/codex' ? 'codex-cli selected' : 'codex-cli default'),
    spawn: (file) => {
      launched.push(file);
      return fakeChild({
        modelId: file === '/selected/codex' ? 'selected-model' : 'default-model',
      });
    },
  });
...
test('an executable changed after admission cannot dispatch a run', async () => {
  ...
  await assert.rejects(
    adapter.run({
      ...runInput(new AbortController().signal),
      executable: '/selected/codex',
      executableVersion: 'codex-cli admitted',
    }),
    (error) => {
      assert.ok(error instanceof AdapterRunFailure);
...
test('a Codex version drift fails the Runtime run with confirmed pre-dispatch cleanup', async (t) => {
  ...
  const run = await runtime.send({
    ...
    text: 'Validate drift handling.',
  });
  for (let i = 0; i < 100 && runtime.hasActiveWork(); i += 1)
...
test('verified 0.154 supports Code and checks with normalized effective writable roots', async (t) => {
  ...
  const adapter = adapterFor(
    [fakeChild({ threadResponse: response }), fakeChild({ threadResponse: response })],
    { execFile: () => 'codex-cli 0.154.0' },
  );
  ...
  assert.equal(result.status, 'completed');
  const check = await adapter.runCommand({
    workspace,
    command: ['/bin/echo', 'ok'],
    signal: new AbortController().signal,
    onOutput() {},
...
const appToolDefinition = {
  ...
  description: 'Read retained task state.',
...
test('application tools use full native function schema and respond only after acknowledged session binding', async () => {
  const events = [],
    calls = [];
  const child = fakeChild({
    beforeTurnResponse({ send }) {
      send(appCall());
    },
  });
  const result = await adapterFor([child], { execFile: () => 'codex-cli 0.154.0' }).run({
    ...runInput(new AbortController().signal, events),
    applicationTools: {
      ...
      onRequest(request) {
        assert.ok(
          events.some(
            (event) =>
              event.type === 'session.turn-started' && event.data.turnId === request.turnId,
          ),
        );
        calls.push(request);
        return { success: true, text: 'retained-receipt' };
      },
    },
  });
  assert.equal(result.status, 'completed');
  ...
  assert.deepEqual(
    child.replies.find((reply) => reply.id === 0),
    {
      id: 0,
      result: { success: true, contentItems: [{ type: 'inputText', text: 'retained-receipt' }] },
    },
  );
});
...
test('application tools reject wrong identities, names, namespaces and malformed payloads without invoking the handler', async () => {
  let calls = 0;
  const badCalls = [
    appCall({ threadId: 'foreign' }),
    appCall({ turnId: 'foreign' }),
    appCall({ namespace: 'foreign' }),
    appCall({ tool: 'other_tool' }),
    appCall({ arguments: null }),
    appCall({ callId: '' }),
    appCall({ arguments: { text: 'x'.repeat(65_537) } }),
  ];
  const child = fakeChild({ notifications: badCalls.map((request, i) => ({ ...request, id: i })) });
  await adapterFor([child], { execFile: () => 'codex-cli 0.154.0' }).run({
    ...runInput(new AbortController().signal),
    applicationTools: {
      definitions: [appToolDefinition],
      onRequest() {
...
test('application tools stay absent by default, require their verified version, and preserve native approval denial', async () => {
  ...
  assert.equal(
    ordinary.requests.find((request) => request.method === 'thread/start').params.dynamicTools,
    undefined,
  );
  assert.equal(ordinary.replies[0].error.code, -32601);
  assert.deepEqual(events.find((event) => event.type === 'session.turn-started')?.data, {
    threadId: 'thread-1',
    turnId: 'turn-1',
  });
  const child = fakeChild();
...
test('application requests cannot mutate after cancellation or a completed turn', async () => {
  ...
  const stopped = await adapterFor([cancelled], { execFile: () => 'codex-cli 0.154.0' }).run({
    ...runInput(controller.signal),
    applicationTools: tools,
    onEvent(event) {
      if (event.type === 'session.turn-started') controller.abort();
    },
  });
  assert.equal(stopped.status, 'interrupted');
  const completed = fakeChild({
    notifications: [
      {
        method: 'turn/completed',
  ...
  await adapterFor([completed], { execFile: () => 'codex-cli 0.154.0' }).run({
    ...runInput(new AbortController().signal),
    applicationTools: tools,
  });
  assert.equal(calls, 0);
...
test('application handler failures or invalid receipts fail the run with confirmed cleanup', async () => {
  for (const onRequest of [
    ...
    () => ({ success: true, text: 'x'.repeat(65_537) }),
    () => Promise.resolve({ success: true, text: 'async handlers are forbidden' }),
  ]) {
    const child = fakeChild({ notifications: [appCall()] });
    await assert.rejects(
      adapterFor([child], { execFile: () => 'codex-cli 0.154.0' }).run({
        ...runInput(new AbortController().signal),
        applicationTools: { definitions: [appToolDefinition], onRequest },
      }),
      /handler|receipt|database/i,
    );
...
test('enabling application tools does not authorize native filesystem or command approvals', async () => {
  ...
  const child = fakeChild({
    ...
    ].map((method, id) => ({ id, method, params: { threadId: 'thread-1', turnId: 'turn-1' } })),
  });
  await adapterFor([child], { execFile: () => 'codex-cli 0.154.0' }).run({
    ...runInput(new AbortController().signal),
    applicationTools: {
