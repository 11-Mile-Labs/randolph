...
export class CodexAdapter implements HarnessAdapter {
  ...
  async discover(executable?: string, signal?: AbortSignal): Promise<HarnessInfo> {
    ...
    if (executable) return new CodexAdapter({ ...this.options, executable }).discover(undefined, signal);
    let selected: string;
    try { selected = this.executablePath(); } catch { return { available: false, authenticated: false, models: [], cleanupVerified: true, reason: 'Codex CLI was not found. Install it and sign in with ChatGPT.' }; }
    const resolved = existsSync(selected) ? realpathSync(selected) : selected;
    if (resolved !== selected) return new CodexAdapter({ ...this.options, executable: resolved }).discover(undefined, signal);
    let version: string;
    try { version = this.exec(this.executablePath(), ['--version'], { encoding: 'utf8', timeout: 5_000, env: environment() }).trim(); }
    catch { return { available: false, authenticated: false, models: [], cleanupVerified: false, reason: 'Codex CLI could not be started. Install it and sign in with ChatGPT.' }; }
    ...
    try {
      if (signal?.aborted) return { ...info, cleanupVerified: true, reason: 'Discovery was cancelled before dispatch.' };
      ...
      if (account.type !== 'chatgpt') info.reason = 'Sign into the Codex CLI with ChatGPT. API-key authentication is not supported.';
      else {
