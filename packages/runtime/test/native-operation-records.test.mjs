...
test('persists exact native intent, admission, settlement, and reopen quarantine', (t) => {
  const root = mkdtempSync(join(tmpdir(), 'randolph-native-operations-')),
    store = new Store(root),
    records = new NativeOperationRecords(store);
  t.after(() => {
    store.close();
  ...
  const intent = {
    id: 'operation-one',
    owner: { kind: 'app-discovery', id: 'codex' },
    harness: 'codex',
    purpose: 'discovery',
    capacity: { role: 'discovery' },
    generation: 1,
    origin: { fixture: true },
  };
  assert.equal(records.create(intent).state, 'queued');
  assert.equal(records.create(intent).state, 'queued');
  ...
  assert.equal(
    records.resolveIdentity({
      id: intent.id,
      expectedGeneration: 1,
      resolvedIdentity: { executable: '/fixture/codex', version: '1' },
    }).resolvedIdentity.version,
    '1',
  );
  assert.equal(
    records.settle({
...
test('settles only a durably later-boot reconciled project-setup model turn', () => {
  ...
  const sessionRow = store.db
    .prepare('SELECT document FROM delegation_sessions WHERE id=?')
    .get('session');
  const savedSession = JSON.parse(sessionRow.document);
  store.db
    .prepare('UPDATE delegation_sessions SET document=? WHERE id=?')
    .run(JSON.stringify({ ...savedSession, cleanupEvidence: undefined }), 'session');
  assert.throws(() => records.reconcileSetupCleanup(run.id), /proof/);
  assert.equal(records.list()[0].state, 'quarantined');
  store.db
    .prepare('UPDATE delegation_sessions SET document=? WHERE id=?')
    .run(sessionRow.document, 'session');
  store.putRun({ ...run, executionMode: 'code' });
  assert.throws(() => records.reconcileSetupCleanup(run.id), /read-only/);
  store.putRun(run);
  assert.equal(records.reconcileSetupCleanup(run.id)[0].state, 'settled');
  assert.equal(records.reconcileSetupCleanup(run.id).length, 0);
  store.close();
  rmSync(root, { recursive: true, force: true });
});
...
test('canonical replay and failed event persistence preserve exact prior operation state', (t) => {
  ...
  const intent = {
    id: 'atomic',
    owner: { kind: 'app-discovery', id: 'catalog' },
    harness: 'codex',
    purpose: 'discovery',
    capacity: { role: 'discovery' },
    generation: 1,
  ...
  store.db.exec(
    "CREATE TRIGGER event_fault BEFORE INSERT ON native_operation_events BEGIN SELECT RAISE(ABORT, 'event fault'); END",
  );
  assert.throws(() => records.admit({ id: intent.id, expectedGeneration: 1 }), /event fault/);
  assert.equal(records.list()[0].state, 'queued');
  assert.equal(records.events(intent.id).length, 1);
  store.db.exec('DROP TRIGGER event_fault');
  records.admit({ id: intent.id, expectedGeneration: 1 });
  store.db.exec(
    "CREATE TRIGGER event_fault BEFORE INSERT ON native_operation_events BEGIN SELECT RAISE(ABORT, 'event fault'); END",
  );
  ...
  assert.equal(records.list()[0].state, 'admitted');
  assert.equal(records.events(intent.id).length, 2);
});
