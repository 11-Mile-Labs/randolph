...
import { mkdtempSync, mkdirSync, writeFileSync, readFileSync, rmSync, chmodSync, unlinkSync, symlinkSync, renameSync, cpSync } from 'node:fs';
import { realpathSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import test from 'node:test';
import { inspectGitWorkspace, createGitReview, createGitDeliveryPlan, commitGitDelivery, mergeGitDelivery, reconcileGitDelivery, cleanupGitDelivery } from '../dist/git-review.js';

function git(path, args) { return execFileSync('/usr/bin/git', ['-c', 'core.hooksPath=/dev/null', '-c', 'commit.gpgsign=false', '-C', path, ...args], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], env: { ...process.env, GIT_CONFIG_GLOBAL: '/dev/null', GIT_CONFIG_NOSYSTEM: '1' } }).trim(); }
function fixture(t, ignoreWorktrees = true) {
  const base = realpathSync(mkdtempSync(join(tmpdir(), 'randolph-git-review-')));
  const root = join(base, 'project'); const evidence = join(base, 'evidence');
...
test('review captures all nonignored file changes without altering branch, HEAD, or real index', t => {
  const f = fixture(t); const before = inspectGitWorkspace(f.root, f.workspace);
  ...
  assert.equal(review.parentOid, before.parentOid); assert.equal(review.workspaceHead, before.workspaceHead);
  assert.deepEqual(review.files.map(file => file.path).sort(), ['binary.dat', 'original.txt', 'removed.txt', 'run.sh']);
...
test('delivery plan is durable before effects and commit and merge reconcile without duplicate actions', t => {
  ...
  assert.deepEqual(reconcileGitDelivery(f.root, f.workspace, plan), { commitCreated: false, merged: false });
  ...
  assert.deepEqual(reconcileGitDelivery(f.root, f.workspace, plan), { commitCreated: true, merged: false });
  assert.equal(git(f.root, ['rev-parse', 'HEAD']), review.parentOid);
  mergeGitDelivery(f.root, f.workspace, recovered);
  assert.deepEqual(reconcileGitDelivery(f.root, f.workspace, plan), { commitCreated: true, merged: true });
  ...
  commitGitDelivery(f.root, f.workspace, recovered); mergeGitDelivery(f.root, f.workspace, recovered);
...
test('dirty parent changes including project settings are preserved and block delivery', t => {
  ...
  const review = createGitReview(f.root, f.workspace, f.evidence);
  assert.throws(() => createGitDeliveryPlan(f.root, f.workspace, review, 'Approved'), /parent.*uncommitted/i);
  assert.equal(readFileSync(join(f.root, 'config.harness.yaml'), 'utf8'), 'operator-owned settings\n');
  assert.equal(git(f.root, ['rev-parse', 'HEAD']), review.parentOid);
});
...
test('review fingerprints raw bytes without running clean filters or normalizing line endings', t => {
  ...
  assert.deepEqual(execFileSync('/usr/bin/git', ['-C', f.root, 'show', `${review.treeOid}:original.txt`]), Buffer.from('raw\r\n'));
  assert.throws(() => readFileSync(marker), { code: 'ENOENT' });
  writeFileSync(join(f.workspace, 'original.txt'), 'raw\n');
  assert.throws(() => createGitDeliveryPlan(f.root, f.workspace, review, 'Approved'), /stale|changed/i);
});
...
test('cleanup preserves late edits and then removes only the unchanged approved worktree idempotently', t => {
  ...
  commitGitDelivery(f.root, f.workspace, plan); mergeGitDelivery(f.root, f.workspace, plan);
...
test('redirected worktrees, detached parent and parent branch switches are refused', t => {
  ...
  renameSync(f.workspace, `${f.workspace}-moved`); symlinkSync(`${f.workspace}-moved`, f.workspace);
...
test('nested repositories are rejected instead of hiding their files in a gitlink', t => {
  const f = fixture(t); const nested = join(f.workspace, 'nested'); mkdirSync(nested);
  git(nested, ['init', '-b', 'main']); writeFileSync(join(nested, 'hidden.txt'), 'must not disappear');
  assert.throws(() => createGitReview(f.root, f.workspace, f.evidence), /nested|non-file|submodule/i);
});

test('local delivery disables checkout filters and hooks and does not push to configured remotes', t => {
  const f = fixture(t); const marker = join(f.evidence, 'unexpected-execution');
  git(f.root, ['config', 'filter.bad.smudge', `touch '${marker}'; cat`]);
  git(f.root, ['config', 'filter.bad.required', 'true']);
  const hooks = join(f.evidence, 'hooks'); mkdirSync(hooks);
...
test('oversized displayed diffs are explicitly truncated while their complete trees stay fingerprinted', t => {
  ...
  assert.equal(git(f.root, ['cat-file', '-s', `${review.treeOid}:large.txt`]), String(Buffer.byteLength('large line of changed text\n'.repeat(15000))));
});
...
test('replacing Git metadata at the same path invalidates existing approval even with identical commits', t => {
  ...
  renameSync(join(f.root, '.git'), originalMetadata); cpSync(originalMetadata, join(f.root, '.git'), { recursive: true });
  assert.equal(git(f.root, ['rev-parse', 'HEAD']), review.parentOid);
...
test('same-length dirty parent detection never executes repository clean or process filters', t => {
  for (const kind of ['clean', 'process']) {
    const f = fixture(t); const marker = join(f.evidence, 'outside-marker');
    writeFileSync(join(f.root, '.gitattributes'), '*.txt filter=run\n');
    git(f.root, ['add', '.gitattributes']); git(f.root, ['commit', '-m', 'fixture attributes']);
    git(f.workspace, ['reset', '--hard', 'main']);
    git(f.root, ['config', `filter.run.${kind}`, `touch '${marker}'; ${kind === 'clean' ? 'cat' : 'exit 1'}`]);
    ...
    assert.throws(() => createGitDeliveryPlan(f.root, f.workspace, review, 'Approved'), /uncommitted|parent/i);
    assert.throws(() => readFileSync(marker), { code: 'ENOENT' });
...
test('review retains exact non-UTF8 symlink target bytes', t => {
  ...
  const review = createGitReview(f.root, f.workspace, f.evidence);
  const retained = execFileSync('/usr/bin/git', ['-C', f.root, 'show', `${review.treeOid}:raw-link`]);
