...
test('synchronous subprocess cleanup preserves uncertainty through wrapped errors', () => {
  assert.equal(
    workspaceCleanupConfirmed(
      Object.assign(new Error('git exited'), { status: 1, signal: null, pid: 42 }),
    ),
    true,
  );
  for (const details of [
    ...
    const error = new Error('outer persistence boundary', {
      cause: new Error('operation wrapper', {
        cause: Object.assign(new Error('subprocess'), details),
      }),
    });
...
test('Git wrappers preserve timeout evidence and workspace discovery cannot reinterpret a timeout as a non-Git folder', () => {
  ...
  try {
    assert.throws(
      () => runSafeGit('/fixture', ['status']),
      (error) =>
        error.message.startsWith('Git operation failed:') &&
        workspaceCleanupConfirmed(error) === false,
    );
    assert.throws(
      () => plannedConversationWorkspace('/fixture', '00000000-0000-0000-0000-000000000001'),
      (error) => workspaceCleanupConfirmed(error) === false,
    );
  } finally {
