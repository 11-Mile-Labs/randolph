  ...
  const adapter = {
    ...
    async run(input) {
      ...
      input.onEvent({
        ...
        data: { threadId: 'context-thread', turnId: `context-turn-${calls.length}` },
      });
      return { status: result };
    },
  };
  const data = join(root, 'data');
  const runtime = new Runtime(adapter, data);
...
async function settle(runtime) {
  for (let i = 0; i < 200 && runtime.hasActiveWork(); i++)
...
test('runs retain approved context revisions and checkpoint reruns use them after configuration changes', async (t) => {
  const f = fixture(t);
  const before = writeProjectContext(
    f.project,
    { purpose: 'Original purpose', instructions: 'First guidance', documents: [] },
    null,
  );
  const conversation = f.runtime.createConversation(f.registered.id);
  const run = await f.runtime.send({ conversationId: conversation.id, text: 'Inspect' });
  ...
  const checkpoint = new Checkpoints(f.runtime.store).capture(
    f.runtime.snapshot().runs.find((item) => item.id === run.id),
    'completed-turn',
  );
  const after = writeProjectContext(
    f.project,
    { purpose: 'Updated purpose', instructions: 'Later guidance', documents: [] },
    before.revision,
  );
  const rerun = await f.runtime.rerunFromCheckpoint({
    runId: run.id,
  ...
  assert.equal(
    f.calls[1].messages.some((message) => message.text.includes('Updated purpose')),
    false,
  );
  const next = await f.runtime.send({ conversationId: conversation.id, text: 'Inspect again' });
  await settle(f.runtime);
...
test('invalid approved context prevents native dispatch without recording a run', async (t) => {
  const f = fixture(t);
  writeFileSync(join(f.project, 'config.project.yaml'), 'schemaVersion: 1\npurpose: [broken\n');
  const conversation = f.runtime.createConversation(f.registered.id);
  await assert.rejects(
    f.runtime.send({ conversationId: conversation.id, text: 'Inspect' }),
    /valid YAML/,
  );
...
test('frozen approved context survives runtime close and explicit same-conversation restart', async (t) => {
  ...
  const conversation = f.runtime.createConversation(f.registered.id);
  const original = await f.runtime.send({
  ...
  assert.equal(source.projectContext.revision, before.revision);
  await f.close();
  writeProjectContext(
    f.project,
    { purpose: 'New purpose', instructions: 'New guidance', documents: [] },
    before.revision,
  );
  ...
  const result = await reopened.restartRun({
    runId: source.id,
  ...
  assert.equal(
    f.calls[1].messages.some((message) => message.text.includes('New purpose')),
    false,
  );
});
...
test('legacy checkpoint without project context does not inject newer settings on rerun', async (t) => {
  ...
  const result = await f.runtime.rerunFromCheckpoint({
    runId: source.id,
    checkpointDigest: checkpoint.digest,
  });
  await settle(f.runtime);
