...
function fixture(t, result = 'completed') {
 ...
 const calls=[];const adapter={async installations(){return[{executable:'/fixture'}];},async discover(){return{available:true,authenticated:true,cleanupVerified:true,executable:'/fixture',version:'1',models:[{id:'fixture',name:'Fixture',efforts:['low'],defaultEffort:'low'}],executionModes:['read-only']};},async run(input){calls.push(input);input.onEvent({type:'session.turn-started',summary:'fixture turn established',data:{threadId:'context-thread',turnId:`context-turn-${calls.length}`}});return{status:result};}};
