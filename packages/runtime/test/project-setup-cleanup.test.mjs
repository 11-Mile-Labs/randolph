...
const response = JSON.stringify({
  context: { purpose: 'Fixture project', instructions: '', documents: [] },
  evidence: [],
  questions: [],
});
async function fixture(t) {
  ...
  const adapter = {
    ...
    async run(input) {
      ...
      input.onEvent({
        ...
        data: { threadId: 'setup-cleanup-thread', turnId: `setup-cleanup-turn-${calls.length}` },
      });
      input.onEvent({
        type: 'message.delta',
        summary: 'Proposal',
        data: { messageId: 'answer', text: response },
      });
      return { status: 'completed' };
    },
  };
  const state = {
    runtime: new Runtime(adapter, join(root, 'data'), { executionOrigin: () => firstBoot }),
    calls,
    adapter,
  };
  ...
  state.reopen = async (origin) => {
    await state.runtime.close();
    state.runtime = new Runtime(adapter, join(root, 'data'), { executionOrigin: () => origin });
  };
  t.after(async () => {
    await state.runtime.close();
  ...
  state.interrupt = (origin = firstBoot, status = 'running') => {
    ...
    session.error = 'Fixture simulates app ownership loss.';
    if (origin === undefined) delete session.origin;
    else session.origin = origin;
    state.runtime.store.db
      .prepare('UPDATE delegation_sessions SET document=? WHERE id=?')
      .run(JSON.stringify(session), session.id);
    const operation = new NativeOperationRecords(state.runtime.store)
      .list({ runId: state.runId })
      .find((item) => item.purpose === 'model-turn' && item.sessionId === session.id);
    assert.ok(operation, 'fixture requires the main session native operation');
    operation.state = 'admitted';
    delete operation.terminalStatus;
    delete operation.cleanupConfirmed;
    delete operation.cleanupEvidence;
    if (origin === undefined) delete operation.origin;
    else operation.origin = origin;
    state.runtime.store.db
      .prepare('UPDATE native_operations SET document=? WHERE id=?')
      .run(JSON.stringify(operation), operation.id);
    const workspaceRow = state.runtime.store.db
      .prepare(
        "SELECT id, document FROM workspace_ownership WHERE json_extract(document, '$.provenance.kind')='run' AND json_extract(document, '$.provenance.id')=?",
      )
      .get(state.runId);
    assert.ok(workspaceRow, 'fixture requires original workspace ownership');
    const workspace = JSON.parse(workspaceRow.document);
    workspace.state = 'active';
    delete workspace.cleanupEvidence;
    if (origin === undefined) delete workspace.provenance.origin;
    else workspace.provenance.origin = origin;
    state.runtime.store.db
      .prepare('UPDATE workspace_ownership SET document=? WHERE id=?')
      .run(JSON.stringify(workspace), workspaceRow.id);
  };
  return state;
}
...
test('a verified later boot reconciles setup cleanup without approving or restarting old work', async (t) => {
  ...
  assert.equal(
    f.runtime.store.events(f.runId).filter((event) => event.type === 'run.cleanup-reconciled')
      .length,
    1,
  );
  assert.equal(
    f.runtime.store
      .events(f.runId)
      .filter((event) => event.type === 'delegation.session-cleanup-reconciled').length,
    1,
  );
  await f.reopen(laterBoot);
  ...
  await f.runtime.inspectProject(f.request);
...
for (const [name, origin, current] of [
  ...
  test(`setup cleanup stays quarantined with ${name}`, async (t) => {
    ...
    f.interrupt(origin);
    if (origin === undefined) {
      const run = f.runtime.store.runs()[0];
      delete run.executionOrigin;
...
test('reconciliation transaction failure preserves quarantine and stop-unconfirmed cannot remain active after reconciliation', async (t) => {
  const f = await fixture(t);
  f.interrupt(firstBoot, 'stop-unconfirmed');
  await f.reopen(laterBoot);
  const append = f.runtime.store.append.bind(f.runtime.store);
  f.runtime.store.append = (...args) => {
    if (args[1] === 'run.cleanup-reconciled') throw new Error('Storage unavailable');
    return append(...args);
  };
  ...
  assert.equal(
    new DelegationRecords(f.runtime.store).sessions(f.runId)[0].state,
    'cleanup-unconfirmed',
  );
...
test('setup cleanup displays a retained session origin blocker and leaves it quarantined', async (t) => {
  const f = await fixture(t);
  f.interrupt();
  const session = new DelegationRecords(f.runtime.store).sessions(f.runId)[0];
  ...
  assert.throws(
    () => f.runtime.reconcileProjectSetupCleanup(f.projectId),
    /original Mac execution record is missing or malformed/,
  );
  assert.equal(
    new DelegationRecords(f.runtime.store).sessions(f.runId)[0].state,
    'cleanup-unconfirmed',
...
for (const [name, change, expected] of [
  [
    ...
    /read-only project setup sessions/,
  ],
  [
    'worker session',
    (f) => {
    ...
    /worker, verification, task, or tool sessions/,
  ],
  [
    'application-tool receipt',
    (f) => {
      const session = new DelegationRecords(f.runtime.store).sessions(f.runId)[0];
      f.runtime.store.db
    ...
    /task or application-tool authority/,
  ],
]) {
...
test('setup cleanup blocks retained delegation control activity with the shared eligibility reason', async (t) => {
  ...
  try {
    f.runtime.store.db
      .prepare('INSERT INTO delegation_controls VALUES (?, ?, ?)')
      .run(f.runId, 'fixture-authorization', JSON.stringify({ activities: [] }));
  } finally {
    f.runtime.store.db.exec('PRAGMA foreign_keys=ON');
  }
  const records = new DelegationRecords(f.runtime.store);
  const expectedReason = records.setupSessionCleanupReason(f.runId, laterBoot);
  const cleanup = f.runtime.projectSetup(f.projectId).cleanup;
  assert.equal(
    expectedReason,
    'Setup cleanup reconciliation cannot settle delegation control activity.',
  );
  assert.throws(
    () => records.reconcileProjectSetupSessions(f.runId, laterBoot, false),
...
test('cleanup reconciliation cannot interrupt inspection admission or release an owned active run', async (t) => {
  ...
  for (let i = 0; i < 100 && typeof release !== 'function'; i++)
    await new Promise((resolve) => setTimeout(resolve, 5));
...
test('setup cleanup displays the original workspace origin blocker before the action is offered', async (t) => {
  const f = await fixture(t);
  f.interrupt();
  const row = f.runtime.store.db
    .prepare(
      "SELECT id, document FROM workspace_ownership WHERE json_extract(document, '$.state')='active'",
    )
    .get();
  const workspace = JSON.parse(row.document);
  delete workspace.provenance.origin;
  f.runtime.store.db
    .prepare('UPDATE workspace_ownership SET document=? WHERE id=?')
    .run(JSON.stringify(workspace), row.id);
  await f.reopen(laterBoot);
  const cleanup = f.runtime.projectSetup(f.projectId).cleanup;
  assert.equal(cleanup.canReconcile, false);
  assert.match(cleanup.reason, /original Mac execution record is missing or malformed/);
