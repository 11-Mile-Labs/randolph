...
async function fixture(t) {
  const root = await mkdtemp(join(tmpdir(), 'randolph-control-'));
  ...
  store.putProject({ id: 'project', root: '/tmp/project' });
  ...
  const assignment = (id, role, dependencies) => ({
    ...
    deliverables: ['report'],
    completionCriteria: ['report completed'],
  });
  const plan = records.recordPlan({
    ...
    plan: {
      ...
      id: 'plan',
      revision: 1,
      limits: { maxWorkers: 4, maxParallel: 2, maxAttempts: 1, activeMinutes: 1 },
      assignments: [
...
test('accounting charges the union of runtime and native work and retains individual durations', async (t) => {
  ...
  const prep = controls.begin('run', 1, 'prep', 'source-preparation');
  advance(100);
  const native = controls.begin('run', 1, 'main', 'native-session');
  advance(200);
  controls.finish('run', prep.token, { confirmed: true, evidence: { files: 'settled' } });
  advance(300);
  const settled = controls.finish('run', native.token, {
    confirmed: true,
    evidence: { processes: 'gone' },
  });
  assert.equal(settled.spentMs, 600);
  assert.deepEqual(
    settled.activities.map((item) => item.elapsedMs),
    [300, 500],
  );
  advance(5000);
  assert.equal(controls.tick('run').spentMs, 600);
  assert.throws(
    () =>
      controls.finish('run', native.token, { confirmed: true, evidence: { processes: 'gone' } }),
    /stale|settled/,
  );
});
test('pause and stop invalidate admissions while old generation activity can settle exactly once', async (t) => {
  const { controls, advance } = await fixture(t);
  const activity = controls.begin('run', 1, 'verify', 'verification');
  advance(20);
  const paused = controls.command('run', 1, 'pause');
  assert.equal(controls.status(paused), 'pausing');
  ...
  const stopped = controls.command('run', paused.revision, 'stop');
  assert.equal(controls.status(stopped), 'stopping');
  advance(10);
  const settled = controls.finish('run', activity.token, {
    confirmed: true,
    evidence: { exit: true },
  });
  assert.equal(controls.status(settled), 'stopped');
  assert.equal(settled.spentMs, 30);
...
test('a tick detects exhaustion during open activity and extension preserves spent time without resuming', async (t) => {
  const { controls, advance } = await fixture(t);
  const activity = controls.begin('run', 1, 'integrate', 'integration-preparation');
  advance(60_001);
  const exhausted = controls.tick('run');
  assert.equal(exhausted.desired, 'paused');
  assert.equal(exhausted.generation, 2);
  assert.equal(exhausted.revision, 2);
  assert.equal(controls.status(exhausted), 'pausing');
  assert.throws(() => controls.begin('run', 1, 'late', 'checkpoint'), ExecutionAdmissionClosed);
  ...
  const extended = controls.extendBudget('run', exhausted.revision, 30_000);
  assert.equal(extended.spentMs, 60_001);
  assert.equal(extended.budgetMs, 90_000);
  assert.equal(extended.generation, 2);
  assert.equal(extended.revision, 3);
  assert.equal(extended.desired, 'paused');
...
test('a legitimate Stop still closes admission when its own accounting tick exhausts the budget', async (t) => {
  const { controls, advance } = await fixture(t);
  controls.begin('run', 1, 'main', 'native-session');
  advance(60_001);
  const stopped = controls.command('run', 1, 'stop');
  assert.equal(stopped.desired, 'stopped');
  assert.equal(controls.status(stopped), 'stopping');
  assert.equal(stopped.spentMs, 60_001);
});
test('uncertain cleanup closes admission but other old activities still settle honestly', async (t) => {
  const { controls, advance } = await fixture(t);
  const first = controls.begin('run', 1, 'first', 'native-session'),
    second = controls.begin('run', 1, 'second', 'native-session');
  advance(50);
  const unknown = controls.finish('run', first.token, { confirmed: false });
  assert.equal(controls.status(unknown), 'interrupted');
  const settled = controls.finish('run', second.token, {
    confirmed: true,
    evidence: { processes: 'gone' },
  });
  assert.equal(settled.activities[1].state, 'settled');
  assert.equal(settled.activities[0].state, 'cleanup-unconfirmed');
  assert.throws(() => controls.command('run', settled.revision, 'resume'), /cleanup/);
  assert.throws(
    () => controls.finish('run', first.token, { confirmed: true, evidence: { claim: true } }),
    /reconciliation/,
  );
});
test('reopen charges open work conservatively and never silently resumes active or queued work', async (t) => {
  const { root, store, controls, advance, time } = await fixture(t);
  controls.begin('run', 1, 'writer', 'native-session');
  advance(2500);
  store.close();
  const reopened = new Store(root);
  const recovered = new DelegationControls(reopened, time);
  const [value] = recovered.reconcileOnReopen();
  assert.equal(value.spentMs, 2500);
  assert.equal(value.activities[0].state, 'cleanup-unconfirmed');
  assert.equal(value.recoveryRequired, true);
  assert.equal(value.desired, 'paused');
  advance(200);
  assert.deepEqual(recovered.reconcileOnReopen(), [value]);
  assert.throws(
    () => recovered.begin('run', value.generation, 'replay', 'native-session'),
    /closed/,
  );
  reopened.close();
});
test('reopen with only queued work requires an explicit recovery decision and clock rollback never refunds', async (t) => {
  const { controls, advance } = await fixture(t);
  const idle = controls.reconcileOnReopen()[0];
  assert.equal(idle.recoveryRequired, true);
  assert.equal(idle.spentMs, 0);
  assert.throws(() => controls.command('run', idle.revision, 'resume'), /cleanup/);
  const other = await fixture(t);
  other.controls.begin('run', 1, 'main', 'native-session');
  other.advance(100);
  other.controls.tick('run');
  other.advance(-200);
  const rollback = other.controls.tick('run');
  assert.equal(rollback.spentMs, 100);
  assert.equal(rollback.recoveryRequired, true);
  assert.equal(rollback.activities[0].state, 'cleanup-unconfirmed');
  advance(1);
});
test('current authorization, unique activity identities, evidence, and priority generations are enforced', async (t) => {
  ...
  const priority = controls.setPriority('run', 1, 8);
  ...
  const activity = controls.begin('run', 1, 'main', 'native-session');
  assert.throws(() => controls.begin('run', 1, 'main', 'native-session'), /already used/);
  ...
  assert.throws(() => controls.create('run', 'foreign'), /already exists/);
  assert.ok(store.events('run').some((event) => event.type === 'delegation.control-priority'));
});
test('activity and its event roll back together on failed persistence', async (t) => {
  ...
  store.append = (run, type, ...args) => {
    if (type === 'delegation.control-activity-began') throw new Error('disk fault');
    original(run, type, ...args);
  };
  assert.throws(() => controls.begin('run', 1, 'fault', 'native-session'), /disk fault/);
  assert.equal(controls.read('run').activities.length, 0);
  store.append = original;
  assert.equal(controls.begin('run', 1, 'fault', 'native-session').state, 'active');
});
...
test('begin commits budget expiry and reports typed closed admission using one clock observation', async (t) => {
  ...
  let calls = 0;
  const expiring = new DelegationControls(f.store, () => {
    calls++;
    return f.time() + 60_001;
  });
  assert.throws(
    () => expiring.begin('run', 1, 'must-not-start', 'verification'),
    ExecutionAdmissionClosed,
  );
  assert.equal(calls, 1);
  assert.equal(expiring.read('run').desired, 'paused');
  assert.equal(expiring.read('run').spentMs, 60_001);
...
test('begin never makes a rollback-prone second clock charge inside admission', async (t) => {
  const f = await fixture(t);
  f.controls.begin('run', 1, 'open', 'checkpoint');
  let calls = 0;
  const clock = new DelegationControls(f.store, () => f.time() + (++calls === 1 ? 1 : 60_001));
  clock.begin('run', 1, 'admitted', 'verification');
  assert.equal(calls, 1);
  assert.equal(clock.read('run').spentMs, 1);
  clock.tick('run');
  assert.equal(clock.read('run').desired, 'paused');
  assert.equal(clock.read('run').spentMs, 60_001);
});
...
test('priority and nonexhausting budget edits advance only revision, leaving admitted generation valid', async (t) => {
  ...
  assert.equal(extended.generation, 1);
  assert.equal(extended.revision, 3);
  assert.equal(extended.desired, 'running');
...
test('capacity-one queue reprioritizes B by control revision without restarting queued A or waiting for B to settle', async (t) => {
  ...
  const plan = records.recordPlan({
    ...
    plan: {
      ...
      assignments: [assignment, synthesis],
    },
  });
  records.readyPlan({
    runId: 'run-b',
  ...
  const starts = [];
  let releaseBlocker, releaseB, signalB;
  const beganB = new Promise((resolve) => {
    signalB = resolve;
  });
  t.after(async () => {
    releaseBlocker?.();
  ...
  const blocker = admission.perform(
    'codex',
    { owner: { kind: 'app-discovery', id: 'blocker' }, priority: () => 0 },
    'discovery',
    undefined,
    undefined,
    async () => {
  ...
  const operation = (id, runId, control, release) =>
    admission.perform(
      'codex',
      {
        owner: { kind: 'delegation', id },
        runId,
        generation: control.read(runId).generation,
  ...
  releaseB();
  await Promise.all([queuedB, queuedA]);
