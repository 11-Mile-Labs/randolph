...
function fixture(t, clock) {
  ...
  const plan = records.recordPlan({
    ...
    plan: {
      ...
      assignments: [
        assignment('worker', 'worker', [], 'run-basis', { producesSource: true }),
        assignment('integrate', 'main-integration', ['worker'], 'output:worker', {
          producesSource: true,
          integrationInputs: ['worker'],
          repairAttempts: 0,
        }),
        assignment('verify', 'runtime-verification', ['integrate'], 'output:integrate', {
          producesSource: true,
        }),
        assignment('synthesis', 'main-synthesis', ['verify'], 'output:verify'),
      ],
    },
  });
  const revision = {
  ...
  const initial = records.tasks(run.id),
    producer = initial.find((item) => item.assignmentId === 'integrate');
  const source = {
    checkpointDirectory: checkpoint.directory,
    checkpointDigest: checkpoint.digest,
    treeOid: checkpoint.snapshotTreeOid,
    producerTaskId: producer.id,
  };
  for (const task of initial.filter((item) =>
    ['worker', 'integrate'].includes(item.assignmentId),
  )) {
    ...
    task.attempts = [
      {
        ...
        result: { success: true, source: { ...source, producerTaskId: task.id } },
      },
    ];
    store.db
  ...
  const task = records.tasks(run.id).find((item) => item.assignmentId === 'verify'),
  ...
  return {
    ...
    run,
    auth,
    checks: new DelegationChecks(store),
  };
}
function adapter(overrides = {}) {
  return {
    ...
    async run() {
      throw new Error('Checks cannot invoke a model turn.');
    },
    async runCommand(input) {
      input.onDispatch({ processId: randomUUID() });
      input.onOutput('check output');
      return { exitCode: 0, output: 'check output', cleanupVerified: true, truncated: false };
    },
    ...overrides,
  };
}
const executor = (f, value) =>
  new DelegationVerificationExecutor(
    f.store,
    f.controls,
    f.admission,
    f.leases,
...
test('native verification runs one exact command per admission, retains all identities, and completes only after all checks', async (t) => {
  const f = fixture(t),
    seen = [];
  const native = executor(
    f,
    adapter({
      async runCommand(input) {
        seen.push(input);
        return adapter().runCommand(input);
      },
    }),
  );
  ...
  assert.ok(
    seen.every(
      (value) =>
        value.executable === selection.executable &&
        value.executableVersion === selection.executableVersion,
    ),
  );
  ...
  assert.equal(f.controls.read('run').activities.length, 6);
...
test('Pause during a check keeps its pass; explicit Resume runs only the next check', async (t) => {
  ...
  const native = executor(
    ...
    adapter({
      async runCommand(input) {
        calls++;
        const result = await adapter().runCommand(input);
  ...
  assert.equal((await native.runNext(f.input)).status, 'next-check');
  assert.throws(
    () => f.tasks.assertRuntimeStage({ ...f.input, expectedGeneration: 2 }),
    /running control/,
  );
  const resumed = f.controls.command('run', 2, 'resume');
  assert.equal(
    (await native.runNext({ ...f.input, expectedGeneration: resumed.generation })).status,
    'passed',
  );
...
test('Pause during discovery defers the undispatched command with cleanup; Resume reclaims without stale evidence', async (t) => {
  ...
  const native = executor(
    ...
    adapter({
      async discover() {
        if (!discoveries++) f.controls.command('run', 1, 'pause');
        return info;
      },
      async runCommand(input) {
        calls++;
  ...
  assert.equal((await native.runNext(f.input)).status, 'pending');
...
test('concurrent executors cannot dispatch or settle the same command twice', async (t) => {
  ...
  const first = executor(f, value).runNext(f.input);
  await started;
  const duplicate = await executor(f, value).runNext(f.input);
  assert.equal(duplicate.status, 'failed');
  assert.match(duplicate.error, /cannot claim/);
  assert.equal(f.records.sessions('run')[0].state, 'prepared');
  release();
  assert.equal((await first).status, 'next-check');
  assert.equal(f.capacity.snapshot().occupied, 0);
});
...
test('missing command capability and command identity are failures without model or fallback execution', async (t) => {
  ...
  const unavailable = await executor(
    ...
    adapter({
      async discover() {
        return { ...info, commandLifecycle: false };
      },
      async runCommand() {
        called = true;
      },
  ...
  assert.equal(f.checks.snapshot(f.input).checks[0].state, 'unavailable');
  const g = fixture(t);
  ...
  assert.equal(g.checks.passed(g.input), false);
  assert.equal(g.records.sessions('run')[0].native, undefined);
});
...
test('a check that changes eligible source fails and blocks later checks', async (t) => {
  ...
  const native = executor(
    ...
    adapter({
      async runCommand(input) {
        const result = await adapter().runCommand(input);
        writeFileSync(join(input.workspace, 'source.txt'), 'changed');
        return result;
      },
    }),
  );
  assert.equal((await native.runNext(f.input)).status, 'failed');
  assert.equal(f.checks.snapshot(f.input).checks[0].state, 'failed');
  assert.equal((await native.runNext(f.input)).status, 'failed');
});
...
test('unknown command cleanup retains capacity quarantine and rejects later work', async (t) => {
  ...
  const result = await executor(
    ...
    adapter({
      async runCommand(input) {
        const result = await adapter().runCommand(input);
        return { ...result, cleanupVerified: false };
      },
    }),
  ).runNext(f.input);
  assert.equal(result.cleanupConfirmed, false);
  assert.equal(f.capacity.snapshot().occupied, 1);
...
test('Stop invalidates a command during native startup before its dispatch callback', async (t) => {
  ...
  const result = await executor(
    ...
    adapter({
      async runCommand(input) {
        f.controls.command('run', 1, 'stop');
        try {
          input.onDispatch({ processId: 'must-not-dispatch' });
          dispatched = true;
        } catch {
          return {
            exitCode: null,
...
test('shared deadline prevents command binding during cleanup grace and late results cannot clear quarantine', async (t) => {
  ...
  f.admission.perform = (harness, context, purpose, ...args) =>
    perform(
      harness,
      { ...context, ...(purpose === 'command' ? { timeoutMs: 10, cleanupTimeoutMs: 10 } : {}) },
      purpose,
      ...args,
    );
  const result = await executor(
    ...
    adapter({
      async runCommand(input) {
        ...
        input.signal.addEventListener(
          ...
          () => {
            assert.throws(
              () => input.onDispatch({ processId: 'after-deadline' }),
              /late or duplicated/,
            );
            graceRejected = true;
          },
  ...
  assert.equal(graceRejected, true);
  assert.equal(result.cleanupConfirmed, false);
  const operations = f.admission.records.list(),
    checks = f.checks.snapshot(f.input);
  assert.equal(operations.at(-1).state, 'quarantined');
  assert.equal(checks.checks[0].commandId, undefined);
  assert.throws(() => raw.onDispatch({ processId: 'late' }), /late or duplicated/);
  raw.onOutput('late output');
  late({ exitCode: 0, output: 'late pass', cleanupVerified: true, truncated: false });
  await new Promise((resolve) => setImmediate(resolve));
  assert.deepEqual(f.admission.records.list(), operations);
  assert.deepEqual(f.checks.snapshot(f.input), checks);
  assert.equal(f.capacity.snapshot().occupied, 1);
});
