...
function fixture(t) {
  const root = realpathSync(mkdtempSync(join(tmpdir(), 'randolph-workspace-recovery-'))),
    store = new Store(join(root, 'data'));
  const project = { id: 'project', root: join(root, 'project') },
    conversation = {
      id: 'conversation',
...
test('imports deterministic missing-path main, review delivery, integration, and unknown push witnesses without filesystem probes', (t) => {
  ...
  const review = {
    ...
    status: 'delivering',
    basis: {
      root: join(f.root, 'missing-parent'),
    ...
    verification: { checks: [{ cleanupVerified: false }] },
    originOperation: 'cleanup-unconfirmed',
  ...
  f.store.putRun({
    ...
    integration: {
      status: 'applying',
      plan: { basis: { root: join(f.root, 'integration-parent') } },
    },
  });
  reconstructLegacyWorkspaceOwnership(f.store, f.ownership);
  const retained = f.ownership.snapshot();
  assert.ok(
    retained.some((item) => item.provenance?.kind === 'run' && item.workspace === f.run.workspace),
  );
  assert.ok(
    retained.some(
      (item) => item.provenance?.kind === 'delivery' && item.workspace === review.basis.root,
    ),
  );
  assert.ok(
    retained.some(
      (item) => item.provenance?.kind === 'push' && item.workspace === review.basis.root,
    ),
  );
  assert.ok(
    retained.some(
      (item) =>
        item.provenance?.kind === 'integration' &&
        item.workspace === join(f.root, 'integration-parent'),
    ),
  );
  assert.ok(
    retained.some(
      (item) =>
        item.provenance?.kind === 'verification' && item.workspace === review.basis.workspace,
    ),
  );
...
test('an exact existing receipt suppresses only its original witness and repeated recovery preserves originals', (t) => {
  ...
  f.ownership.retainUncertain({
    ...
    cleanupEvidence: { reconciliation: 'legacy-run', fixture: 'prior' },
  });
  reconstructLegacyWorkspaceOwnership(f.store, f.ownership);
  ...
  f.ownership.release({
    ...
    cleanupEvidence: { fixture: 'known-clean' },
  });
  reconstructLegacyWorkspaceOwnership(f.store, f.ownership);
...
test('imports a retained ordinary dispatching attempt before task reconciliation can mark recovery', (t) => {
  const f = fixture(t),
    workspace = join(f.root, 'attempt-workspace');
  const task = {
    id: 'task',
    runId: f.run.id,
    authorizationId: 'legacy-auth',
    assignmentId: 'worker',
    dependencies: [],
    workspaceIdentity: '',
    contextArtifacts: [],
    ...
    attempts: [
      {
        ...
        createdAt: f.run.createdAt,
        updatedAt: f.run.updatedAt,
        workspace: { path: workspace, identity: { device: 1, inode: 1 } },
      },
    ],
    createdAt: f.run.createdAt,
    updatedAt: f.run.updatedAt,
  ...
  f.store.db
    ...
    .run(task.id, f.run.id, task.authorizationId, JSON.stringify(task));
  ...
  assert.ok(
    f.ownership
      ...
      .some(
        (item) =>
          item.provenance?.kind === 'delegation-preparation' &&
          item.provenance.id === 'attempt' &&
...
test('imports a retained pushing record before the Pushes constructor can quarantine it', (t) => {
  ...
  const review = {
    ...
    cleaned: true,
    basis: {
      root: join(f.root, 'pushing-parent'),
      workspace: f.run.workspace,
      evidenceDir: join(f.root, 'evidence'),
      treeOid: 'tree',
      parentOid: 'parent',
    ...
    push: { revision: 'pending', plan: {}, status: 'pushing' },
  };
  f.store.putReview(review);
  reconstructLegacyWorkspaceOwnership(f.store, f.ownership);
  assert.ok(
    f.ownership
      ...
      .some(
        (item) =>
          item.provenance?.kind === 'push' &&
          item.provenance.id === review.id &&
          item.workspace === review.basis.root,
      ),
  );
});
