...
function fixture(t) {
  ...
  const runtime = new Runtime(
    {
      ...
      async run(input) {
        ...
        input.onEvent({
          ...
          data: { threadId: 'memory-thread', turnId: `memory-turn-${calls.length}` },
        });
  ...
  const first = runtime.addProject(join(root, 'a'));
  const second = runtime.addProject(join(root, 'b'));
  t.after(async () => {
    await runtime.close();
...
function create(f, project, title, text, scope = { kind: 'project', projectId: project.id }) {
  return f.runtime
    .memoryCommand({ projectId: project.id, action: 'create', draft: { scope, title, text } })
    .lessons.at(-1);
}
...
test('only relevant approved scoped lessons reach the agent and exact context survives later edits', async (t) => {
  ...
  const approved = create(
    ...
    f.first,
    'Database migrations',
    'Migrations must preserve existing rows.',
  );
  ...
  f.runtime.memoryCommand({
    projectId: f.first.id,
    action: 'approve',
    references: [reference(approved), reference(unrelated)],
  });
  f.runtime.memoryCommand({
    projectId: f.second.id,
    action: 'approve',
    references: [reference(foreign)],
  });
  const conversation = f.runtime.createConversation(f.first.id);
  const run = await f.runtime.send({
    conversationId: conversation.id,
    text: 'Review database migrations',
  });
  ...
  f.runtime.memoryCommand({
    ...
    patch: { text: 'Changed after dispatch.' },
  });
  ...
  assert.equal(
    f.runtime.memorySnapshot(f.first.id).lessons.find((item) => item.lessonId === approved.lessonId)
      .status,
    'draft',
  );
});
...
test('edited pinned lessons block dispatch until explicitly repinned or removed', async (t) => {
  ...
  f.runtime.memoryCommand({
    ...
    action: 'edit',
    reference: reference(lesson),
    patch: { text: 'Keep revised evidence.' },
  });
  const conversation = f.runtime.createConversation(f.first.id);
  await assert.rejects(
    f.runtime.send({ conversationId: conversation.id, text: 'Inspect files' }),
    /pinned|memory/i,
  );
...
test('project approval preferences use editable YAML with stale-write protection and independent global policy', (t) => {
  ...
  f.runtime.memoryCommand({
    ...
    action: 'settings',
    scope: 'project',
    value: { autoApprove: true, frameworks: { React: '19' } },
    expectedRevision: initial.projectSettings.revision,
  });
  assert.equal(create(f, f.first, 'React effects', 'Keep effects bounded.').status, 'approved');
  assert.equal(
    create(f, f.first, 'Global approval', 'Requires separate approval.', { kind: 'global' }).status,
    'draft',
  );
  ...
  assert.throws(
    () =>
      f.runtime.memoryCommand({
        ...
        action: 'settings',
        scope: 'project',
        value: { autoApprove: false, frameworks: {} },
        expectedRevision: saved.projectSettings.revision,
      }),
    /changed|reload/i,
  );
