...
test('unfinished delegation sessions quarantine even a completed aggregate run across reopenings', async (t) => {
  ...
  new DelegationRecords(store).recordSession({
    id: randomUUID(),
    runId,
    role: 'main',
    harness: 'codex',
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      assert.equal(run.cleanupUnconfirmed, true);
      assert.equal(
        new DelegationRecords(runtime.store).sessions(runId)[0].state,
        'cleanup-unconfirmed',
      );
      await assert.rejects(
        runtime.send({ conversationId, text: 'Continue' }),
        /already has active work/,
      );
