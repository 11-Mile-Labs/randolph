...
const git = (root, ...args) => execFileSync('/usr/bin/git', ['-c', 'init.templateDir=', '-c', 'core.hooksPath=/dev/null', '-c', 'commit.gpgsign=false', '-c', 'user.name=Fixture', '-c', 'user.email=fixture@example.invalid', '-C', root, ...args], { encoding: 'utf8' }).trim();
function fixture(t, clock) {
  const root = realpathSync(mkdtempSync(join(tmpdir(), 'randolph-coordinator-'))), projectRoot = join(root, 'project'); mkdirSync(projectRoot);
  git(projectRoot, 'init', '-b', 'main'); writeFileSync(join(projectRoot, '.gitignore'), '.worktrees/\n'); writeFileSync(join(projectRoot, 'package.json'), JSON.stringify({ packageManager: 'pnpm@10.0.0', scripts: { lint: 'echo lint', test: 'echo test' } })); git(projectRoot, 'add', '.'); git(projectRoot, 'commit', '-m', 'fixture');
  const store = new Store(join(root, 'data')), records = new DelegationRecords(store), controls = new DelegationControls(store, () => clock ? clock(store) : Date.now());
  const conversationId = randomUUID(), workspace = prepareWorkspace(projectRoot, conversationId);
  const run = { createdAt: '2026-09-12T00:00:00.000Z', ...selection, id: 'run', projectId: 'project', conversationId, status: 'completed', workspace, workspaceIdentity: workspaceIdentity(workspace), enabledHarnessRoutes: [{ harness: 'codex', executable: selection.executable }] };
  ...
  const assignment = (id, role, dependencies, source = 'run-basis', extra = {}) => ({ ...selection, id, role, dependencies, source, mode: ['review', 'main-synthesis'].includes(role) ? 'read-only' : 'code', task: id, rationale: 'bounded work', deliverables: ['report'], completionCriteria: ['report retained'], ...extra });
...
function adapter(events, override = {}) {
  return { async discover() { return info; }, async run(input) {
    const { assignment } = JSON.parse(input.messages[0].text); events.push(assignment.id);
    input.onEvent({ type: 'session.turn-started', summary: 'started', data: { threadId: randomUUID(), turnId: randomUUID() } });
    if (assignment.id.startsWith('writer-')) writeFileSync(join(input.workspace, assignment.id + '.txt'), assignment.id);
    else { assert.equal(readFileSync(join(input.workspace, 'writer-a.txt'), 'utf8'), 'writer-a'); assert.equal(readFileSync(join(input.workspace, 'writer-b.txt'), 'utf8'), 'writer-b'); }
    if (assignment.role === 'main-integration') writeFileSync(join(input.workspace, 'integrated.txt'), 'combined');
    if (assignment.role === 'main-synthesis' || assignment.role === 'review') { assert.equal(input.executionMode, 'read-only'); assert.equal(readFileSync(join(input.workspace, 'integrated.txt'), 'utf8'), 'combined'); }
...
test('the coordinator drives two writers through conversation integration, all checks, review, and selected-main synthesis', async t => {
  ...
  const done = await coordinator(f, adapter(events)).drive({ runId: 'run', expectedGeneration: 1, signal: new AbortController().signal });
  assert.deepEqual(done.map(value => [value.assignmentId, value.state, value.attempts.at(-1)?.error]), done.map(value => [value.assignmentId, 'completed', undefined]));
  assert.deepEqual(events.slice(2), ['integrate', 'lint', 'test', 'review', 'synthesis']);
  assert.equal(done.find(value => value.assignmentId === 'integrate').attempts[0].workspace.path, f.run.workspace);
  assert.equal(f.capacity.snapshot().occupied, 0); assert.equal(f.leases.snapshot().length, 0);
  assert.equal(git(f.projectRoot, 'rev-parse', 'HEAD'), head); assert.equal(git(f.projectRoot, 'write-tree'), index); assert.equal(git(f.projectRoot, 'status', '--porcelain'), '');
  assert.ok(f.controls.read('run').activities.every(value => value.state === 'settled'));
  await coordinator(f, adapter(events)).drive({ runId: 'run', expectedGeneration: 1, signal: new AbortController().signal }); assert.equal(events.length, 7);
});

test('coordinator construction launches nothing and a retained active claim prevents duplicate ownership', async t => {
  const f = fixture(t), events = []; let began, release;
  const started = new Promise(resolve => { began = resolve; });
  const native = adapter(events, { async discover() { began(); return await new Promise(resolve => { release = () => resolve(info); }); } });
  const first = coordinator(f, native); assert.deepEqual(events, []);
...
test('budget exhaustion after source preparation resumes its retained receipt without preparing again', async t => {
  let advance = false, at = Date.now();
  const f = fixture(t, store => {
    if (!advance && new DelegationRecords(store).tasks('run').some(task => task.attempts.some(attempt => attempt.preparation?.state === 'completed' && !attempt.workspace))) { advance = true; at += 301_000; }
    return at;
  });
...
test('budget exhaustion after integration apply resumes without reapplying its candidate', async t => {
  let advance = false, at = Date.now();
  const f = fixture(t, store => {
    if (!advance && new DelegationRecords(store).tasks('run').some(task => task.attempts.some(attempt => attempt.integration?.state === 'applied' && !attempt.workspace))) { advance = true; at += 301_000; }
    return at;
  });
  const events = [], native = adapter(events);
  await coordinator(f, native).drive({ runId: 'run', expectedGeneration: 1, signal: new AbortController().signal });
...
test('failed raw integration keeps the conversation writer lease quarantined', async t => {
  const f = fixture(t), events = [], original = DelegationIntegration.prototype.apply;
  DelegationIntegration.prototype.apply = () => { throw new Error('Injected partial raw apply failure'); };
  try {
    await coordinator(f, adapter(events)).drive({ runId: 'run', expectedGeneration: 1, signal: new AbortController().signal });
    assert.ok(f.controls.read('run').activities.some(value => value.state === 'cleanup-unconfirmed'));
    const lease = f.leases.snapshot().find(value => value.workspace === f.run.workspace);
    assert.equal(lease.state, 'cleanup-unconfirmed'); assert.equal(events.length, 2);
...
test('budget expiry inside begin stays paused instead of failing an unlaunched task', async t => {
  let at = Date.now();
  const f = fixture(t, () => at), events = [];
  const active = f.controls.begin('run', 1, 'existing-runtime-stage', 'checkpoint');
  const original = f.controls.begin.bind(f.controls); let inject = true;
  f.controls.begin = (...args) => { if (inject) { inject = false; at += 301_000; } return original(...args); };
  const paused = await coordinator(f, adapter(events)).drive({ runId: 'run', expectedGeneration: 1, signal: new AbortController().signal });
  assert.equal(f.controls.read('run').desired, 'paused'); assert.ok(paused.every(task => task.state !== 'failed')); assert.equal(events.length, 0);
