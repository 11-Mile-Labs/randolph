...
const project = () => ({ id: randomUUID(), name: 'fixture', root: '/fixture', createdAt: new Date().toISOString() });
const conversation = (projectId) => ({ id: randomUUID(), projectId, title: 'fixture', model: 'model', effort: 'low', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), lastReadSequence: 0 });
const run = (projectId, conversationId) => ({ id: randomUUID(), projectId, conversationId, status: 'running', model: 'model', effort: 'low', workspace: '/fixture', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), lastActivityAt: new Date().toISOString() });
...
test('chat event reads ignore the workspace event cap and filter by run and cursor', () => {
  ...
  try {
    ...
    for (let index = 0; index < 2_100; index += 1) store.append(secondRun, 'noise', `noise ${index}`);
    assert.equal(store.snapshot().events.some(event => event.runId === firstRun.id), false);
    ...
    assert.deepEqual(all.events.map(event => event.type), ['message.delta', 'run.completed']);
    assert.deepEqual(store.chatEvents(firstConversation.id, firstRun.id, all.events[0].sequence).events.map(event => event.type), ['run.completed']);
