...
async function fixture(t, run) {
  ...
  const adapter = {
    async discover(executable) {
      return {
        executable: executable ?? '/fixture-codex',
        version: 'fixture-1',
        available: true,
        authenticated: true,
        cleanupVerified: true,
        executionModes: ['read-only'],
        models: [{ id: 'fixture-model', name: 'Fixture', efforts: ['low'], defaultEffort: 'low' }],
      };
    },
    async run(input) {
      calls.push(input);
...
async function settle(runtime) {
  for (let i = 0; i < 200 && runtime.hasActiveWork(); i += 1)
    await new Promise((resolve) => setTimeout(resolve, 5));
  assert.equal(runtime.hasActiveWork(), false);
}

function send(runtime, conversation) {
  return runtime.send({
    conversationId: conversation.id,
    text: 'Record this native session.',
    model: 'fixture-model',
    effort: 'low',
  });
}
...
test('records and binds the main session only from the adapter native turn event', async (t) => {
  const f = await fixture(t, async (input) => {
    input.onEvent({
      ...
      summary: 'thread established',
      data: { threadId: 'thread-one' },
    });
    input.onEvent({
      type: 'session.turn-started',
      summary: 'turn established',
      data: { threadId: 'thread-one', turnId: 'turn-one' },
    });
    return { status: 'completed' };
  });
  const run = await send(f.runtime, f.conversation);
  ...
  assert.deepEqual(
    [
      session.harness,
      session.executable,
      session.executableVersion,
      session.model,
      session.effort,
      session.allowedTools,
    ],
    ['codex', '/fixture-codex', 'fixture-1', 'fixture-model', 'low', []],
  );
...
test('a native adapter failure with confirmed cleanup settles the main session as failed', async (t) => {
  const f = await fixture(t, async (input) => {
    input.onEvent({
      type: 'session.turn-started',
      summary: 'turn established',
      data: { threadId: 'thread-failure', turnId: 'turn-failure' },
    });
...
test('a projection failure after confirmed session cleanup does not create cleanup uncertainty', async (t) => {
  const f = await fixture(t, async (input) => {
    input.onEvent({
      type: 'session.turn-started',
      summary: 'turn established',
      data: { threadId: 'thread-projection', turnId: 'turn-projection' },
    });
    return { status: 'completed' };
  });
...
test('a failure before adapter invocation retains dispatch-not-invoked cleanup evidence', async (t) => {
  ...
  const ownership = f.runtime.workspaceOwnership.acquire({
    reservationId: 'pre-dispatch-workspace',
    runId: run.id,
    workspace: run.workspace,
    provenance: { kind: 'run', id: run.id },
  });
...
test('an unbound completed adapter fails the aggregate run and skips the completed checkpoint', async (t) => {
  ...
  assert.equal(
    persisted.checkpoints?.some((checkpoint) => checkpoint.boundary === 'completed-turn'),
    false,
  );
});
...
test('a bound stop with unconfirmed cleanup quarantines the main session', async (t) => {
  const f = await fixture(t, async (input) => {
    input.onEvent({
      type: 'session.turn-started',
      summary: 'turn established',
      data: { threadId: 'thread-stop', turnId: 'turn-stop' },
    });
  ...
  const [session] = new DelegationRecords(f.runtime.store).sessions(run.id);
  assert.equal(
    f.runtime.snapshot().runs.find((item) => item.id === run.id).status,
    'stop-unconfirmed',
  );
...
test('duplicate identical binding is idempotent and the completed record survives reopen', async (t) => {
  const f = await fixture(t, async (input) => {
    const event = {
      type: 'session.turn-started',
      summary: 'turn established',
      data: { threadId: 'thread-reopen', turnId: 'turn-reopen' },
    };
    input.onEvent(event);
    input.onEvent(event);
    return { status: 'completed' };
  ...
  assert.equal(
    f.runtime.store.events(run.id).filter((event) => event.type === 'delegation.session-bound')
      .length,
    1,
  );
  await f.close();
  const reopened = new Runtime(
    {
      discover: async () => {
        throw new Error('must not discover on reopen');
      },
      run: async () => {
        throw new Error('must not run on reopen');
      },
    },
    f.dataRoot,
