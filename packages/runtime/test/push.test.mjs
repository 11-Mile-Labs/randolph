...
function git(root, args) {
  return execFileSync('/usr/bin/git', ['-c', 'core.hooksPath=/dev/null', '-c', 'commit.gpgsign=false', '-c', 'user.name=Fixture', '-c', 'user.email=fixture@example.invalid', '-C', root, ...args], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }).trim();
}
async function fixture(t) {
  const temporary = await realpath(await mkdtemp(join(tmpdir(), 'randolph-push-')));
  t.after(() => rm(temporary, { recursive: true, force: true }));
  const root = join(temporary, 'project'); const origin = join(temporary, 'origin.git');
  await mkdir(root); await mkdir(origin);
...
test('preview does not push; explicit execution publishes the exact branch once', { timeout: 30_000 }, async t => {
...
async function commit(root, text) {
  ...
  git(root, ['add', 'value.txt']); git(root, ['commit', '-m', text]);
...
test('a proven fast-forward updates only the previewed branch', { timeout: 30_000 }, async t => {
  ...
  assert.equal(git(f.origin, ['for-each-ref', '--format=%(refname)', 'refs/heads/']), 'refs/heads/main');
});

test('local commit or origin changes invalidate the immutable preview without pushing', { timeout: 30_000 }, async t => {
  const f = await fixture(t);
  const plan = await previewOriginPush(f.root, options);
...
test('remote advancement after preview refuses the original push without overwriting', { timeout: 30_000 }, async t => {
  const f = await fixture(t);
  await executeOriginPush(await previewOriginPush(f.root, options), options);
...
test('divergent histories are rejected even though an exact lease could otherwise overwrite them', { timeout: 30_000 }, async t => {
  const f = await fixture(t);
  await executeOriginPush(await previewOriginPush(f.root, options), options);
...
test('unsafe transports, credential URLs and missing local authorization fail before execution', { timeout: 30_000 }, async t => {
  const f = await fixture(t);
  await assert.rejects(previewOriginPush(f.root), /fixture authorization/i);
  for (const url of ['ext::touch forbidden', 'git://example.invalid/repository', 'https://token@example.invalid/repository', 'ssh://git@example.invalid/repository']) {
...
test('an aborted execution leaves origin unchanged and reports uncertain outcome for reconciliation', { timeout: 30_000 }, async t => {
  const f = await fixture(t);
  const plan = await previewOriginPush(f.root, options);
...
test('competing approved fast-forwards from one remote revision cannot overwrite each other', { timeout: 30_000 }, async t => {
  const f = await fixture(t);
  await executeOriginPush(await previewOriginPush(f.root, options), options);
  ...
  const plans = await Promise.all([previewOriginPush(f.root, options), previewOriginPush(peer, options)]);
...
test('completed push remains confirmed after local HEAD and origin settings change', { timeout: 30_000 }, async t => {
  const f = await fixture(t);
  const plan = await previewOriginPush(f.root, options);
  assert.equal((await executeOriginPush(plan, options)).status, 'pushed');
