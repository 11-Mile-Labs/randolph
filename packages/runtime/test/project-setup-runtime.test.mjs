...
function fixture(t){
 ...
 const adapter={async discover(){if(state.onDiscover){const callback=state.onDiscover;state.onDiscover=undefined;callback();}return{executable:'/fixture-codex',version:'fixture-1',available:true,authenticated:true,models:[{id:'fixture',name:'Fixture',efforts:['low'],defaultEffort:'low'}],executionModes:['read-only']};},async run(input){calls.push(input);input.onEvent({type:'session.turn-started',summary:'fixture turn established',data:{threadId:'setup-thread',turnId:`setup-turn-${calls.length}`}});if(state.wait){await new Promise(resolve=>{if(input.signal.aborted)resolve();else input.signal.addEventListener('abort',resolve,{once:true});});return{status:'interrupted'};}input.onEvent({type:'message.delta',summary:'Proposal',data:{messageId:'proposal',text:state.response}});return{status:'completed'};}};
