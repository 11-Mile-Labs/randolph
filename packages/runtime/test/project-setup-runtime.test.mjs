...
const proposal={context:{purpose:'Project purpose',instructions:'Project guidance',documents:[{path:'README.md',description:'Project overview'}]},evidence:['Read README.'],questions:['What is the first milestone?']};
const selection={harness:'codex',model:'fixture',effort:'low'};
function fixture(t){
 const root=realpathSync(mkdtempSync(join(tmpdir(),'randolph-setup-')));const project=join(root,'project');
 const calls=[];const state={response:JSON.stringify(proposal),wait:false,onDiscover:undefined};
 const adapter={async discover(){if(state.onDiscover){const callback=state.onDiscover;state.onDiscover=undefined;callback();}return{executable:'/fixture-codex',version:'fixture-1',available:true,authenticated:true,cleanupVerified:true,models:[{id:'fixture',name:'Fixture',efforts:['low'],defaultEffort:'low'}],executionModes:['read-only']};},async run(input){calls.push(input);input.onEvent({type:'session.turn-started',summary:'fixture turn established',data:{threadId:'setup-thread',turnId:`setup-turn-${calls.length}`}});if(state.wait){await new Promise(resolve=>{if(input.signal.aborted)resolve();else input.signal.addEventListener('abort',resolve,{once:true});});return{status:'interrupted'};}input.onEvent({type:'message.delta',summary:'Proposal',data:{messageId:'proposal',text:state.response}});return{status:'completed'};}};
 const runtime=new Runtime(adapter,join(root,'data'));mkdirSync(project);writeFileSync(join(project,'README.md'),'Uncommitted project idea\n');const registered=runtime.addProject(project);t.after(async()=>{await runtime.close();rmSync(root,{recursive:true,force:true});});return{root,project,registered,runtime,calls,state};
}
async function settle(runtime){for(let i=0;i<100&&runtime.hasActiveWork();i++)await new Promise(resolve=>setTimeout(resolve,5));assert.equal(runtime.hasActiveWork(),false);}
const request=f=>({projectId:f.registered.id,selection,brief:'Inspect the existing idea.'});
const approval=(f,inspection,value=inspection.proposal.value.context)=>({projectId:f.registered.id,runId:inspection.run.id,proposalRevision:inspection.proposal.revision,expectedContextRevision:inspection.run.projectContext.revision,value});

test('setup inspects the registered folder and only explicit approval writes the displayed context',async t=>{
 const f=await fixture(t);assert.deepEqual(f.runtime.projectSetup(f.registered.id).inspections,[]);assert.equal(f.calls.length,0);
 await f.runtime.inspectProject(request(f));await settle(f.runtime);
 const snapshot=f.runtime.projectSetup(f.registered.id);const inspection=snapshot.inspections[0];
 assert.equal(f.calls[0].workspace,f.project);assert.equal(f.calls[0].executionMode,'read-only');assert.equal(inspection.canApprove,true);
 assert.equal(existsSync(join(f.project,'config.project.yaml')),false);assert.equal(readFileSync(join(f.project,'README.md'),'utf8'),'Uncommitted project idea\n');
 f.runtime.projectSetup(f.registered.id);assert.equal(f.calls.length,1);
 await assert.rejects(f.runtime.setExecutionMode({conversationId:inspection.conversationId,executionMode:'code'}),/setup is read-only/);
 ...
 assert.deepEqual(saved.value,value);assert.equal(f.runtime.projectSetup(f.registered.id).inspections[0].canApprove,false);
 assert.ok(f.runtime.store.events(inspection.run.id).some(event=>event.type==='project-context.approved'));
...
test('new inspection output and external edits invalidate an older approval',async t=>{
 const f=await fixture(t);await f.runtime.inspectProject(request(f));await settle(f.runtime);const old=f.runtime.projectSetup(f.registered.id).inspections[0];
 f.state.response=JSON.stringify({...proposal,context:{...proposal.context,purpose:'Revised purpose'}});
 await f.runtime.inspectProject({...request(f),brief:'Revise the purpose.'});await settle(f.runtime);
 assert.throws(()=>f.runtime.approveProjectSetup(approval(f,old)),/changed/);
 const latest=f.runtime.projectSetup(f.registered.id).inspections.at(-1);assert.equal(latest.canApprove,true);
...
test('malformed and stopped inspections stay unapprovable and reopening does not dispatch',async t=>{
 const f=await fixture(t);f.state.response='Not a JSON proposal';await f.runtime.inspectProject(request(f));await settle(f.runtime);
 const bad=f.runtime.projectSetup(f.registered.id).inspections[0];assert.equal(bad.canApprove,false);assert.match(bad.error,/JSON proposal/);
 f.state.wait=true;const running=await f.runtime.inspectProject(request(f));
 ...
 await f.runtime.stop(running.id);await settle(f.runtime);
 assert.equal(f.runtime.projectSetup(f.registered.id).inspections.at(-1).run.status,'interrupted');
 assert.equal(f.calls.length,2);assert.equal(existsSync(join(f.project,'config.project.yaml')),false);
});
