...
const draft = (overrides = {}) => ({
  ...
  text: 'Run the existing checks before delivery.',
  tags: ['testing'],
  evidence: [{ label: 'Review result', uri: 'randolph://runs/source-run' }],
  ...overrides,
});
...
function fixture(t) {
  ...
  t.after(() => db.close());
  return { db, lessons: new Lessons(db) };
}
...
test('project and global approval settings apply independently without retroactive approval', (t) => {
  ...
  const global = lessons.create(draft({ scope: { kind: 'global' } }), {
    ...manual,
...
test('edits preserve evidence and exact previous versions while revoking approval until reviewed', (t) => {
  const { lessons } = fixture(t);
  const original = lessons.create(draft(), manual);
  lessons.approve([ref(original)]);
  const edited = lessons.edit(
    ref(original),
    { title: 'Revised', text: 'Run tests and inspect the diff.' },
    manual,
  );
  assert.equal(edited.version, 2);
  assert.equal(edited.status, 'draft');
  const history = lessons.history(original.lessonId);
  assert.equal(history[0].text, original.text);
  ...
  assert.throws(
    () => lessons.edit(ref(original), { text: 'stale overwrite' }, manual),
    /stale|current/i,
  );
});
...
test('group decisions are atomic and rejection or supersession can be reversed with preserved history', (t) => {
  const { lessons } = fixture(t);
  const first = lessons.create(draft(), manual);
  const second = lessons.create(draft({ title: 'Second' }), manual);
  assert.throws(
    () => lessons.approve([ref(first), { lessonId: second.lessonId, version: 99 }]),
    /stale|version/i,
  );
  ...
  assert.deepEqual(
    lessons.events(first.lessonId).map((event) => event.action),
    ['created', 'rejected', 'approved', 'superseded', 'restored'],
  );
});
...
test('retrieval filters scope, text, tags and exact framework/version applicability', (t) => {
  ...
  const applicable = lessons.create(
    draft({
      scope: { kind: 'global' },
      applicability: [{ framework: 'Next.js', versions: ['16'] }],
    }),
    settings,
  );
  lessons.create(draft({ scope: { kind: 'project', projectId: 'project-b' } }), settings);
  lessons.create(
    draft({ title: 'Different topic', text: 'Water the trees.', tags: ['gardening'] }),
    settings,
  );
  assert.deepEqual(
    lessons
  ...
  assert.equal(
    lessons.retrieve({ projectId: 'project-a', query: 'checks', frameworks: { 'Next.js': '15' } })
      .lessons.length,
    0,
  );
...
test('pinned and explicitly selected versions are never silently dropped by result limits', (t) => {
  const { lessons } = fixture(t);
  const settings = { ...manual, projectAutoApprove: true };
  const first = lessons.create(draft(), settings);
  const second = lessons.create(draft({ title: 'Other' }), settings);
  lessons.pin('project-a', ref(first), true);
  const result = lessons.retrieve({
    projectId: 'project-a',
    query: 'unmatched',
    limit: 1,
    selected: [ref(second)],
  });
  assert.deepEqual(result.lessons.map(ref), [ref(first), ref(second)]);
  ...
  assert.deepEqual(
    blocked.blockedPins.map((value) => value.reference),
    [ref(first)],
  );
...
test('pins and explicit selections cannot bypass approval, project boundaries or applicability', (t) => {
  ...
  const framework = lessons.create(
    draft({ applicability: [{ framework: 'React', versions: ['19'] }] }),
    { ...manual, projectAutoApprove: true },
  );
  lessons.pin('project-a', ref(framework), true);
  const result = lessons.retrieve({
    projectId: 'project-a',
    selected: [ref(pending), { lessonId: 'unknown', version: 1 }],
  });
  assert.equal(result.blockedPins.length, 1);
  assert.equal(result.blockedSelections.length, 1);
});
...
test('supplied versions and approval provenance stay immutable when later lessons change', (t) => {
  ...
  const use = lessons.recordUse({
    projectId: 'project-a',
    runId: 'run-1',
    agentId: 'agent-1',
    references: [ref(initial)],
  });
  ...
  assert.equal(use.lessons[0].text, initial.text);
  assert.equal(use.lessons[0].status, 'approved');
  assert.throws(
    () =>
      reopened.recordUse({
        projectId: 'project-a',
