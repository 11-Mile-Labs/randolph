...
async function fixture(t, { cleanupVerified = true } = {}) {
  ...
  writeFileSync(
    ...
    JSON.stringify({ packageManager: 'pnpm@11', scripts: { test: 'fixture check' } }),
  );
  git(root, ['add', '.']);
  ...
  const adapter = {
    ...
    async run(input) {
      ...
      input.onEvent({
        ...
        data: {
          threadId: 'delivery-recovery-thread',
          turnId: `delivery-recovery-turn-${calls.runs}`,
        },
      });
    ...
    async runCommand() {
      calls.checks++;
      return { exitCode: 0, output: 'fixture passed', truncated: false, cleanupVerified };
    },
  };
  ...
  const run = await runtime.send({
    ...
    text: 'Apply the fixture edit.',
  });
  const deadline = Date.now() + 2_000;
  while (runtime.hasActiveWork() && Date.now() < deadline)
  ...
  return {
    ...
    async reopen() {
      await runtime.close();
      runtime = new Runtime(adapter, dataRoot);
      return runtime;
    },
  };
}
...
for (const boundary of ['delivery.approved', 'delivery.committed', 'delivery.merged']) {
  test(
    `explicit recovery after ${boundary} retains the approved commit exactly once`,
    { timeout: 60_000 },
    async (t) => {
      ...
      await assert.rejects(
        f.runtime.approveReview({ reviewId: f.review.id, message: 'Approved original message' }),
      ...
      const recovered = await f.runtime.approveReview({
        reviewId: f.review.id,
        message: 'Continuation cannot replace the approved message',
      });
...
test(
  'export failure cannot clear a verification cleanup quarantine',
  { timeout: 60_000 },
  async (t) => {
    ...
    assert.equal(wasInjected(), true);
    assert.equal(
      f.runtime.snapshot().reviews.find((review) => review.id === f.review.id).status,
      'stop-unconfirmed',
    );
    await assert.rejects(
    ...
    await f.reopen();
    await assert.rejects(
      f.runtime.send({ conversationId: f.conversation.id, text: 'Still blocked' }),
      /active|cleanup/i,
    );
    assert.deepEqual(f.calls, { runs: 1, checks: 1 });
  },
...
test(
  'schema v3 adds delegation records while preserving existing v1 history and sequence',
  { timeout: 60_000 },
  (t) => {
  ...
  `);
    ...
    database
      ...
      .run(conversation.id, project.id, JSON.stringify(conversation));
    database
      .prepare('INSERT INTO runs VALUES (?, ?, ?)')
      .run(run.id, conversation.id, JSON.stringify(run));
    database
    ...
    try {
      ...
      assert.ok(
        store.db.prepare("SELECT 1 FROM sqlite_master WHERE name='delegation_controls'").get(),
      );
...
test(
  'a fresh review permanently invalidates an unmerged approval while retaining its intent',
  { timeout: 60_000 },
  async (t) => {
    const f = await fixture(t);
    await f.runtime.verifyReview(f.review.id);
    ...
    await assert.rejects(
      f.runtime.approveReview({ reviewId: f.review.id, message: 'Earlier approved message' }),
      /Injected export failure/,
    );
    assert.equal(wasInjected(), true);
    const savedPlan = f.runtime
      .snapshot()
      .reviews.find((review) => review.id === f.review.id).deliveryPlan;
    ...
    await assert.rejects(
      f.runtime.approveReview({ reviewId: old.id, message: 'Cannot reuse' }),
      /fresh review/i,
    );
    await f.reopen();
    await assert.rejects(
      f.runtime.approveReview({ reviewId: old.id, message: 'Still cannot reuse' }),
      /fresh review/i,
    );
    assert.equal(git(f.root, ['rev-parse', 'HEAD']), f.seed);
...
test(
  'a merged delivery awaiting cleanup cannot be discarded by preparing a new review',
  { timeout: 60_000 },
  async (t) => {
    const f = await fixture(t);
    await f.runtime.verifyReview(f.review.id);
    ...
    await assert.rejects(
      f.runtime.approveReview({ reviewId: f.review.id, message: 'Merged approved message' }),
      /Injected export failure/,
    );
    assert.equal(wasInjected(), true);
