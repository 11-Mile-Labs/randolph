...
async function fixture(t, { cleanupVerified = true } = {}) {
  ...
  const adapter = {
    async discover() { return { available: true, authenticated: true, executionModes: ['read-only', 'code'], models: [{ id: 'fixture', name: 'Fixture', efforts: ['low'], defaultEffort: 'low' }] }; },
    async run(input) { calls.runs++; writeFileSync(join(input.workspace, 'value.txt'), 'after\n'); return { status: 'completed' }; },
