  ...
  const adapter = {
    ...
    run: async (input) => {
      ...
      input.onEvent({
        ...
        data: { threadId: 'linked-thread', turnId: `linked-turn-${calls.length}` },
      });
      ...
      if (calls.length === 1)
        input.onEvent({
          type: 'message.delta',
          summary: 'partial',
          data: { messageId: 'partial', text: 'partial output' },
        });
...
async function settle(runtime) {
  for (let count = 0; count < 500 && runtime.hasActiveWork(); count++)
...
test('explicit restart creates a linked run in the same conversation from its last safe starting checkpoint', async (t) => {
  ...
  runtime.memory.prepare = () => ({
    references: [],
    text: 'Saved memory context.',
    estimatedTokens: 5,
    frameworks: {},
  });
  ...
  const original = await runtime.send({
    conversationId: conversation.id,
    text: 'Do the interrupted work.',
    model: 'model-a',
    effort: 'low',
  });
  ...
  assert.deepEqual(
    source.checkpoints.map((checkpoint) => checkpoint.boundary),
    ['before-turn'],
  );
  await runtime.setConversationSelection({
    conversationId: conversation.id,
    selection: { harness: 'codex', model: 'model-b', effort: 'high' },
  });
  await runtime.close();
  ...
  const result = await runtime.restartRun({
    runId: source.id,
    checkpointDigest: source.checkpoints[0].digest,
  });
  ...
  await assert.rejects(
    runtime.restartRun({ runId: source.id, checkpointDigest: source.checkpoints[0].digest }),
    /newer run|latest run/i,
  );
  const restarted = runtime.snapshot().runs.find((run) => run.id === result.run.id);
  const restartedCheckpoint = restarted.checkpoints.findLast(
    (checkpoint) => checkpoint.boundary === 'completed-turn',
  );

  await runtime.send({
    conversationId: conversation.id,
    text: 'Continue from the clean recovery.',
    model: 'model-a',
    effort: 'low',
  });
  await settle(runtime);
  ...
  assert.equal(
    runtime.snapshot().messages.some((message) => message.text === 'partial output'),
    true,
  );

  const rerun = await runtime.rerunFromCheckpoint({
    runId: restarted.id,
    checkpointDigest: restartedCheckpoint.digest,
  });
  await settle(runtime);
  ...
  assert.deepEqual(rerun.run.recoveryMessages, [
    { role: 'user', text: 'Do the interrupted work.' },
  ]);
...
test('explicit rerun of an earlier result creates a linked conversation with saved context and fresh review state', async (t) => {
  ...
  const first = await runtime.send({
    conversationId: conversation.id,
    text: 'First retained request.',
    model: 'model-a',
    effort: 'low',
  });
  await settle(runtime);
  const firstRun = runtime.snapshot().runs.find((run) => run.id === first.id);
  const firstCheckpoint = firstRun.checkpoints.findLast(
    (checkpoint) => checkpoint.boundary === 'completed-turn',
  );
  await runtime.send({
    conversationId: conversation.id,
  ...
  const result = await runtime.rerunFromCheckpoint({
    runId: firstRun.id,
    checkpointDigest: firstCheckpoint.digest,
  });
  ...
  assert.deepEqual(result.run.recoveryMessages, [
    { role: 'user', text: 'First retained request.' },
  ]);
  assert.equal(
    runtime.snapshot().reviews.some((review) => review.conversationId === result.conversation.id),
    false,
  );
...
test('explicit rerun accepts a failed run before-turn checkpoint', async (t) => {
  const f = fixture(t, [
    new AdapterRunFailure('fixture failure', { processTermination: 'confirmed' }),
    'completed',
  ]);
  const runtime = new Runtime(f.adapter, f.dataRoot);
  t.after(() => runtime.close());
...
test('linked execution preserves a saved read-only mode and does not require code capability', async (t) => {
  ...
  await runtime.send({
    conversationId: conversation.id,
    text: 'Create a managed worktree.',
    model: 'model-a',
    effort: 'low',
  });
  await settle(runtime);
...
test('cleanup quarantine blocks rerun without dispatch', async (t) => {
  ...
  const checkpoint = source.checkpoints.findLast((item) => item.boundary === 'completed-turn');
  await assert.rejects(
    runtime.rerunFromCheckpoint({ runId: source.id, checkpointDigest: checkpoint.digest }),
    /cleanup|reconcil/i,
  );
  assert.equal(f.calls.length, 1);
});
...
test('a failed pre-dispatch restart remains in history and the original safe checkpoint can be retried', async (t) => {
  ...
  const original = await runtime.send({
    conversationId: conversation.id,
    text: 'Recover after a local restore failure.',
    model: 'model-a',
    effort: 'low',
  });
  await settle(runtime);
  ...
  runtime.checkpoints.restoreWorktree = () => {
  ...
  assert.equal(f.calls.length, 2);
  assert.equal(
    runtime.snapshot().runs.some((run) => run.id === failedAttempt.id && run.status === 'failed'),
    true,
  );
});
