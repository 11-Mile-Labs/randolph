...
function seed(root) {
  ...
  const controls = new DelegationControls(store, () => 1_000);
  controls.create(run.id, authorization.id);
  controls.begin(run.id, 1, 'prepare-source', 'source-preparation');
  assert.equal(records.sessions(run.id).length, 0);
  store.close();
  return run;
}

test('Runtime startup quarantines an unfinished runtime-only control activity without discovering or launching', async (t) => {
  const root = mkdtempSync(join(tmpdir(), 'randolph-delegation-control-recovery-')),
    run = seed(root),
    calls = { discover: 0, run: 0 };
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const adapter = {
    async discover() {
      calls.discover += 1;
      throw new Error('Startup must not discover.');
    },
    async run() {
      calls.run += 1;
  ...
  const first = new Runtime(adapter, root);
  let firstClosed = false;
  t.after(async () => {
    if (!firstClosed) await first.close();
  });
  const interrupted = first.snapshot().runs.find((item) => item.id === run.id),
    control = new DelegationControls(first.store).read(run.id);
  assert.equal(interrupted.status, 'interrupted');
  assert.equal(interrupted.cleanupUnconfirmed, true);
  assert.equal(control.recoveryRequired, true);
  assert.equal(control.activities[0].stage, 'source-preparation');
  assert.equal(control.activities[0].state, 'cleanup-unconfirmed');
  assert.deepEqual(calls, { discover: 0, run: 0 });
  await first.close();
  firstClosed = true;
  const second = new Runtime(adapter, root);
  t.after(() => second.close());
  const persisted = second.snapshot().runs.find((item) => item.id === run.id),
    recovered = new DelegationControls(second.store).read(run.id);
