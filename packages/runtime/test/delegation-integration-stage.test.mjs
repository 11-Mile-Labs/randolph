...
const assignment = (id, role, dependencies, source, extra = {}) => ({
  id,
  task: id,
  role,
  harness: 'codex',
  ...
  ...(role === 'main-synthesis' ? {} : { producesSource: true }),
  ...extra,
});

function fixture(t) {
  const root = realpathSync(mkdtempSync(join(tmpdir(), 'randolph-integration-stage-'))),
    projectRoot = join(root, 'project'),
    data = join(root, 'data');
  mkdirSync(projectRoot);
  git(projectRoot, 'init', '-b', 'main');
  writeFileSync(join(projectRoot, 'file.txt'), 'base\n');
  git(projectRoot, 'add', '.');
  ...
  const records = new DelegationRecords(store),
    plan = records.recordPlan({
      ...
      plan: {
        ...
        id: 'plan',
        revision: 1,
        limits: { maxWorkers: 1, maxParallel: 1, maxAttempts: 1, activeMinutes: 5 },
        assignments: [
          assignment('worker', 'worker', [], 'run-basis'),
          assignment('integrate', 'main-integration', ['worker'], 'run-basis', {
  ...
  new DelegationControls(store).create(run.id, auth.id);
  const worker = records.tasks(run.id).find((item) => item.assignmentId === 'worker');
  const workerAttempt = {
    id: randomUUID(),
    generation: 1,
    status: 'completed',
  ...
  mkdirSync(outputs);
  restoreCheckpoint(checkpoint.directory, checkpoint.digest, writer);
  writeFileSync(join(writer, 'file.txt'), 'worker\n');
  const output = createCheckpoint(writer, outputs, {
    runId: run.id,
  ...
  store.db
    ...
    .run(JSON.stringify({ ...integration, state: 'running', attempts: [attempt] }), integration.id);
  const leases = new WorkspaceLeases();
  const acquired = leases.acquire({ reservationId: attempt.id, runId: run.id, workspace });
  if (acquired.status !== 'acquired') throw new Error('fixture lease failed');
  t.after(() => {
    store.close();
    rmSync(root, { recursive: true, force: true });
  });
  return { store, records, run, integration, attempt, leases };
}

test('retains a derived candidate, applies once, and only exposes it after the conversation workspace changed', (t) => {
  const f = fixture(t),
    stage = new DelegationIntegrationStage(f.store, f.leases),
    input = {
      runId: f.run.id,
      taskId: f.integration.id,
      attemptId: f.attempt.id,
      expectedGeneration: 1,
      lease: { reservationId: f.attempt.id, generation: 1 },
    };
