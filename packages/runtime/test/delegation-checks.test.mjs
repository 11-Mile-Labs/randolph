...
async function checksFixture(t) {
  const f = await fixture(t, [
    assignment('writer', 'worker', [], 'run-basis', true),
    {
      ...assignment('integrate', 'main-integration', ['writer'], 'output:writer', true),
      integrationInputs: ['writer'],
      repairAttempts: 0,
    },
    assignment('verify', 'runtime-verification', ['integrate'], 'output:integrate', true),
  ...
  for (const prerequisite of f.taskRecords.slice(0, 2)) {
    prerequisite.state = 'completed';
    prerequisite.attempts = [
      {
        id: prerequisite.id + '-done',
        status: 'completed',
        cleanupConfirmed: true,
        result: { success: true, source: source(prerequisite.id) },
      },
    ];
    f.store.db
      .prepare('UPDATE delegation_tasks SET document=? WHERE id=?')
      .run(JSON.stringify(prerequisite), prerequisite.id);
  }
  const task = f.taskRecords[2],
    input = {
      runId: 'run',
      taskId: task.id,
      attemptId: 'attempt',
  ...
  f.tasks.beginAttempt({
    ...
    session: session('primary', 'verification'),
  });
  f.tasks.bindPreparedAttempt({
    ...input,
    source: source(f.taskRecords[1].id),
    contextArtifacts: [],
    workspace,
  });
...
function complete(f, claim, options = {}) {
  const input = {
    ...f.input,
    expectedGeneration: f.controls.read('run').generation,
    checkId: claim.check.id,
  ...
  f.records.finishSession({
    ...
    status: 'completed',
    cleanupConfirmed: true,
    cleanupEvidence: { group: 'gone' },
    ...options.session,
  });
  return f.checks.finishCheck({
    ...input,
    status: 'completed',
    exitCode: 0,
    observedTreeOid: source().treeOid,
    ...options.check,
  });
}
...
test('checks freeze one manifest, reject duplicate/out-of-order claims, and require every clean command before publication', async (t) => {
  ...
  assert.equal(second.session.admissionClaim, undefined);
  complete(f, second);
  assert.equal(f.checks.passed(f.input), true);
  assert.deepEqual(
    f.records.sessions('run').map((item) => item.native),
    [{ commandId: 'command-check-1' }, { commandId: 'command-check-2' }],
  );
...
test('Pause settles a current check, blocks the next, and Resume preserves passed check identities', async (t) => {
  const f = await checksFixture(t),
    first = f.checks.claimNext(f.input);
  ...
  const second = f.checks.claimNext({
    ...f.input,
...
test('an undispatched check can defer only with retained cleanup, then gets fresh cleanup and cannot forge a model turn', async (t) => {
  ...
  assert.throws(
    () => f.records.bindSession({ ...input, threadId: 'fake', turnId: 'fake' }),
...
test('source-changing and unknown-cleanup checks cannot pass or release later checks; output caps preserve UTF8', async (t) => {
  const f = await checksFixture(t),
    first = f.checks.claimNext(f.input);
  const result = complete(f, first, {
    check: { observedTreeOid: 'c'.repeat(40), output: '𐐀'.repeat(10_000) },
  });
  ...
  const g = await checksFixture(t),
    unknown = g.checks.claimNext(g.input);
  ...
  const failure = g.checks.finishCheck({
    ...g.input,
...
test('reopen quarantines unfinished checks without replay and Stop closes claims', async (t) => {
  const f = await checksFixture(t);
  f.checks.claimNext(f.input);
  f.records.reconcileUnfinishedSessions();
  f.tasks.reconcileOnReopen();
  f.checks.reconcileOnReopen();
