...
const value = {
  context: {
    purpose: 'A project workspace',
    instructions: 'Keep history.\nAsk before delivery.',
    documents: [{ path: 'docs/product-spec.md', description: 'Approved behavior' }],
  },
  evidence: ['README describes the workspace.'],
  questions: ['Which target platform comes next?'],
};

test('completed proposal parsing retains editable context and a revision tied to the exact response', () => {
  const raw = JSON.stringify(value);
  const parsed = parseSetupProposal(raw);
...
test('malformed or unsafe proposals cannot become approvable context', () => {
  for (const raw of [
    'not json',
    'Here is a proposal: ' + JSON.stringify(value),
    JSON.stringify({
      ...value,
      context: { ...value.context, documents: [{ path: '../outside', description: '' }] },
    }),
    JSON.stringify({ ...value, questions: 'not a list' }),
    JSON.stringify({ ...value, evidence: Array(31).fill('x') }),
    'x'.repeat(100_001),
  ])
    assert.throws(() => parseSetupProposal(raw));
});

test('setup prompts retain approved context and corrections without giving the model write authority', () => {
  const prompt = setupPrompt(
    { revision: 'a'.repeat(64), value: value.context },
