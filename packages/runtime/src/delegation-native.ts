...
export type DelegationNativeResult = {
...
export class DelegationNative {
  ...
  async run(request: DelegationNativeInput): Promise<DelegationNativeResult> {
    const input: DelegationNativeInput = {
      runId: request.runId,
    ...
    const task = this.tasks.assertAdmission(input),
    ...
    if (
      ...
      throw new Error(
        'Native execution requires the exact prepared attempt session and workspace.',
      );
    if (assignment.role === 'runtime-verification')
      throw new Error('Runtime verification requires its command executor, not a model turn.');
    if (
      ['main-integration', 'main-synthesis'].includes(assignment.role) &&
      (assignment.harness !== run.harness ||
        assignment.executable !== run.executable ||
        assignment.executableVersion !== run.executableVersion ||
        assignment.model !== run.model ||
        assignment.effort !== run.effort)
    )
      throw new Error('A main-agent phase must preserve the selected main identity.');
    if (
      ...
      throw new Error('This exact native route was not enabled for the run.');
    if (
      !input.messages.length ||
      input.messages.length > 200 ||
      Buffer.byteLength(JSON.stringify(input.messages)) > 256 * 1024 ||
      input.messages.some(
        (message) =>
          !['user', 'assistant'].includes(message.role) || typeof message.text !== 'string',
      )
    )
      throw new Error('Native task context must be a bounded application-owned message list.');
    const claim = randomUUID();
    this.store.transaction(() => {
      const current = this.records.sessions(input.runId).find((value) => value.id === session.id);
      if (!current || current.state !== 'prepared' || current.admissionClaim)
        throw new Error('This native session is already claimed or no longer prepared.');
      current.admissionClaim = claim;
      this.store.db
        .prepare('UPDATE delegation_sessions SET document=? WHERE id=?')
        .run(JSON.stringify(current), current.id);
      this.store.append(
        run,
        'delegation.native-admission-claimed',
        'Exact native session admission claimed before asynchronous work.',
        { taskId: task.id, attemptId: attempt.id, sessionId: session.id, claim },
      );
    });
    const assertClaim = (): void => {
      if (
        this.records.sessions(input.runId).find((value) => value.id === session.id)
          ?.admissionClaim !== claim
      )
        throw new Error('Native admission claim is no longer owned by this executor.');
    };
    ...
    const audit = (type: string, text: string, data: Record<string, unknown> = {}): void => {
      this.store.append(run, type, text, {
        taskId: task.id,
    ...
    const guard = (): void => {
      ...
      const ownership = this.workspaceLeases
        .snapshot()
        .find((value) => value.reservationId === input.workspaceLease.reservationId);
      if (
        !ownership ||
        ownership.reservationId !== attempt.id ||
        ownership.generation !== input.workspaceLease.generation ||
        ownership.runId !== run.id ||
        ownership.state !== 'active' ||
        ownership.workspace !== workspace.path ||
        ownership.identity?.device !== workspace.identity.device ||
        ownership.identity.inode !== workspace.identity.inode
      )
        throw new Error('Native task workspace ownership is absent, stale, or quarantined.');
    ...
    const timer = setInterval(() => {
      try {
        ...
        if (
          ...
          this.records.plans(input.runId).at(-1)?.id !== authorization.planId
        )
          controller.abort();
      } catch {
        controller.abort();
    ...
    try {
      ...
      const context = (phase: 'discovery' | 'model-turn') => ({
        owner: { kind: 'delegation' as const, id: attempt.id },
        runId: input.runId,
        sessionId: session.id,
        generation: input.expectedGeneration,
        role: assignment.role,
        ...(assignment.role === 'worker' || assignment.role === 'review'
          ? {
              authorizationId: task.authorizationId,
              workerParallelLimit: plan.plan.limits.maxParallel,
            }
          : {}),
        priority: () => this.controls.read(input.runId)?.priority ?? 0,
        assertCurrent: guard,
        queued: (reason: CapacityQueueReason) =>
          audit('delegation.native-queued', 'Delegation task is waiting for native capacity.', {
            reason,
            phase,
          }),
        onAdmitted: () => {
          ...
          activity = this.controls.begin(
            ...
            `${attempt.id}:${stage}:${randomUUID()}`,
            stage,
          );
          audit(
            phase === 'discovery' ? 'delegation.discovery-started' : 'delegation.native-started',
            phase === 'discovery'
              ? 'Checking the exact native route before task dispatch.'
              : 'Native task dispatch has durable admission.',
            {
          ...
          if (phase === 'model-turn') {
            if (
              !inputTree ||
              captureGitTree(workspace.path, this.store.runDirectory(run)) !== inputTree
            )
              throw new Error('Prepared task source changed while native admission was pending.');
      ...
      try {
        info = await this.admission.perform(
          assignment.harness,
          context('discovery'),
          'discovery',
          assignment.executable,
          controller.signal,
          (signal) => adapter.discover(assignment.executable, signal),
          (value) => {
        ...
        cleanupEvidence =
          cause instanceof AdapterRunFailure
            ? cause.cleanupEvidence
            : { reason: 'discovery-failure-without-cleanup-evidence' };
        throw cause;
      } finally {
        finishActivity();
      }
      audit(
        'delegation.discovery-finished',
        'Native route discovery settled without a model turn.',
        { cleanupConfirmed },
      );
      ...
      if (
        ...
        !info.executionModes?.includes(assignment.mode)
      )
        throw new Error(
          'The authorized native route, model, effort, or execution capability is unavailable.',
        );
      activity = this.controls.begin(
        input.runId,
        input.expectedGeneration,
        `${attempt.id}:source-validation:${randomUUID()}`,
        'source-preparation',
      );
      try {
        inputTree =
          assignment.role === 'main-integration'
            ? new DelegationIntegrationStage(this.store, this.workspaceLeases).candidate(input)
                .treeOid
            : attempt.source.treeOid;
        if (captureGitTree(workspace.path, this.store.runDirectory(run)) !== inputTree)
          throw new Error('Prepared task source changed while native admission was pending.');
        this.controls.tick(input.runId);
        guard();
      } finally {
      ...
      const onEvent = (event: AdapterEvent): void => {
        ...
        if (event.type === 'session.turn-started') {
          ...
          if (typeof threadId !== 'string' || typeof turnId !== 'string')
            throw new Error('Native task did not supply its thread and turn identity.');
        ...
        if (event.type !== 'message.delta')
          audit('delegation.native-activity', event.summary.slice(0, 1000), {
            nativeEventType: event.type.slice(0, 160),
          });
      };
      let result: Awaited<ReturnType<HarnessAdapter['run']>>;
      try {
        result = await this.admission.perform(
          assignment.harness,
          context('model-turn'),
          'model-turn',
          assignment.executable,
          ...
          async (signal) => {
            ...
            guard();
            this.tasks.dispatchAttempt(input);
            turnDispatched = true;
          ...
          (value) => {
            cleanupConfirmed = value.status !== 'stop-unconfirmed';
            cleanupEvidence = { adapterStatus: value.status };
            return {
              status: value.status === 'stop-unconfirmed' ? 'failed' : value.status,
              confirmed: cleanupConfirmed,
              evidence: cleanupEvidence,
            };
          },
        ...
        cleanupEvidence =
          cause instanceof AdapterRunFailure
            ? cause.cleanupEvidence
            : { reason: 'model-failure-without-cleanup-evidence' };
        throw cause;
      } finally {
        finishActivity();
      }
      const bound = this.records
        .sessions(input.runId)
        .find((item) => item.id === session.id)?.native;
      status =
        result.status === 'completed' && bound?.threadId && bound.turnId
          ? 'completed'
      ...
      if (status === 'failed')
        error = cleanupConfirmed
          ? 'Native task completed without its registered thread and turn identity.'
          : 'Native task cleanup could not be confirmed.';
    } catch (cause) {
      error = (cause instanceof Error ? cause.message : 'Native task execution failed.').slice(
        0,
        1000,
      );
      ...
      if (
        ...
        authorization?.planId === this.records.plans(input.runId).at(-1)?.id &&
        control &&
        !control.recoveryRequired &&
        control.desired !== 'stopped' &&
      ...
      if (status !== 'pending')
        ...
        this.store.transaction(() => {
          ...
          delete current.admissionClaim;
          this.store.db
            .prepare('UPDATE delegation_sessions SET document=? WHERE id=?')
            .run(JSON.stringify(current), current.id);
          audit(
            'delegation.native-admission-pending',
            'Native cleanup settled; prepared task awaits explicit current admission.',
          );
        });
    }
