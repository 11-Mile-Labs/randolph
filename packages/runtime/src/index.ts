...
export class Runtime {
  ...
  readonly active = new Map<string, { controller: AbortController; done: Promise<void>; run: Run }>();
    ...
    this.delegation = new DelegationCommands(this.store, {
      ...
      availability: async (run, plan) => {
        ...
        const results = await Promise.allSettled(unique.map(async assignment => {
          ...
          if (!found) return;
          const info = await this.nativeAdmission.adapter(assignment.harness, found, { owner: { kind: 'delegation', id: run.id }, runId: run.id }).discover(assignment.executable);
          if (!info.available || !info.authenticated || info.executable !== assignment.executable || !info.version) return;
          available.push({ harness: assignment.harness, executable: info.executable, version: info.version, models: info.models.map(model => ({ id: model.id, efforts: model.efforts })), modes: info.executionModes ?? ['read-only'], enabled: true, commandCapability: Boolean(found.runCommand && info.commandLifecycle === true && info.executionModes?.includes('code')) });
        }));
        ...
        return { routes: available, mainSelection: { harness: run.harness ?? 'codex', executable: run.executable, executableVersion: run.executableVersion, model: run.model, effort: run.effort } };
      },
    }, () => this.changed());
  ...
  project(id: string): Project { return requireProject(this, id); }
  conversation(id: string): Conversation { return requireConversation(this, id); }
  planWorkspace(root: string, provenance: WorkspaceProvenance, plan: () => string, held: Map<string, WorkspaceLease>, readOnlyPlanning = false) { return this.workspaces.plan(root, provenance, plan, held, readOnlyPlanning); }
  inspectExecutable(harness: HarnessId, executable?: string | null): Promise<HarnessInfo> { return this.routes.inspectExecutable(harness, executable); }
  validateSelection(selection: Parameters<RuntimeHarness['validateSelection']>[0], info: HarnessInfo): void { this.routes.validateSelection(selection, info); }
  validateExecutionMode(mode: NonNullable<Run['executionMode']>, info: HarnessInfo): void { this.routes.validateExecutionMode(mode, info); }
  ...
  snapshot(): WorkspaceSnapshot { return workspaceSnapshot(this); }
  chatEvents(input: ChatEventsInput): ChatEventsResult { return chatEvents(this, input); }
  ...
  rerunFromCheckpoint(input: RerunCheckpointInput): Promise<LinkedRunResult> { return recoverLinkedCheckpoint(recoveryHost(this), 'rerun', input); }
  ...
  projectSetup(projectId: string): ProjectSetupSnapshot { return projectSetupSnapshot(setupHost(this), projectId); }
  reconcileProjectSetupCleanup(projectId: string): ProjectSetupSnapshot { return reconcileProjectSetupCleanup(setupHost(this), projectId); }
  async inspectProject(input: InspectProjectInput): Promise<Run> { return inspectProject(setupHost(this), input); }
  approveProjectSetup(input: ApproveProjectSetupInput): ProjectContextSnapshot { return approveProjectSetup(setupHost(this), input); }
  ...
  addProject(path: string): Project { return addProject(this, path); }
  createConversation(projectId: string): Conversation { return createConversation(this, projectId); }
  markRead(conversationId: string): void { markRead(this, conversationId); }
  async send(input: SendInput): Promise<Run> { return this.prepareSend(input); }
  prepareSend(input: SendInput, setup?: { executable?: string; expectedContextRevision: string | null }): Promise<Run> { return dispatchOrdinaryRun(dispatchHost(this), input, setup); }
  async execute(run: Run, controller: AbortController, lease: WorkspaceLease): Promise<void> { return executeNativeTurn(this, run, controller, lease); }
  event(run: Run, event: AdapterEvent): void { recordAdapterEvent(this, run, event); }
  finish(run: Run, status: Run['status'], error?: string): void { finishRun(this, run, status, error); }
  async stop(runId: string): Promise<void> { return stopRun(this, runId); }
  hasActiveWork(options: { includeDiscovery?: boolean } = {}): boolean { return hasActiveWork(this, options); }
  async stopAll(): Promise<void> { return stopAllWork(this); }
  async close(): Promise<void> { return closeRuntime(this); }
}
