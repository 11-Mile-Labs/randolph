...
export class Runtime {
  ...
  constructor(readonly adapter: HarnessAdapter, dataRoot: string) {
    ...
    this.reviews = new Reviews(this.store, adapter, id => !this.admission.has(id) && !this.store.runs().some(run => run.conversationId === id && (activeStatuses.has(run.status) || run.cleanupUnconfirmed)), () => this.changed());
  }
  ...
  prepareReview(conversationId: string): ReviewRecord { return this.reviews.prepare(conversationId); }
  ...
  async send(input: SendInput): Promise<Run> {
    ...
    if (this.admission.has(conversation.id) || this.reviews.hasActiveWork(conversation.id) || this.store.runs().some(run => run.conversationId === conversation.id && (activeStatuses.has(run.status) || run.cleanupUnconfirmed))) throw new Error('This conversation already has active work.');
    ...
    try {
      ...
      const run: Run = { id: randomUUID(), projectId: project.id, conversationId: conversation.id, status: 'starting', model: selection.model, effort: selection.effort, executionMode, settingsSource, projectSettingsRevision: settings.revision, workspace, createdAt: now(), updatedAt: now(), lastActivityAt: now() };
  ...
  hasActiveWork(): boolean { return this.active.size > 0 || this.admission.size > 0 || this.reviews.hasActiveWork(); }
