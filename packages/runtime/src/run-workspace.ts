...
export class RunWorkspace {
  ...
  own(workspace: string, provenance: WorkspaceProvenance, held: Map<string, WorkspaceLease>, access: 'read' | 'write' = 'write'): WorkspaceLease {
    const acquired = this.ownership.acquire({ reservationId: randomUUID(), ownerId: provenance.id, workspace, provenance, phase: 'preparation', access });
  ...
  plan(root: string, provenance: WorkspaceProvenance, plan: () => string, held: Map<string, WorkspaceLease>, readOnlyPlanning = false): PlannedWorkspace {
    ...
    if (readOnlyPlanning) {
      const promoted = this.store.transaction(() => {
        this.ownership.release({ ...parent, cleanupConfirmed: true, cleanupEvidence: { operation: 'workspace-planning', outcome: 'read-only-probe-returned' } });
        const acquired = this.ownership.acquire({ reservationId: randomUUID(), ownerId: provenance.id, workspace: root, provenance, phase: 'workspace-preparation', access: 'write' });
      ...
      workspace = plan(); this.ownership.assert(parent);
      if (workspace === root) return { parent, lease: parent, workspace };
    }
  ...
  release(held: Map<string, WorkspaceLease>, failure?: unknown): void {
    ...
    this.store.transaction(() => {
      for (const lease of held.values()) this.ownership.release({ reservationId: lease.reservationId, generation: lease.generation, cleanupConfirmed, cleanupEvidence: { operation: lease.provenance?.kind ?? 'runtime', phase: lease.phase ?? 'preparation', runtimeCleanupConfirmed: cleanupConfirmed, outcome: failure instanceof Error ? failure.message : failure ? 'failed' : 'returned' } });
    });
    held.clear();
  }
  ...
  observe(run: Run, work: Promise<void>): Promise<void> {
    return (async () => {
      try { await work; }
        ...
        try { this.settle(run, 'stop-unconfirmed', `Workspace settlement failed: ${error instanceof Error ? error.message : 'unknown failure'}`); } catch { /* The retained ownership row continues to block admission if SQLite is unavailable. */ }
      }
    })();
  }

  transfer(plan: Pick<PlannedWorkspace, 'parent' | 'lease'>, held: Map<string, WorkspaceLease>): WorkspaceLease {
