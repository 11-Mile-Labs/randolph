...
export async function stopRun(
  host: Pick<RuntimeBindings, 'active' | 'store' | 'changed'>,
  runId: string,
): Promise<void> {
  ...
  host.store.transaction(() => {
    ...
    host.store.append(
      run,
      'run.stopping',
      `Stopping ${run.harness ?? 'codex'}; awaiting confirmation`,
    );
  });
  host.changed();
...
export function hasActiveWork(
  host: Pick<RuntimeBindings, 'nativeAdmission' | 'active' | 'admission' | 'reviews' | 'pushes'>,
  options: { includeDiscovery?: boolean } = {},
): boolean {
  return (
    host.nativeAdmission.hasActiveWork(options.includeDiscovery !== false) ||
    host.active.size > 0 ||
    host.admission.size > 0 ||
    host.reviews.hasActiveWork() ||
    host.pushes.hasActiveWork()
  );
}
...
export async function stopAllWork(
  host: Pick<
    RuntimeBindings,
    'nativeAdmission' | 'pushes' | 'active' | 'store' | 'reviews' | 'changed'
  >,
): Promise<void> {
  ...
  await Promise.all(
    host.store
      .reviews()
      .filter((review) => review.status === 'checking')
      .map((review) => host.reviews.stop(review.id)),
  );
}
