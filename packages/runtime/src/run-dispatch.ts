...
export type DispatchHost = {
  ...
  planWorkspace(
    root: string,
    provenance: { kind: string; id: string; projectId?: string; conversationId?: string },
    plan: () => string,
...
export async function dispatchOrdinaryRun(
  host: DispatchHost,
  input: SendInput,
  setup?: { executable?: string; expectedContextRevision: string | null },
): Promise<Run> {
  assertOpen(host.isAccepting());
  if (typeof input.text !== 'string' || !input.text.trim() || input.text.length > 64_000)
    throw new Error('Enter a message of at most 64,000 characters.');
  let conversation = host.conversation(input.conversationId);
  if (conversation.kind === 'project-setup' && !setup)
    throw new Error('Use the dedicated project inspection action for setup conversations.');
  if (
    host.admission.has(conversation.id) ||
    host.reviews.hasActiveWork(conversation.id) ||
    host.pushes.hasActiveWork(conversation.id) ||
    conversationHasBlockingRun(host.store.runs(), conversation.id)
  )
    throw new Error('This conversation already has active work.');
  if (host.integrations.blocksNewWork(conversation.id))
    throw new Error('Finish interrupted parent integration before starting new work.');
  ...
  try {
    ...
    const explicit =
      input.harness !== undefined || input.model !== undefined || input.effort !== undefined;
    const defaultHarness = settings.defaults?.harness ?? 'codex';
    const selectedHarness =
      input.harness ?? savedSelection?.harness ?? settings.defaults?.harness ?? 'codex';
    const sameHarnessSelection =
      savedSelection?.harness === selectedHarness
        ? savedSelection
        : settings.defaults?.harness === selectedHarness
          ? settings.defaults
          : undefined;
    const selection: HarnessSelection = explicit
      ? {
          harness: selectedHarness,
          model: input.model ?? sameHarnessSelection?.model ?? '',
    ...
    if (setup && projectContext.revision !== setup.expectedContextRevision)
      throw new Error(
        'Project context changed during setup discovery. Inspect again with the current context.',
      );
    const memory = host.memory.prepare(project.id, input.text);
    const settingsSource =
      explicit || savedSelection ? 'conversation' : settings.defaults ? 'project' : 'native';
    const executionMode = conversation.executionMode ?? 'read-only';
    if (conversation.kind === 'project-setup' && executionMode !== 'read-only')
    ...
    const recoveryMessages = previousRun?.recoveryMessages
      ? [
          ...
          ...host.store
            ...
            .filter(
              (message) =>
                message.runId === previousRun.id &&
                !message.id.startsWith(`${previousRun.id}:recovery:`),
            )
            .map(({ role, text }) => ({ role, text })),
          { role: 'user' as const, text: input.text },
        ]
      : undefined;
    const cleaned = host.store
      .reviews()
      .some((review) => review.runId === previousRun?.id && review.cleaned);
    const previous =
      cleaned || (executionMode === 'code' && previousRun?.workspace === project.root)
        ? undefined
        : previousRun?.workspace;
    const runId = randomUUID();
    const plan = () =>
      conversation.kind === 'project-setup'
        ? project.root
        : plannedConversationWorkspace(project.root, conversation.id, previous);
    const planned = host.planWorkspace(
      project.root,
      { kind: 'run', id: runId, projectId: project.id, conversationId: conversation.id },
      plan,
      held,
      executionMode === 'read-only',
    );
    const workspace =
      planned.lease.access === 'read' || conversation.kind === 'project-setup'
        ? project.root
        : prepareWorkspace(project.root, conversation.id, previous);
    if (workspace !== planned.workspace)
      throw new Error('Conversation workspace changed during preparation.');
    ...
    const run: Run = {
      ...
      enabledHarnessRoutes:
        ...
        (info.executable ? [{ harness: selection.harness, executable: info.executable }] : []),
    ...
    host.store.transaction(() => {
      host.store.putConversation({
        ...conversation,
        title:
          conversation.title === 'New conversation'
            ? input.text.trim().slice(0, 64)
            : conversation.title,
        ...(explicit
          ? { harness: selection.harness, model: selection.model, effort: selection.effort }
          : {}),
        updatedAt: now(),
      });
      ...
      host.store.append(
        run,
        'run.created',
        executionMode === 'code' ? 'Code conversation queued' : 'Read-only conversation queued',
        {
          harness: run.harness,
          model: run.model,
          effort: run.effort,
          settingsSource,
          projectSettingsRevision: settings.revision,
          nativeVersion: info.version ?? 'unknown',
    ...
    createdRun = run;
    try {
      host.memory.retain(project.id, run.id, memory);
      host.store.exportRun(run);
    } catch {
      host.finish(run, 'failed', 'Could not export run logs. No harness was launched.');
      throw new Error('Could not export run logs.');
    }
    if (workspace !== project.root) {
      try {
        ...
        host.finish(
          run,
          'failed',
          'Could not retain the starting checkpoint. No harness was launched.',
        );
        throw cause;
      }
    } else {
      host.checkpoints.failed(
        run,
        new Error(
          'File recovery is currently supported only for Git worktrees with a committed HEAD. This read-only folder run has retained history but no recoverable code checkpoint.',
        ),
      );
    }
    ...
    if (createdRun && !host.active.has(createdRun.id)) {
      ...
      if (createdRun.status === 'starting')
        host.finish(createdRun, 'failed', 'Workspace preparation failed before native dispatch.');
      else host.store.putRun(createdRun);
    }
    throw error;
  } finally {
    try {
      host.workspaces.release(held, preparationFailure);
    } finally {
      host.admission.delete(conversation.id);
      prepared();
    }
  }
}
