...
export type CheckpointRecord = {
  ...
  directory: string;
  createdAt: string;
  boundary: 'before-turn' | 'completed-turn';
...
export class Checkpoints {
  ...
  capture(run: Run, boundary: CheckpointRecord['boundary']): CheckpointRecord {
    ...
    if (realpathSync(runDirectory) !== runDirectory)
      throw new Error('Run evidence directory was redirected.');
    const directory = join(runDirectory, 'checkpoints');
    mkdirSync(directory, { recursive: true, mode: 0o700 });
    if (realpathSync(directory) !== directory)
      throw new Error('Checkpoint directory was redirected.');
    ...
    const recoveryMessages = run.recoveryMessages?.map((message, index) => ({
      ...message,
      id: `${run.id}:recovery:${index}`,
      runId: run.id,
      conversationId: run.conversationId,
      createdAt: run.createdAt,
    }));
    const newRecoveryMessages =
    ...
    const manifest = createCheckpoint(run.workspace, directory, {
      ...
      messages: recoveryMessages
        ? [...recoveryMessages, ...newRecoveryMessages]
        : snapshot.messages.filter((message) => message.conversationId === run.conversationId),
      events: this.store.events(run.id),
      externalActions: snapshot.reviews.filter(
        (review) => review.conversationId === run.conversationId,
      ),
    });
    const checkpoint: CheckpointRecord = {
      ...
      snapshotTreeOid: manifest.snapshotTreeOid,
    };
    ...
    this.store.transaction(() => {
      ...
      this.store.append(
        ...
        boundary === 'before-turn'
          ? 'Starting state retained in a recoverable checkpoint'
          : 'Completed work retained in a recoverable checkpoint',
        { checkpoint },
      );
    });
  ...
  failed(run: Run, error: unknown): void {
    run.checkpointError =
      error instanceof Error ? error.message : 'Checkpoint could not be retained.';
    this.store.transaction(() => {
      this.store.putRun(run);
      this.store.append(run, 'checkpoint.failed', run.checkpointError!);
    });
  }
  ...
  selected(
    runId: string,
    digest: string,
  ): { run: Run; checkpoint: CheckpointRecord; manifest: CheckpointManifest } {
    ...
    if (!run || !checkpoint)
      throw new Error('This run does not contain the selected recoverable checkpoint.');
  ...
  restoreWorktree(
    ...
    return restoreCheckpointWorktree(
