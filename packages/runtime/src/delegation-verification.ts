...
export type DelegationVerificationInput = {
  ...
  workspaceLease: { reservationId: string; generation: number }; signal: AbortSignal;
};
export type DelegationVerificationResult = {
  status: 'passed' | 'next-check' | 'pending' | 'failed' | 'interrupted' | 'unavailable';
  cleanupConfirmed: boolean; checkId?: string; error?: string;
};
const bounded = (value: string): string => new TextDecoder().decode(Buffer.from(value).subarray(0, 16_384), { stream: true });
...
export class DelegationVerificationExecutor {
  ...
  async runNext(request: DelegationVerificationInput): Promise<DelegationVerificationResult> {
    ...
    if (assignment.role !== 'runtime-verification' || !attempt.workspace || !attempt.source || !attempt.sessionId) throw new Error('Native checks require a prepared runtime-verification attempt.');
    if (!run.enabledHarnessRoutes?.some(route => route.harness === assignment.harness && route.executable === assignment.executable)) throw new Error('This exact verification route was not enabled for the run.');
    const workspace = attempt.workspace;
    const audit = (type: string, summary: string, data: Record<string, unknown> = {}): void => { this.store.append(run, type, summary, { taskId: task.id, attemptId: attempt.id, ...data }); };
    const ownership = (): void => {
      const lease = this.workspaceLeases.snapshot().find(value => value.reservationId === input.workspaceLease.reservationId);
      if (!lease || lease.reservationId !== attempt.id || lease.generation !== input.workspaceLease.generation || lease.runId !== run.id || lease.state !== 'active' || lease.workspace !== workspace.path || lease.identity?.device !== workspace.identity.device || lease.identity.inode !== workspace.identity.inode) throw new Error('Verification workspace ownership is absent, stale, or quarantined.');
    ...
    let truncated = false;
    let error: string | undefined, output = '', exitCode: number | null = null, observedTreeOid = 'unavailable';
    const settle = (): void => {
      if (!activity) return;
      this.controls.finish(run.id, activity.token, { confirmed: cleanupConfirmed, ...(cleanupConfirmed ? { evidence: cleanupEvidence } : {}) }); activity = undefined;
    };
    ...
    try {
      ...
      if (exactTree() !== attempt.source.treeOid) throw new Error('Verification input no longer matches its prepared source.');
      if (!this.checks.snapshot(input)) {
        if (exactTree() !== attempt.source.treeOid) throw new Error('Verification input changed before manifest detection.');
        const detected = await detectVerificationCommands(workspace.path);
        guard();
        if (exactTree() !== attempt.source.treeOid) throw new Error('Verification input changed during manifest detection.');
        if (!detected.length || detected.some(command => command.unsupportedReason || !command.command)) {
          status = 'unavailable'; throw new Error(detected.find(command => command.unsupportedReason)?.unsupportedReason ?? 'No supported project checks were detected.');
        }
        this.checks.initialize({ ...input, commands: detected.map(command => [command.command, ...command.args]) });
      }
      settle(); guard();
      if (this.checks.passed(input)) { status = 'passed'; return { status, cleanupConfirmed }; }
      ...
      status = controller.signal.aborted ? 'interrupted' : dispatched && cleanupConfirmed && !error && exitCode === 0 && observedTreeOid === attempt.source.treeOid ? 'next-check' : 'failed';
      const current = this.controls.read(run.id);
      if (!dispatched && cleanupConfirmed && !controller.signal.aborted && current && !current.recoveryRequired && current.desired !== 'stopped' && (current.desired === 'paused' || current.generation !== input.expectedGeneration)) status = 'pending';
      if (status === 'failed' && !error) error = !dispatched ? 'Native check returned without its command identity.' : !cleanupConfirmed ? 'Native check cleanup could not be confirmed.' : observedTreeOid !== attempt.source.treeOid ? 'Native check changed its eligible input source.' : 'Native project check failed.';
    } catch (cause) {
      error = (cause instanceof Error ? cause.message : 'Native project check failed.').slice(0, 1000);
      if (cause instanceof AdapterRunFailure) { cleanupConfirmed = true; cleanupEvidence = cause.cleanupEvidence; }
      if (!nativeInvoked) { cleanupConfirmed = true; cleanupEvidence = { dispatch: 'not-invoked' }; }
      ...
      try {
        if (claimed) {
          ...
          if (status === 'pending') {
            this.records.finishSession({ ...identity, status: 'interrupted', cleanupConfirmed: true, cleanupEvidence });
            this.checks.deferCheck(identity);
          }
          else {
            const nativeStatus = status === 'next-check' ? 'completed' : status === 'interrupted' ? 'interrupted' : 'failed';
            this.records.finishSession({ ...identity, status: nativeStatus, cleanupConfirmed, ...(cleanupConfirmed ? { cleanupEvidence } : {}), ...(error ? { error } : {}) });
            if (dispatched) this.checks.finishCheck({ ...identity, status: nativeStatus, exitCode, output, truncated, observedTreeOid });
