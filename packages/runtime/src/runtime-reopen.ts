...
export function reconcileInterruptedRuns(store: Store, delegation: DelegationCommands, controls: DelegationControls): void {
  for (const run of store.runs()) {
    const sessions = delegation.records.sessions(run.id);
    const nativeCleanupConfirmed = sessions.length > 0 && sessions.every(session => session.cleanupConfirmed === true) && !controls.read(run.id)?.activities.some(activity => activity.state === 'cleanup-unconfirmed');
    if (['starting', 'running', 'stopping'].includes(run.status) && run.cleanupUnconfirmed !== true && nativeCleanupConfirmed) {
      ...
      store.transaction(() => { store.putRun(run); store.append(run, 'run.interrupted', run.error!, { nativeCleanupConfirmed: true }); });
    } else if (ACTIVE_RUN_STATUSES.has(run.status) || (!run.cleanupUnconfirmed && (delegation.records.sessions(run.id).some(session => session.state === 'cleanup-unconfirmed') || controls.read(run.id)?.activities.some(activity => activity.state === 'cleanup-unconfirmed')))) {
      store.transaction(() => {
    ...
    } else if (delegation.records.tasks(run.id).some(task => task.attempts.some(attempt => attempt.runtimeRecoveryRequired)) && run.status !== 'interrupted') {
      store.transaction(() => {
        ...
        store.putRun(run); store.append(run, 'run.interrupted', run.error, { nativeCleanupConfirmed: true, taskRecoveryRequired: true });
      });
    }
    store.exportRun(run);
  }
