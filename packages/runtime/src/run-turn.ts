...
export function finishRun(host: Pick<RuntimeBindings, 'store'>, run: Run, status: Run['status'], error?: string): void {
  run.status = status; run.updatedAt = now(); run.error = error;
  host.store.transaction(() => { host.store.putRun(run); host.store.append(run, `run.${status}`, error ?? `Run ${status}`); });
  try { host.store.exportRun(run); } catch { /* SQLite remains authoritative; logs are repaired when reopened. */ }
}

export function recordAdapterEvent(host: Pick<RuntimeBindings, 'store' | 'active' | 'changed'>, run: Run, event: AdapterEvent): void {
  ...
  host.store.transaction(() => {
    ...
    if (event.type === 'message.delta' && typeof event.data?.text === 'string' && typeof event.data?.messageId === 'string') {
      ...
      host.store.putMessage({ id, runId: run.id, conversationId: run.conversationId, role: 'assistant', text: (existing?.text ?? '') + event.data.text, createdAt: existing?.createdAt ?? now() });
    }
  });
  host.store.exportRun(run); host.changed();
}

export async function executeNativeTurn(host: RuntimeBindings, run: Run, controller: AbortController, lease: WorkspaceLease): Promise<void> {
  ...
  const settleSession = (status: 'completed' | 'failed' | 'interrupted', cleanupConfirmed: boolean, cleanupEvidence?: Record<string, unknown>, error?: string): void => {
    ...
    host.delegation.records.finishSession({ runId: run.id, sessionId, status, cleanupConfirmed, ...(cleanupConfirmed && cleanupEvidence ? { cleanupEvidence } : {}), ...(error ? { error } : {}) });
    sessionSettled = true;
  ...
  const onEvent = (event: AdapterEvent): void => {
    if (event.type === 'session.turn-started') {
      ...
      if (sessionId && threadId && turnId) { host.delegation.records.bindSession({ runId: run.id, sessionId, threadId, turnId }); bound = true; }
    }
  ...
  try {
    ...
    host.event(run, { type: 'run.started', summary: `Connecting to ${harness}`, data: { harness, executable: run.executable, executableVersion: run.executableVersion } });
    const messages = run.recoveryMessages?.map(message => ({ ...message })) ?? host.store.messages(run.conversationId).map(({ role, text }) => ({ role, text }));
    ...
    const adapter = host.nativeAdmission.adapter(run.harness ?? 'codex', host.routes.adapterForRun(run), { owner: { kind: 'run', id: run.id }, runId: run.id, sessionId, assertCurrent: () => { host.workspaceOwnership.assert(lease); if (run.workspaceIdentity) assertWorkspaceIdentity(run.workspace, run.workspaceIdentity); } });
    adapterInvoked = true;
    ...
    if (cleanupUnconfirmed) {
      ...
      host.finish(run, 'stop-unconfirmed', 'The harness stopped responding; cleanup could not be confirmed.');
    } else if (result.status === 'completed' && !bound) host.finish(run, 'failed', 'Native adapter completed without a registered thread and turn identity.');
    else host.finish(run, result.status);
    if (result.status === 'completed' && bound && run.checkpoints?.length) {
      try { host.workspaceOwnership.stage({ ...lease, phase: 'completed-checkpoint' }); host.workspaceOwnership.assert(lease); host.checkpoints.capture(run, 'completed-turn'); }
      catch (cause) { runtimeCleanupConfirmed &&= workspaceCleanupConfirmed(cause); host.checkpoints.failed(run, cause); if (!workspaceCleanupConfirmed(cause)) { run.cleanupUnconfirmed = true; host.finish(run, 'stop-unconfirmed', 'Checkpoint process cleanup could not be confirmed.'); } }
    }
  } catch (error) {
    ...
    try {
      ...
      host.workspaceOwnership.release({ ...lease, cleanupConfirmed, cleanupEvidence: { runId: run.id, nativeCleanupConfirmed, runtimeCleanupConfirmed, durableStatus: host.store.runs().find(value => value.id === run.id)?.status ?? 'unknown' } });
    } finally { host.active.delete(run.id); host.changed(); }
  }
}
