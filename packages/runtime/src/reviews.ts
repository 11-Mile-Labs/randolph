...
export class Reviews {
  private readonly checking = new Map<
    string,
    { controller: AbortController; done: Promise<ReviewRecord> }
  >();
  ...
  constructor(
    ...
    private readonly adapterFor: (
      run: Run,
      review: ReviewRecord,
      check?: { id: string; assertCurrent: () => void },
    ) => HarnessAdapter,
    private readonly canWork: (conversationId: string) => boolean,
    private readonly changed: () => void,
    private readonly executorOwnsDeadlines = false,
    ...
    for (const review of store.reviews()) {
      if (review.status === 'checking' || review.status === 'delivering') {
        ...
        review.error =
          'The app ended during this operation. Nothing has resumed. Review the evidence and explicitly continue.';
  ...
  hasActiveWork(conversationId?: string): boolean {
    return conversationId
      ? this.busy.has(conversationId) ||
          this.store
            .reviews()
            .some(
              (review) =>
                review.conversationId === conversationId && review.status === 'stop-unconfirmed',
            )
      : this.busy.size > 0;
  }
  ...
  private assertIdle(conversationId: string): void {
    ...
    if (!this.canWork(conversationId) || this.hasActiveWork(conversationId))
      throw new Error('Wait for active conversation work or checks to finish.');
  }
  invalidate(conversationId: string): void {
    for (const review of this.store
      .reviews()
  ...
  prepare(conversationId: string): ReviewRecord {
    ...
    const run = this.store
      .runs()
      .findLast((item) => item.conversationId === conversationId && item.executionMode === 'code');
    if (!run) throw new Error('Run a Code conversation before requesting review.');
    if (run.status === 'stop-unconfirmed')
      throw new Error('Process cleanup must be confirmed before review.');
    const project = this.store.projects().find((item) => item.id === run.projectId);
    if (!project) throw new Error('Project does not exist.');
    const acquired = this.ownership.acquire({
      reservationId: randomUUID(),
      ownerId: `review-prepare:${conversationId}`,
      runId: run.id,
    ...
    if (acquired.status !== 'acquired')
      throw new Error('Review workspace ownership is unavailable.');
    ...
    try {
      ...
      const priorDeliveries = this.store
        .reviews()
        .filter(
          (review) =>
      ...
      for (const prior of priorDeliveries)
        if (
          reconcileSupersededDelivery(prior.basis.root, prior.basis.workspace, prior.deliveryPlan!)
            .merged
        )
          throw new Error(
            'The earlier delivery already merged. Continue its cleanup before requesting a new review.',
          );
      ...
      for (const prior of priorDeliveries) {
        ...
        this.save(
          prior,
          'delivery.superseded',
          'Unmerged delivery intent replaced by a fresh review. The earlier approval cannot be used again.',
        );
      }
      ...
      this.save(review, 'review.created', 'Changes are ready for checks and final review.', {
        treeOid: basis.treeOid,
        parentOid: basis.parentOid,
        files: basis.files.length,
      });
      return review;
    } catch (error) {
      ...
      this.ownership.release({
        ...
        cleanupEvidence: cleanupConfirmed
          ...
          : { stage: 'review-prepare', cleanup: 'unconfirmed' },
      });
    }
  }
  private assertCurrent(review: ReviewRecord): void {
    const current = createGitReview(
      review.basis.root,
      review.basis.workspace,
      review.basis.evidenceDir,
    );
    if (
      current.treeOid !== review.basis.treeOid ||
      current.parentOid !== review.basis.parentOid ||
      current.parentBranch !== review.basis.parentBranch ||
      current.workspaceHead !== review.basis.workspaceHead
    ) {
      review.status = 'stale';
  ...
  async verify(id: string): Promise<ReviewRecord> {
    ...
    if (review.status !== 'pending' && review.status !== 'interrupted')
      throw new Error('Request a fresh review before running checks.');
    if (review.deliveryPlan)
      throw new Error('Delivery is pending; continue that delivery before running new checks.');
    if (!this.adapterFor(this.run(review), review).runCommand)
      throw new Error("This run's harness has no verified command execution capability.");
    const ownership = this.ownership.acquire({
      reservationId: randomUUID(),
      ownerId: `review:${review.id}`,
      runId: review.runId,
      workspace: review.basis.workspace,
      provenance: {
        kind: 'verification',
        id: review.id,
    ...
    try {
      ...
      cleanupConfirmed =
        String(review.status) !== 'stop-unconfirmed' &&
        !review.verification?.checks.some((check) => !check.cleanupVerified);
      return result;
    } catch (error) {
      cleanupConfirmed =
        String(review.status) !== 'stop-unconfirmed' &&
      ...
      this.busy.delete(review.conversationId);
      this.ownership.release({
        reservationId: lease.reservationId,
        generation: lease.generation,
        cleanupConfirmed,
        cleanupEvidence: cleanupConfirmed
          ? { stage: 'review-verification', status: review.status }
          : { stage: 'review-verification', cleanup: 'unconfirmed' },
      });
      this.changed();
    }
  }
  private async runChecks(
    ...
    try {
      ...
      this.save(
        ...
        'Running project checks through the native permission boundary.',
      );
      ...
      const assertCurrent = () => {
        ...
        assertWorkspaceIdentity(review.basis.workspace, identity);
        this.assertCurrent(review);
      };
      ...
      review.verification = await runVerification(review.basis.workspace, commands, {
        ...
        executorOwnsDeadlines: this.executorOwnsDeadlines,
        executor: async (workspace, command, options) =>
          this.adapterFor(run, review, { id: command.id, assertCurrent }).runCommand!({
        ...
        onEvent: (event) => {
          if (event.type === 'check-started')
            review.progress = { checkId: event.checkId, startedAt: now(), output: '' };
          if (event.type === 'check-output') {
            if (review.progress)
              review.progress.output = (review.progress.output + (event.output ?? '')).slice(
                -64 * 1024,
              );
            this.save(review, 'verification.progress', `Checking ${event.checkId}`, {
            ...
            this.save(
              ...
              event.type === 'check-started'
                ? `Checking ${event.checkId}`
                : `Check ${event.checkId} ${event.result?.status ?? 'finished'}`,
              { checkId: event.checkId, ...(event.result ? { result: event.result } : {}) },
            );
        },
      });
      if (review.verification.checks.some((check) => !check.cleanupVerified)) {
        ...
        review.error =
          'Check process cleanup could not be confirmed. Further work in this conversation is blocked, including after reopening.';
        this.save(review, 'verification.stop-unconfirmed', review.error);
        return review;
      }
      this.assertCurrent(review);
      review.status = 'pending';
      this.save(review, 'verification.completed', `Project checks ${review.verification.status}.`, {
        status: review.verification.status,
      });
      return review;
    } catch (error) {
      if (!workspaceCleanupConfirmed(error)) review.status = 'stop-unconfirmed';
      if (review.status !== 'stale' && review.status !== 'stop-unconfirmed')
  ...
  async approve(input: ApproveReviewInput): Promise<ReviewRecord> {
    ...
    this.assertIdle(review.conversationId);
    if (review.status === 'stale' || review.status === 'stop-unconfirmed')
      throw new Error('Request a fresh review before final approval.');
    if (!review.deliveryPlan && review.status !== 'pending')
      throw new Error('Request a fresh review before final approval.');
    if (review.verification?.status !== 'passed')
      throw new Error('Successful project checks are required before final approval.');
    const acquired = this.ownership.acquireMany([
      {
        reservationId: randomUUID(),
        ownerId: `delivery:${review.id}`,
        runId: review.runId,
        workspace: review.basis.workspace,
        provenance: {
          kind: 'delivery',
    ...
    const leases = acquired.map((item) => {
      if (item.status !== 'acquired')
        throw new Error('Delivery workspace ownership is unavailable.');
      return item.lease;
    });
    this.busy.add(review.conversationId);
    let cleanupConfirmed = true;
    try {
      for (const lease of leases) {
        ...
        this.ownership.stage({
          reservationId: lease.reservationId,
          generation: lease.generation,
          phase: 'delivery-mutation',
        });
      }
      if (!review.deliveryPlan) {
        this.assertCurrent(review);
        review.deliveryPlan = createGitDeliveryPlan(
          review.basis.root,
          review.basis.workspace,
          review.basis,
          input.message,
        ...
        this.save(
          review,
          'delivery.approved',
          'Final approval recorded for this exact commit and parent.',
          {
            commitOid: review.deliveryPlan.commitOid,
            treeOid: review.basis.treeOid,
            parentOid: review.basis.parentOid,
          },
        );
      } else {
        review.status = 'delivering';
        ...
        this.save(
          review,
          'delivery.continued',
          'Explicitly continuing the previously approved delivery.',
        );
      }
      ...
      this.save(review, 'delivery.committed', 'Approved commit is retained.', {
        commitOid: plan.commitOid,
      });
      if (!state.merged) mergeGitDelivery(review.basis.root, review.basis.workspace, plan);
      review.merged = true;
      this.save(review, 'delivery.merged', 'Approved commit merged into the parent branch.', {
        commitOid: plan.commitOid,
        parentBranch: review.basis.parentBranch,
      });
      ...
      this.save(
        review,
        'delivery.completed',
        'Local delivery completed. Push remains a separate action.',
        { commitOid: plan.commitOid, cleaned: review.cleaned },
      );
      return review;
    } catch (error) {
      cleanupConfirmed = workspaceCleanupConfirmed(error);
      review.status = review.deliveryPlan
        ? 'interrupted'
        : this.get(review.id).status === 'stale'
          ? 'stale'
          : 'failed';
      review.error = errorText(error);
      ...
      for (const lease of leases)
        this.ownership.release({
          ...
          cleanupConfirmed,
          cleanupEvidence: cleanupConfirmed
            ? { stage: 'delivery', status: review.status }
            : { stage: 'delivery', cleanup: 'unconfirmed' },
        });
      this.busy.delete(review.conversationId);
      this.changed();
    }
  ...
  async stop(id: string): Promise<void> {
    ...
    if (state) {
      ...
      try {
        ...
        /* Failure is recorded by runChecks. */
      }
    }
  }
