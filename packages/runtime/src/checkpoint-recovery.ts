...
export type RecoveryHost = {
  ...
  preparation(): () => void;
  planWorkspace(root: string, provenance: WorkspaceProvenance, plan: () => string, held: Map<string, WorkspaceLease>, readOnlyPlanning?: boolean): { parent: WorkspaceLease; lease: WorkspaceLease; workspace: string };
  transferWorkspace(plan: { parent: WorkspaceLease; lease: WorkspaceLease }, held: Map<string, WorkspaceLease>): WorkspaceLease;
...
export function recoveryContext(metadata: Record<string, unknown>, kind: 'restart' | 'rerun', sourceRunId: string): RecoveryContext {
...
export async function recoverLinkedCheckpoint(host: RecoveryHost, kind: 'restart' | 'rerun', input: RestartCheckpointInput | RerunCheckpointInput): Promise<LinkedRunResult> {
  ...
  if (kind === 'restart') {
    ...
    const sourceRuns = host.store.runs().filter(run => run.conversationId === sourceConversation.id);
    const sourceIndex = sourceRuns.findIndex(run => run.id === sourceRun.id);
    const laterRunBlocksRetry = sourceRuns.slice(sourceIndex + 1).some(run => run.recoveryKind !== 'restart' || run.sourceRunId !== sourceRun.id || run.sourceCheckpointDigest !== selected.checkpoint.digest || run.status !== 'failed' || run.error !== PRE_DISPATCH_RECOVERY_FAILURE);
  ...
  try {
    ...
    const targetConversation: Conversation = kind === 'restart'
      ? { ...host.conversation(sourceConversation.id), harness: context.harness, model: context.model, effort: context.effort, executionMode: context.executionMode, updatedAt: createdAt }
    ...
    const planned = host.planWorkspace(project.root, { kind: 'run', id: runId, projectId: project.id, conversationId: targetConversation.id }, () => join(project.root, '.worktrees', `randolph-${runId}`), held);
    ...
    host.store.transaction(() => {
      ...
      if (kind === 'rerun') for (const [index, message] of context.messages.entries()) host.store.putMessage({ ...message, id: `${runId}:recovery:${index}`, runId, conversationId: targetConversation.id, createdAt });
      host.store.append(run!, kind === 'restart' ? 'run.restart-requested' : 'run.rerun-requested', kind === 'restart' ? 'Explicit restart requested from the last safe checkpoint' : 'Explicit rerun requested in a linked conversation', { sourceRunId: sourceRun.id, sourceConversationId: sourceConversation.id, checkpointDigest: selected.checkpoint.digest, harness: context.harness, executable: info.executable, executableVersion: info.version, workspace });
    });
    try {
      if (context.memory) host.memory.retain(project.id, run.id, context.memory);
      host.store.exportRun(run);
      const restored = host.checkpoints.restoreWorktree(sourceRun.id, selected.checkpoint.digest, project.root, run.id);
    ...
    if (run && !host.active.has(run.id) && host.store.runs().some(value => value.id === run!.id)) {
    ...
    try { host.releaseWorkspaces(held, preparationFailure); } finally {
