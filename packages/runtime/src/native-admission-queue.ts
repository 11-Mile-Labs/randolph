...
export class NativeAdmissionQueue {
  ...
  acquire(request: Request): Promise<CapacityLease> {
    if (this.pending.has(request.reservation.reservationId))
      return Promise.reject(new Error('Native admission request is already queued.'));
    return new Promise((resolve, reject) => {
      this.pending.set(request.reservation.reservationId, {
        ...request,
        order: this.sequence++,
        resolve,
        reject,
      ...
      try {
        ...
        for (const request of this.pending.values()) {
          try {
            ...
            if (!Number.isFinite(priority)) throw new Error('Native queue priority is invalid.');
            requests.push({ request, priority });
          } catch (error) {
            this.pending.delete(request.reservation.reservationId);
        ...
        for (const { request } of requests) {
          ...
          try {
            ...
            if (result.status === 'queued') {
              ...
              if (reason !== request.lastReason) {
                request.queued(result.reason);
                request.lastReason = reason;
              }
              continue;
            }
            if (result.replayed)
              throw new Error('An existing native reservation cannot authorize another launch.');
            lease = result.lease;
            request.assertCurrent();
            this.pending.delete(request.reservation.reservationId);
