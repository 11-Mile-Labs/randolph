...
export function chatEvents(host: Pick<RuntimeBindings, 'store'>, input: ChatEventsInput): ChatEventsResult {
...
export function restoreCheckpoint(host: Pick<RuntimeBindings, 'accepting' | 'workspaces' | 'workspaceOwnership' | 'checkpoints'>, input: CheckpointInput, destination: string): CheckpointRestore {
  ...
  let failure: unknown;
  try {
    const lease = host.workspaces.own(destination, { kind: 'checkpoint-export', id: randomUUID() }, held);
  ...
  finally { host.workspaces.release(held, failure); }
}
...
export function addProject(host: Pick<RuntimeBindings, 'accepting' | 'store' | 'changed'>, path: string): Project {
  ...
  host.store.putProject(project); host.changed(); return project;
}
...
export function markRead(host: Pick<RuntimeBindings, 'store' | 'changed'> & { conversation(id: string): Conversation }, conversationId: string): void {
  const conversation = host.conversation(conversationId);
  const sequence = host.store.events().filter(event => event.conversationId === conversationId).at(-1)?.sequence ?? 0;
  if (sequence <= conversation.lastReadSequence) return;
  host.store.putConversation({ ...conversation, lastReadSequence: sequence }); host.changed();
}
...
export function requireConversation(host: Pick<RuntimeBindings, 'store'>, id: string): Conversation {
