...
function canonicalDirectory(path: string, label: string): string {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink() || realpathSync(path) !== path)
    throw new Error(`${label} was redirected or is not a real directory.`);
  return path;
}
...
function directoryIdentity(path: string): string {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink())
    throw new Error('Project or worktree identity changed.');
...
function inspectFile(
  ...
  if (!before.isFile() || before.isSymbolicLink() || before.nlink !== 1 || before.size > limit)
    throw new Error(`${label} is linked, nonregular, or oversized.`);
  ...
  try {
    ...
    if (total !== actual.size || fstatSync(fd).size !== actual.size)
      throw new Error(`${label} changed while it was being read.`);
...
function copyObject(
  ...
  if (exists(destination)) {
    ...
    if (current.bytes !== expectedBytes || current.sha256 !== expectedDigest)
      throw new Error(
        'A conflicting Git object pack already exists in this project. Restore files to a new folder first.',
      );
    return false;
  }
...
export function importCheckpointObjects(
  projectRoot: string,
  commonDirectory: string,
  manifest: CheckpointManifest,
): void {
  const packDirectory = canonicalDirectory(
    join(commonDirectory, 'objects', 'pack'),
  ...
  try {
    if (
      copyObject(
        join(manifest.directory, 'objects.pack'),
        pack,
        manifest.pack.bytes,
    ...
    if (
      copyObject(
        join(manifest.directory, 'objects.idx'),
        index,
        manifest.pack.indexBytes,
...
function restoreLimitation(message: string): Error {
  return new Error(
    `${message} Restore files to a new folder first; linked execution requires the original related project.`,
  );
}

export function restoreCheckpointWorktree(
  directory: string,
  expectedDigest: string,
  projectRoot: string,
  workspaceId: string,
  ...
  let root: string;
  try {
    root = canonicalDirectory(projectRoot, 'Original project');
  } catch {
    throw restoreLimitation('The original project is missing or redirected.');
  }
  if (gitText(root, ['rev-parse', '--show-toplevel']) !== root)
    throw restoreLimitation('The original project is no longer a Git repository.');
  const worktrees = ensureWorktreeDirectory(root);
  const workspace = join(worktrees, `randolph-${workspaceId}`);
  if (exists(workspace))
    throw new Error(
      'Linked checkpoint destination already exists; existing work is never overwritten.',
    );
  const registered = runSafeGit(root, ['worktree', 'list', '--porcelain', '-z'])
    .toString('utf8')
    .split('\0')
    .some((row) => row === `worktree ${workspace}`);
  if (registered)
    throw new Error(
      'Linked checkpoint destination is already registered; reconcile it before retrying.',
    );
  let parentBranch: string;
  try {
    parentBranch = gitText(root, ['symbolic-ref', '--quiet', '--short', 'HEAD']);
  } catch (error) {
    if (!workspaceCleanupConfirmed(error)) throw error;
    throw restoreLimitation('The original project parent branch is detached.');
  }
  ...
  if (gitText(root, ['rev-parse', '--show-object-format']) !== manifest.objectFormat)
    throw restoreLimitation('The original project uses a different Git object format.');
  importCheckpointObjects(root, commonDirectory, manifest);
  try {
    runSafeGit(root, ['merge-base', '--is-ancestor', manifest.baseCommitOid, parentOid]);
  } catch (error) {
    if (!workspaceCleanupConfirmed(error)) throw error;
    throw restoreLimitation('The checkpoint base is not an ancestor of the current parent branch.');
  }
  let created = false;
  ...
  try {
    ...
    workspaceIdentity = directoryIdentity(
      canonicalDirectory(workspace, 'Linked checkpoint worktree'),
    );
    ...
    if (
      ...
      directoryIdentity(commonDirectory) !== commonIdentity ||
      gitText(root, ['symbolic-ref', '--quiet', '--short', 'HEAD']) !== parentBranch ||
      gitText(root, ['rev-parse', '--verify', 'HEAD^{commit}']) !== parentOid
    )
      throw new Error(
        'Project identity or parent branch changed during linked checkpoint restore.',
      );
    const gitDirectory = canonicalDirectory(
      gitText(workspace, ['rev-parse', '--path-format=absolute', '--git-dir']),
      'Linked worktree Git directory',
    );
    if (
      captureGitTree(workspace, gitDirectory) !== manifest.snapshotTreeOid ||
      captureGitTree(workspace, gitDirectory) !== manifest.snapshotTreeOid ||
      gitText(workspace, ['rev-parse', '--verify', 'HEAD^{commit}']) !== manifest.baseCommitOid
    )
      throw new Error('Linked checkpoint worktree does not match the retained snapshot.');
    ...
    if (created && workspaceCleanupConfirmed(error)) {
      try {
        if (
          ...
          runSafeGit(root, ['worktree', 'remove', '--force', workspace]);
      } catch (cleanupError) {
        if (!workspaceCleanupConfirmed(cleanupError))
          throw new Error('Linked restore cleanup could not be confirmed.', {
            cause: cleanupError,
          }); /* Preserve unknown replacement paths. */
      }
    }
    throw error;
  }
