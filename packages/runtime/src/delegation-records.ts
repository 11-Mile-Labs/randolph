...
export type DelegationCheck = { id: string; argv: string[]; sessionId: string; commandId?: string; state: 'queued' | 'dispatching' | 'running' | 'passed' | 'failed' | 'interrupted' | 'cleanup-unconfirmed' | 'unavailable'; exitCode?: number | null; output?: string; truncated?: boolean; cleanupConfirmed?: boolean; cleanupEvidence?: Record<string, unknown>; observedTreeOid?: string; error?: string };
export type DelegationVerification = { input: DelegationSourceSnapshot; manifestDigest: string; commands: Array<{ id: string; argv: string[] }>; checks: DelegationCheck[]; createdAt: string; updatedAt: string };
export type DelegationAttempt = {
  ...
  result?: DelegationAttemptResult; preparation?: DelegationPreparation; verification?: DelegationVerification; outputPublication?: DelegationOutputPublication; cleanupConfirmed?: boolean; cleanupEvidence?: Record<string, unknown>; runtimeRecoveryRequired?: boolean; error?: string;
};
...
export class DelegationRecords {
  ...
  bindSession(input: { runId: string; sessionId: string; threadId: string; turnId: string }): DelegationSession {
    ...
    this.store.transaction(() => { const session = this.sessions(input.runId).find(candidate => candidate.id === input.sessionId); if (!session) throw new Error('Delegation session belongs to another run or does not exist.'); if (session.role === 'verification' && session.taskId) throw new Error('Delegated verification sessions require command identity, not a model turn.'); const native = { ...session.native, threadId: input.threadId, turnId: input.turnId }; if (session.state === 'running' && session.native?.threadId === input.threadId && session.native?.turnId === input.turnId) { result = session; return; } if (session.state !== 'prepared' && session.state !== 'dispatch-intent') throw new Error('Delegation session cannot be rebound.'); if (session.native?.threadId || session.native?.turnId) throw new Error('Delegation session native identity cannot be rebound.'); session.native = native; session.state = 'running'; session.updatedAt = now(); this.store.db.prepare('UPDATE delegation_sessions SET document=? WHERE id=?').run(JSON.stringify(session), session.id); this.store.append(run, 'delegation.session-bound', 'Delegation native session bound to its registered turn.', { sessionId: session.id, threadId: input.threadId, turnId: input.turnId }); result = session; }); return result!;
  }
  bindCommandSession(input: { runId: string; sessionId: string; commandId: string }): DelegationSession {
    const run = this.run(input.runId); text(input.commandId, 'Native command ID'); let result: DelegationSession | undefined;
    this.store.transaction(() => { const session = this.sessions(input.runId).find(candidate => candidate.id === input.sessionId); if (!session || session.role !== 'verification' || session.state !== 'prepared' || session.native?.commandId) throw new Error('Verification command session cannot be rebound.'); session.native = { commandId: input.commandId }; session.state = 'running'; session.updatedAt = now(); this.store.db.prepare('UPDATE delegation_sessions SET document=? WHERE id=?').run(JSON.stringify(session), session.id); this.store.append(run, 'delegation.command-bound', 'Verification session bound to its native command.', { sessionId: session.id, commandId: input.commandId }); result = session; }); return result!;
  }
