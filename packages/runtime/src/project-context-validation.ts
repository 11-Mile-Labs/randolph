...
const pathControlCharacter = (value: string): boolean =>
  Array.from(value).some((character) => {
    const code = character.charCodeAt(0);
    return code < 32 || code === 127;
  });

const textControlCharacter = (value: string): boolean =>
  Array.from(value).some((character) => {
    const code = character.charCodeAt(0);
...
function text(value: unknown, field: string, max: number, required = false): string {
  if (typeof value !== 'string' || value.length > max || textControlCharacter(value)) {
    throw new Error(
      `Project context ${field} must be text of at most ${max} characters without control characters.`,
    );
  }
...
function documentPath(value: unknown): string {
  if (
    ...
    value.length === 0 ||
    value.length > 1024 ||
    pathControlCharacter(value) ||
    value.includes('\\') ||
    value.startsWith('/')
  ) {
    throw new Error(
      'Project context document paths must be bounded POSIX relative paths without backslashes or control characters.',
    );
  }
  const segments = value.split('/');
  if (segments.some((segment) => segment === '..'))
    throw new Error('Project context document paths must not contain parent directory segments.');
...
export function parseProjectContext(value: unknown): ProjectContext {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error('Project context must be a YAML object.');
  ...
  if (!Array.isArray(source.documents))
    throw new Error('Project context documents must be an array.');
  if (source.documents.length > 30)
    throw new Error('Project context may contain at most 30 document references.');
  const paths = new Set<string>();
  const documents = source.documents.map((entry) => {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry))
      throw new Error('Project context document entries must be objects.');
    const item = entry as Record<string, unknown>;
    const path = documentPath(item.path);
    if (paths.has(path))
      throw new Error(`Project context contains duplicate document path: ${path}.`);
    paths.add(path);
