...
export class Store {
  ...
  constructor(readonly root: string) {
    ...
    if (version > 5) {
      this.db.close();
      throw new Error('This data directory was created by a newer Randolph version.');
    }
    if (version >= 3 && !this.tableExists('projects')) {
      this.db.close();
      throw new Error('This Randolph database is corrupt.');
    }
  ...
  transaction<T>(action: () => T): T {
    const outer = this.transactionDepth === 0,
      savepoint = `randolph_transaction_${this.transactionDepth}`;
  ...
  projects(): Project[] {
    return this.documents('projects');
  }
  conversations(): Conversation[] {
    return this.documents('conversations');
  }
  runs(): Run[] {
    return this.documents('runs');
  }
  messages(conversationId?: string): Message[] {
    return this.documents<Message>('messages').filter(
      (message) => !conversationId || message.conversationId === conversationId,
    );
  }
  reviews(): ReviewRecord[] {
    return this.documents('reviews');
  }
  putReview(review: ReviewRecord): void {
    this.db
      .prepare(
        'INSERT INTO reviews VALUES (?, ?, ?) ON CONFLICT(id) DO UPDATE SET document=excluded.document',
      )
      .run(review.id, review.conversationId, JSON.stringify(review));
  }
  private documents<T>(table: 'projects' | 'conversations' | 'runs' | 'messages' | 'reviews'): T[] {
    return (this.db.prepare(`SELECT document FROM ${table} ORDER BY rowid`).all() as Row[]).map(
      (row) => JSON.parse(String(row.document)) as T,
    );
  }
  ...
  putConversation(conversation: Conversation): void {
    this.db
      .prepare(
        'INSERT INTO conversations VALUES (?, ?, ?) ON CONFLICT(id) DO UPDATE SET document=excluded.document',
      )
      .run(conversation.id, conversation.projectId, JSON.stringify(conversation));
  }
  putRun(run: Run): void {
    this.db
      .prepare(
      ...
      .run(run.id, run.conversationId, JSON.stringify(run));
  }
  putMessage(message: Message): void {
    this.db
      .prepare(
        'INSERT INTO messages VALUES (?, ?, ?) ON CONFLICT(id) DO UPDATE SET document=excluded.document',
      )
      .run(message.id, message.runId, JSON.stringify(message));
  }
  message(id: string): Message | undefined {
    const row = this.db.prepare('SELECT document FROM messages WHERE id=?').get(id) as
      | Row
      | undefined;
  ...
  events(runId?: string): RunEvent[] {
    const rows = (
      runId
        ? this.db
            .prepare('SELECT sequence, document FROM events WHERE run_id=? ORDER BY sequence')
            .all(runId)
        : this.db
            .prepare(
              'SELECT sequence, document FROM (SELECT sequence, document FROM events ORDER BY sequence DESC LIMIT 2000) ORDER BY sequence',
            )
            .all()
    ) as Row[];
    return rows.map(
      (row) =>
        ({ ...JSON.parse(String(row.document)), sequence: Number(row.sequence) }) as RunEvent,
    );
  }
  chatEvents(conversationId: string, runId: string, afterSequence: number): ChatEventsResult {
    const runRow = this.db
      .prepare('SELECT conversation_id, document FROM runs WHERE id=?')
      .get(runId) as (Row & { conversation_id?: string }) | undefined;
    ...
    const rows = this.db
      .prepare(
        'SELECT sequence, document FROM events WHERE run_id=? AND sequence>? ORDER BY sequence',
      )
  ...
  append(run: Run, type: string, summary: string, data: Record<string, unknown> = {}): void {
    ...
    this.db
      .prepare('INSERT INTO events(run_id, document) VALUES (?, ?)')
      .run(run.id, JSON.stringify(event));
  }
  runDirectory(run: Run): string {
    return join(
      ...
      run.projectId,
      'runs',
      `${run.createdAt.slice(0, 10).replaceAll('-', '')}_${run.id}`,
    );
  }
  exportRun(run: Run): void {
      ...
      JSON.stringify(
        {
          ...
          configuration: { harness: run.harness ?? 'codex', model: run.model, effort: run.effort },
        },
        null,
        2,
      ) + '\n',
    ...
    this.replace(
      join(dir, 'logs', 'events.jsonl'),
      events.map((event) => JSON.stringify(event) + '\n').join(''),
    );
    this.replace(
      join(dir, 'logs', 'activity.log'),
      events
        .map((event) => `${event.at} #${event.sequence} ${event.type} ${event.summary}\n`)
        .join(''),
    );
  }
