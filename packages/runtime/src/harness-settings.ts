...
export function validateHarnessRoutes(value: unknown): HarnessRoute[] {
  if (!Array.isArray(value) || value.length > 32)
    throw new Error('Enabled routes must be a list of at most 32 harness CLIs.');
  const seen = new Set<string>();
  return value.map((route) => {
    const validated = validateDefaults({ ...route, model: 'route', effort: 'route' });
    if (!validated.executable)
      throw new Error('Each enabled route requires an absolute CLI executable.');
...
export function assertHarnessRoute(
  ...
  if (
    ...
    !settings.enabledRoutes.some(
      (route) => route.harness === harness && route.executable === executable,
    )
  ) {
    throw new Error(
      'This harness CLI is not enabled for this project. Enable it in Project settings before starting work.',
    );
  }
}
...
function validateDefaults(value: unknown): HarnessDefaults {
  if (!value || typeof value !== 'object')
    throw new Error('Harness settings must contain a model and effort.');
  ...
  if (
    ...
    throw new Error(
      'Harness settings require a supported harness and nonempty, bounded model and effort names.',
    );
  }
  if (
    executable !== undefined &&
    executable !== null &&
    (!validText(executable, 4096) || !isAbsolute(executable))
  )
    throw new Error('The CLI executable must be a bounded absolute path.');
  return { harness, model, effort, ...(executable === undefined ? {} : { executable }) };
}

function validateConfiguration(
  value: unknown,
): HarnessDefaults & { enabledRoutes?: HarnessRoute[] } {
  const defaults = validateDefaults(value);
  const routes = (value as Record<string, unknown>).enabledRoutes;
  return {
    ...defaults,
    ...(routes === undefined ? {} : { enabledRoutes: validateHarnessRoutes(routes) }),
  };
}
function snapshot(result: {
  revision: string | null;
  value: ReturnType<typeof validateConfiguration> | null;
  error?: string;
}): HarnessSettings {
  const { enabledRoutes, ...defaults } = result.value ?? {};
  return {
    revision: result.revision,
    defaults: result.value ? (defaults as HarnessDefaults) : null,
...
export function writeHarnessSettings(
  root: string,
  ...
  const routes =
    enabledRoutes === undefined ? current.enabledRoutes : validateHarnessRoutes(enabledRoutes);
  const values = { ...defaults, ...(routes === undefined ? {} : { enabledRoutes: routes }) };
  return snapshot(
    writeYamlSettings(root, 'config.harness.yaml', values, expectedRevision, validateConfiguration),
  );
}
