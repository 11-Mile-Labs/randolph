...
function verifyRoot(root: string): void {
  try {
    if (!lstatSync(root).isDirectory() || realpathSync(root) !== resolve(root))
      throw new Error('redirected');
  } catch {
    throw new Error(
      'Project directory is missing or redirected. Re-add the project before editing settings.',
    );
  }
}
...
function readSource(root: string, fileName: string): Buffer | null {
  ...
  if (!before.isFile() || before.nlink !== 1)
    throw new Error(`${fileName} must be a regular file without links.`);
...
): { document: Document; value: T } {
  const source = new TextDecoder('utf-8', { fatal: true }).decode(bytes);
  const document = parseDocument(source, { uniqueKeys: true });
  if (document.errors.length || document.warnings.length || !isMap(document.contents)) {
    throw new Error(
      `${fileName} must contain valid YAML with unique keys and no unsupported tags.`,
    );
  }
...
export function readYamlSettings<T>(
  root: string,
  fileName: string,
  validate: (value: unknown) => T,
  fallback: T,
  ...
  try {
    ...
    return {
      ...
      value: fallback,
      error: error instanceof Error ? error.message : 'Cannot read project settings.',
    };
  }
}

export function writeYamlSettings<T extends Record<string, unknown>>(
  root: string,
  fileName: string,
  values: T,
  ...
  if (currentRevision !== expectedRevision)
    throw new Error('Project settings changed outside this editor. Reload before saving.');
