...
function validate(value: unknown): MemoryPreferences {
  ...
  if (!input || typeof input.autoApprove !== 'boolean')
    throw new Error('Memory settings require autoApprove: true or false.');
  const frameworks = input.frameworks ?? {};
  if (
    typeof frameworks !== 'object' ||
    !frameworks ||
    Array.isArray(frameworks) ||
    Object.keys(frameworks).length > 30
  )
    throw new Error('Invalid framework/version settings.');
  for (const [name, version] of Object.entries(frameworks)) {
    if (
      ...
      !version.trim() ||
      version.length > 100 ||
      Array.from(name + version).some((character) => character.charCodeAt(0) < 32)
    )
      throw new Error('Framework names and versions must be bounded text.');
  }
...
export function readMemorySettings(root: string): MemorySettings {
  return readYamlSettings(root, 'config.memory.yaml', validate, {
    autoApprove: false,
    frameworks: {},
  });
}
