...
import { descendants, identityAlive, sameIdentity, signalOwned, snapshot, type ProcessIdentity } from './process-identity.js';
...
export class ProcessTracker {
  ...
  uncertain = false;
  constructor(readonly binary: string, readonly journal: Journal, root: ProcessIdentity) { this.owned.set(root.pid, root); }
  ...
  scan(): ProcessIdentity[] {
    try {
      ...
      this.uncertain = true;
      this.journal.append('process.uncertain', 'Process enumeration failed; exit cannot be certified', {});
  ...
  async terminate(deadline: number): Promise<ProcessIdentity[]> {
    for (const signal of ['SIGTERM', 'SIGKILL'] as const) {
      for (const identity of this.scan().reverse()) {
        ...
        this.journal.append('process.signal', 'Identity-checked termination', { ...identity, signal, outcome });
