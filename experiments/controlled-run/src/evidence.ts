...
export class Journal {
  ...
  constructor(
    ...
    for (const line of text.split('\n').filter(Boolean)) {
      try {
        ...
        if (
          event.sequence !== this.records.length + 1 ||
        ...
        if (
          ...
          (!Number.isFinite(event.details.milliseconds) || Number(event.details.milliseconds) <= 0)
        ) {
          throw new Error();
        }
        if (
          event.type === 'turn.limit' &&
          (!Number.isInteger(event.details.limit) ||
            Number(event.details.limit) < 1 ||
            Number(event.details.limit) > 8)
        )
          throw new Error();
        this.records.push(event);
      } catch {
        throw new Error('Corrupt journal: no automatic repair');
      }
    }
  ...
  reserveTurn(): void {
    const limit = Math.min(
      8,
      ...this.records
        .filter((event) => event.type === 'turn.limit')
        .map((event) => Number(event.details.limit)),
    );
    if (this.records.filter((event) => event.type === 'turn.reserved').length >= limit)
