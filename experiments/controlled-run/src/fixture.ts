...
const safeRoot = (root: string, evidenceDir: string): { root: string; evidence: string } => {
  if (!isAbsolute(root) || !isAbsolute(evidenceDir))
    throw new Error('fixture paths must be absolute');
  ...
  if (tempRoots.some((temp) => inside(temp, realRoot)))
    throw new Error('fixture root may not be under an OS temporary directory');
  if (inside(realRoot, cwd) || inside(cwd, realRoot))
    throw new Error('fixture root may not contain or be inside the current repository');
  if (inside(realRoot, evidence) || inside(evidence, realRoot))
    throw new Error('evidence must be outside the fixture');
  if (containingRepository(realRoot) || containingRepository(evidence))
    throw new Error('fixture and evidence must be outside existing repositories');
  if (existsSync(evidenceDir) && lstatSync(evidenceDir).isSymbolicLink())
...
export async function createFixture(root: string, evidenceDir: string): Promise<Fixture> {
  ...
  try {
    ...
    writeFileSync(
      join(repo, 'src', 'filter.mjs'),
      'export function searchFilter(items, query) {\n  if (!query.trim()) return [];\n  return items.filter((item) => item.includes(query));\n}\n',
    );
    writeFileSync(
      join(repo, 'test', 'filter.test.mjs'),
...
export async function removeFixture(fixture: Fixture): Promise<void> {
  ...
  if (!record || record.fixture !== fixture)
    throw new Error('fixture is not owned by this process');
  const marker = join(fixture.root, markerName);
  if (
    !existsSync(marker) ||
    JSON.parse(readFileSync(marker, 'utf8')).token !== record.token ||
    lstatSync(fixture.root).isSymbolicLink()
  )
    throw new Error('fixture ownership marker mismatch');
  if (
    JSON.stringify(identity(fixture.root)) !== JSON.stringify(record.root) ||
    JSON.stringify(identity(fixture.remote)) !== JSON.stringify(record.remote)
  )
...
export function observeFixture(fixture: Fixture): {
  workHead: string;
  parentHead: string;
  remoteRefs: string;
} {
