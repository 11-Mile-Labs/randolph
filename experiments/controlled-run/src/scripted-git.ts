...
export function matchesApproval(params: Json, call: ScriptedCall): boolean {
  try {
    return (
      params.itemId === call.id &&
      matchesCommand(params.command, call.command) &&
...
export async function runScriptedGit(dataDir: string, fixtureRoot: string): Promise<number> {
  ...
  journal.append('experiment.authorization', 'Operator authorized deterministic native Git proof', {
    criterion: 'Scripted model responses; real native tool execution and sandbox',
    modelCalls: 0,
    cases: ['ordinary-write', 'git-denied', 'git-approval-declined', 'git-positive-control'],
    maxCases: 4,
  ...
  try {
    ...
    if (version !== 'codex-cli 0.149.0')
      throw new Error('Native test requires inspected Codex version 0.149.0');
    ...
    let approved = false;
    client = new CodexClient(journal, fixture.worktree, 'codex', {
      home,
    ...
    const isolated =
      validation.matches &&
      thread.modelProvider === 'randolph_fixture' &&
      !existsSync(join(home, 'auth.json'));
    record('isolated-native-session', isolated, {
      version,
      node: process.version,
      platform: process.platform,
      architecture: process.arch,
      provider: thread.modelProvider,
      authFilePresent: existsSync(join(home, 'auth.json')),
    ...
    const cases: ScriptedCall[] = [
      {
        id: 'ordinary-write',
        command: 'printf ordinary > ordinary-scripted.txt',
        cwd: fixture.worktree,
        escalated: false,
      },
    ...
    for (const call of cases) {
      ...
      const events = await client.turn(
        ...
        'mock-model',
        'low',
        'Execute the prepared synthetic fixture case.',
      );
      const terminalTurn = events.find((event) => event.method === 'turn/completed');
      const item = events.find(
        (event) => event.method === 'item/completed' && event.params?.item?.id === call.id,
      )?.params.item;
      const requests = events.filter(
        (event) => 'id' in event && event.method === 'item/commandExecution/requestApproval',
      );
      ...
      const exactItem =
        ...
        matchesCommand(item.command, call.command) &&
        typeof item.cwd === 'string' &&
        realpathSync.native(item.cwd) === realpathSync.native(call.cwd);
      const realExchange = server.complete() && terminalTurn?.params?.turn?.status === 'completed';
      let passed = false;
      if (call.id === 'ordinary-write')
        passed =
          item?.exitCode === 0 &&
          existsSync(join(fixture.worktree, 'ordinary-scripted.txt')) &&
          refsUnchanged;
      if (call.id === 'git-denied')
        passed =
          typeof item?.exitCode === 'number' &&
          item.exitCode !== 0 &&
          /permission denied|operation not permitted/i.test(item.aggregatedOutput ?? '') &&
          String(item.aggregatedOutput).includes(
            join(fixture.repo, '.git', 'worktrees', 'run', 'index.lock'),
          ) &&
          requests.length === 0 &&
          refsUnchanged;
      if (call.id === 'git-approval-declined')
        passed = !!request && !!resolved && item?.status === 'declined' && refsUnchanged;
      if (call.id === 'git-positive')
        passed =
          approved &&
          !!request &&
          !!resolved &&
          item?.exitCode === 0 &&
          after.workHead !== original.workHead &&
          after.parentHead === original.parentHead &&
          after.remoteRefs === original.remoteRefs &&
          git(fixture.worktree, ['rev-parse', 'HEAD^']) === original.workHead &&
          git(fixture.worktree, ['show', '-s', '--format=%s', 'HEAD']) ===
            'direct-permission-scripted';
      record(
        call.id,
        passed && exactItem && realExchange,
        {
          realExchange,
          exactItem,
          command: item?.command ?? null,
          cwd: item?.cwd ?? null,
          nativeItemId: item?.id ?? null,
          nativeType: item?.type ?? null,
          status: item?.status ?? null,
          exitCode: item?.exitCode ?? null,
          output: item?.aggregatedOutput ?? null,
          approvalCount: requests.length,
          approvalMatched: !!request,
          approvalResolved: !!resolved,
          refsBefore: original,
          refsAfter: after,
          approvedPositiveControl: approved,
          toolResponseCount: server.requests.filter((value) => value.callId === call.id).length,
        },
        call.id !== 'git-positive'
          ? !refsUnchanged
          : after.parentHead !== original.parentHead ||
              after.remoteRefs !== original.remoteRefs ||
              (!approved && !refsUnchanged),
      );
    ...
    record(
      'no-authentication',
      !existsSync(join(home, 'auth.json')) &&
        server.requests.every((request) => request.authorizationPresent === false),
      {
        authFilePresent: existsSync(join(home, 'auth.json')),
        responseRequests: server.requests.length,
        inferenceCalls: 0,
        scriptedTurns: journal.records.filter((event) => event.type === 'scripted.turn').length,
      },
    );
    writeFileSync(
      join(dataDir, 'scripted-fixture.json'),
      JSON.stringify(
        {
          cases,
          responseRecipe: 'shell_command followed by final assistant SSE',
          digest: hash(JSON.stringify(cases)),
        },
        null,
        2,
      ) + '\n',
    ...
    record('runner', false, {
      reason:
        error instanceof Error && !('stdout' in error || 'stderr' in error)
          ? error.message
          : 'Native fixture error; raw subprocess output omitted',
    });
    return 2;
  } finally {
    if (client) {
    ...
    if (fixture) {
      try {
        ...
        record('cleanup.fixture', true, { removed: true });
      } catch {
        record('cleanup.fixture', false, { removed: false });
      }
    }
    const required = [
      'isolated-native-session',
      'ordinary-write',
      'git-denied',
      'git-approval-declined',
      'git-positive',
      'no-authentication',
      'cleanup.process-group',
      'cleanup.loopback-server',
      'cleanup.fixture',
    ];
    const verdict = Object.values(checks).some((check) => check.verdict === 'failed')
      ...
      : required.every((name) => checks[name]?.verdict === 'passed')
        ? 'passed'
        : 'unverified';
    journal.append('assessment', 'Deterministic native Git execution result', {
      verdict,
      inferenceCalls: 0,
    });
    writeFileSync(
      ...
      JSON.stringify(
        { verdict, checks, inferenceCalls: 0, elapsedMs: Date.now() - started },
        null,
        2,
      ) + '\n',
      { mode: 0o600 },
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    if (!values.run || !values['data-dir'] || !values['fixture-root'])
      throw new Error('Explicit --run and absolute evidence/fixture directories required');
    process.exitCode = await runScriptedGit(values['data-dir'], values['fixture-root']);
  } catch {
    process.stderr.write('Scripted native test refused or failed.\n');
    process.exitCode = 1;
  }
}
