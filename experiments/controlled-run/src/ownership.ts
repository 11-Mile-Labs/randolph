...
function inventory(directory: string): Record<string, string> {
  ...
  const walk = (relative: string): void => {
    for (const name of readdirSync(join(directory, relative)).sort()) {
      ...
      if (lstatSync(path).isDirectory()) { files[key] = id; walk(key); }
...
export class EvidenceOwner {
  private constructor(readonly directory: string, private readonly token: string, private readonly root: string) {}
  ...
  static open(directory: string, corrected: boolean): EvidenceOwner {
    if (!corrected) {
      ...
      const owner = new EvidenceOwner(realpathSync.native(directory), randomUUID(), identity(directory));
      writeFileSync(join(directory, marker), JSON.stringify({ token: owner.token, root: owner.root, sealed: false }), { flag: 'wx', mode: 0o600 });
      return owner;
    }
    const root = identity(directory);
    if (!lstatSync(directory).isDirectory() || (lstatSync(directory).mode & 0o777) !== 0o700) throw new Error('Evidence directory mode or type changed');
    identity(join(directory, marker));
    const saved = JSON.parse(readFileSync(join(directory, marker), 'utf8'));
    if (saved.root !== root || typeof saved.token !== 'string' || !saved.sealed ||
      JSON.stringify(saved.files) !== JSON.stringify(inventory(directory))) throw new Error('Evidence ownership or contents changed');
    return new EvidenceOwner(realpathSync.native(directory), saved.token, root);
  }

  seal(): void {
    if (identity(this.directory) !== this.root) throw new Error('Evidence directory identity changed');
    identity(join(this.directory, marker));
    const saved = JSON.parse(readFileSync(join(this.directory, marker), 'utf8'));
    if (saved.token !== this.token || saved.root !== this.root) throw new Error('Evidence ownership changed');
    writeFileSync(join(this.directory, marker), JSON.stringify({ token: this.token, root: this.root, sealed: true, files: inventory(this.directory) }), { mode: 0o600 });
  }
}
