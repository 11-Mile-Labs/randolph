...
export type ProcessIdentity = { pid: number; ppid: number; pgid: number; uid: number; start: string; zombie: boolean };
...
const parse = (output: string): ProcessIdentity[] => {
  ...
  return value.map((entry) => {
    ...
    if (!["pid", "ppid", "pgid", "uid"].every((key) => Number.isInteger(item[key])) || typeof item.start !== "string" || typeof item.zombie !== "boolean") throw new Error("invalid process identity");
    return { pid: item.pid as number, ppid: item.ppid as number, pgid: item.pgid as number, uid: item.uid as number, start: item.start, zombie: item.zombie };
  });
};
...
export function compileSnapshot(outputAbsolutePath: string): string {
  ...
  execFileSync("clang", ["-O2", "-Wall", "-Wextra", "-Werror", source, "-o", outputAbsolutePath], { timeout: 10000, stdio: "pipe" });
...
export function snapshot(binary: string, pids?: number[]): ProcessIdentity[] {
  try {
    return parse(execFileSync(binary, pids?.map(String) ?? [], { encoding: "utf8", timeout: 5000, stdio: ["ignore", "pipe", "pipe"] }));
  } catch (error) {
    throw new Error(`process enumeration failed: ${error instanceof Error ? error.message : "unknown error"}`);
  }
}
...
export function descendants(processes: ProcessIdentity[], roots: ProcessIdentity[]): ProcessIdentity[] {
  const validRoots = roots.filter((root) => processes.some((process) => sameIdentity(process, root)));
  ...
  for (let cursor = 0; cursor < result.length; cursor++) {
    ...
    for (const process of processes) {
      if (process.ppid === parent.pid && !seen.has(process.pid)) { seen.add(process.pid); result.push(process); }
    }
  }
  return result;
}

export function signalOwned(identity: ProcessIdentity, signal: NodeJS.Signals, binary: string): SignalResult {
  const current = snapshot(binary, [identity.pid]).find((process) => process.pid === identity.pid);
  if (!current) { identityAlive([], identity); return "gone"; }
  if (!sameIdentity(current, identity)) return "identity-changed";
  try { process.kill(identity.pid, signal); return "signalled"; } catch (error) {
