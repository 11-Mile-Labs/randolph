...
const live = (binary: string, identity: ProcessIdentity): boolean =>
  identityAlive(snapshot(binary, [identity.pid]), identity);
...
async function startingIdentity(binary: string, pid: number | undefined): Promise<ProcessIdentity> {
  ...
  await until(() => {
    try {
      identity = snapshot(binary, [pid]).find((item) => item.pid === pid && !item.zombie);
    } catch {
      return false;
    }
    return !!identity;
...
export async function runLifecycleProbe(dataDir: string, fixtureRoot: string): Promise<number> {
  ...
  journal.append(
    ...
    'Operator authorized bounded Stop and owner-loss native experiment',
    {
      cases: ['stop', 'controller-death', 'harness-death', 'detached'],
      inferenceCalls: 0,
      candidate:
        'Per-run supervisor, controller IPC EOF, retained process-tree snapshots with native start identities',
      limitation:
        'Polling can miss rapid reparenting; observer registration and rescue do not count as containment',
      deadlineMs: 5000,
      perCaseWatchdogMs: 15000,
    },
  );
  try {
    ...
    for (const name of ['stop', 'controller-death', 'harness-death', 'detached']) {
      ...
      server.prepare(call);
      const messages: Json[] = [];
      controller = fork(fileURLToPath(new URL('./lifecycle-controller.js', import.meta.url)), [], {
      ...
      controller.send({
        type: 'start',
        directory: supervisorDir,
        home: join(fixture.root, `home-${name}`),
      ...
      await until(
        () =>
          roles.every((role) => existsSync(join(fixtureDirectory, `${role}.heartbeat`))) &&
          new Journal(supervisorDir).records.some(
            (event) => event.type === 'native.tool-confirmed',
          ),
        5_000,
      );
      for (const role of roles) {
        const receipt = JSON.parse(readFileSync(join(fixtureDirectory, `${role}.json`), 'utf8'));
        if (receipt.token !== token || receipt.role !== role)
          throw new Error('Fixture receipt mismatch');
        const identity = snapshot(binary, [receipt.pid]).find(
          (item) => item.pid === receipt.pid && !item.zombie,
        );
        if (!identity) throw new Error('Fixture actor not alive before fault');
        const argv = execFileSync('/bin/ps', ['-p', String(identity.pid), '-o', 'command='], {
          encoding: 'utf8',
        }).trim();
        if (!argv.endsWith(`lifecycle-tool.mjs ${name} ${token} ${role}`))
          throw new Error('Fixture actor command identity mismatch');
      ...
      caseJournal.append('fixture.active', 'Native-launched fixture actors alive before fault', {
        actors,
        native,
      ...
      caseJournal.append('fault.injected', 'Named fault injected without group termination', {
        name,
        faultTime,
      });
      ...
      const heartbeatSizes = roles.map(
        (role) => readFileSync(join(fixtureDirectory, `${role}.heartbeat`)).length,
      );
      await delay(Math.max(0, 6_500 - (Date.now() - faultTime)));
      const stable = roles.every(
        (role, index) =>
          readFileSync(join(fixtureDirectory, `${role}.heartbeat`)).length ===
          heartbeatSizes[index],
      );
      ...
      const reopen = execFileSync(
        process.execPath,
        [fileURLToPath(new URL('./lifecycle-reopen.js', import.meta.url)), supervisorDir],
        { encoding: 'utf8', timeout: 2_000 },
      ).trim();
      const countAfter = new Journal(supervisorDir).records.filter(
        (event) => event.type === 'scripted.turn',
      ).length;
      const refsUnchanged =
        JSON.stringify(observeFixture(fixture)) === JSON.stringify(originalRefs);
      const supervisorLog = new Journal(supervisorDir);
      const dispatch = supervisorLog.records.find((event) => event.type === 'dispatch.closed');
      const expectedState =
        name === 'controller-death' || name === 'harness-death' ? 'interrupted' : 'stopped';
      const passed =
        !candidate.uncertain &&
        !remaining.length &&
        candidate.elapsedMs <= 5_000 &&
        faultToResultMs <= 5_000 &&
        stable &&
        !lateCanary &&
        state === expectedState &&
        reopen === expectedState &&
        countBefore === 1 &&
        countAfter === 1 &&
        !!dispatch &&
        refsUnchanged &&
        live(binary, sentinelIdentity) &&
        !live(binary, native) &&
        !live(binary, supervisorIdentity) &&
        server.errors.length === 0 &&
        server.requests.length >= 1 &&
        !supervisorLog.records.some((event) => event.type === 'turn.reserved');
      const result = {
        name,
        verdict: passed ? 'passed' : candidate.uncertain ? 'unverified' : 'failed',
        candidate,
        faultToResultMs,
        remainingBeforeRescue: remaining,
        stableAfterTermination: stable,
        lateCanary,
        retainedState: state,
        reopenedState: reopen,
        reopenMs: Date.now() - reopenStart,
        scriptedTurnsBeforeReopen: countBefore,
        scriptedTurnsAfterReopen: countAfter,
        refsUnchanged,
        unrelatedSentinelAlive: live(binary, sentinelIdentity),
        nativeExited: !live(binary, native),
        dispatchClosed: !!dispatch,
        dispatchClosedMs: dispatch ? Date.parse(dispatch.time) - faultTime : null,
        localResponses: server.requests.length,
        modelCalls: 0,
      };
      results.push(result);
      caseJournal.append(
        'case.verdict',
        'Candidate verdict recorded before independent watchdog rescue',
        result,
      );
      process.stdout.write(
        `${name}: ${result.verdict}; shutdown ${candidate.elapsedMs}ms; escaped actors ${remaining.length}\n`,
      );
      for (const identity of remaining) signalOwned(identity, 'SIGKILL', binary);
      caseJournal.append('watchdog.rescue', 'Test-only rescue cannot turn failure into pass', {
        rescued: remaining,
      });
    ...
    failure =
      error instanceof Error && !('stdout' in error)
        ? error.message
        : 'Lifecycle probe failed; raw subprocess output omitted';
    ...
    if (binary) {
      ...
      try {
        ...
        for (const identity of descendants(current, [...rescue.values()]))
          rescue.set(identity.pid, identity);
      } catch {
        enumerationCertain = false;
      }
      for (const identity of [...rescue.values()].reverse()) {
        try {
          signalOwned(identity, 'SIGKILL', binary);
        } catch {
          enumerationCertain = false;
        }
      }
      ...
      try {
        cleanupPassed =
          [...rescue.values()].every((identity) => !live(binary, identity)) && enumerationCertain;
      } catch {
        cleanupPassed = false;
      }
    }
    ...
    journal.append('cleanup', 'Observed process cleanup and fixture retention', {
      cleanupPassed,
      fixtureRemoved: !!fixture && cleanupPassed,
    });
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    if (!values.run || !values['data-dir'] || !values['fixture-root'])
      throw new Error('Explicit --run and absolute directories required');
    process.exitCode = await runLifecycleProbe(values['data-dir'], values['fixture-root']);
  } catch {
    process.stderr.write('Lifecycle probe refused or failed.\n');
    process.exitCode = 1;
  }
}
