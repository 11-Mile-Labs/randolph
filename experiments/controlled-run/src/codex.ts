...
export type ScriptedTestOptions = {
  home: string;
  endpoint: string;
  commandDecision?: (params: Json) => 'accept' | 'decline';
};

export function launchArguments(
  worktree: string,
  mcpNames: string[],
  scripted?: ScriptedTestOptions,
): string[] {
  ...
  const settings = [
    ...(scripted
      ? [
          'model_provider="randolph_fixture"',
          ...
          'model_providers.randolph_fixture.requires_openai_auth=false',
          'model_providers.randolph_fixture.supports_websockets=false',
          'model_providers.randolph_fixture.request_max_retries=0',
          'model_providers.randolph_fixture.stream_max_retries=0',
  ...
  for (const name of mcpNames) {
    if (!/^[A-Za-z0-9_-]+$/.test(name))
      throw new Error('Unsupported MCP key: explicit adapter needed');
...
export class CodexClient {
  ...
  constructor(
    ...
    readonly scripted?: ScriptedTestOptions,
  ) {}
  ...
  async start(): Promise<void> {
    ...
    if (this.scripted) {
      ...
      if (
        ...
        endpoint.username ||
        endpoint.password ||
        resolve(nativeHome) === resolve(join(process.env.HOME ?? homedir(), '.codex'))
      )
        throw new Error('Scripted tests require an isolated home and loopback endpoint');
    }
    for (const name of ['AGENTS.override.md', 'AGENTS.md']) {
      ...
      if (content.trim()) {
        this.instructionInventory.push({ path, digest: hash(content) });
        break;
      }
    }
    ...
    try {
      ...
      throw new Error('Native configuration could not be parsed; contents omitted');
    }
    ...
    const ruleDigest = hash(
      ruleFiles.map((name) => readFileSync(join(rulesPath, name), 'utf8')).join('\n'),
    );
    this.journal.append(
      'native.configuration',
      'Native extras disabled; existing rules inventoried',
      {
        ...
        mcpServersDisabled: servers.length,
    ...
    if (this.scripted) {
      ...
      env.XDG_CONFIG_HOME = join(nativeHome, 'xdg');
    }
    ...
    await this.rpc('initialize', {
      clientInfo: { name: 'randolph_controlled_run', version: '0.0.0' },
  ...
  private receive(line: string): void {
    ...
    try {
      ...
      this.failPending(new Error('Invalid native JSON'));
      return;
    }
    if (message.method) {
      ...
      this.journal.append(
        'native.event',
        `${message.method}${item.type ? ': ' + item.type : ''}`,
        details,
      );
      ...
      if ('id' in message) {
        ...
        const commandDecision =
          !this.dispatchClosed && message.method === 'item/commandExecution/requestApproval'
            ? (this.scripted?.commandDecision?.(message.params ?? {}) ?? 'decline')
            : 'decline';
        this.journal.append('native.approval-decision', 'Controlled test resolves native request', {
          method: message.method,
          requestDigest: hash(JSON.stringify(message.id)),
          itemDigest: hash(message.params?.itemId ?? ''),
          commandDigest: message.params?.command ? hash(message.params.command) : null,
          decision:
            message.method === 'item/permissions/requestApproval' ? 'empty-grant' : commandDecision,
        });
        if (message.method === 'item/permissions/requestApproval') {
          ...
          this.send({
            id: message.id,
            error: { code: -32601, message: 'Unsupported request denied by experiment' },
          });
      }
    } else {
      ...
      if (message.error)
        pending.reject(new Error(`Native RPC error ${message.error.code ?? 'unknown'}`));
  ...
  rpc(method: string, params: Json, timeoutMs = 20_000): Promise<Json> {
    if (this.dispatchClosed && ['thread/start', 'turn/start'].includes(method))
      return Promise.reject(new Error('Dispatch is closed'));
    const id = ++this.sequence;
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id);
        reject(new Error(`Native RPC timed out: ${method}`));
      }, timeoutMs);
  ...
  async startThread(model: string): Promise<Json> {
    return this.rpc('thread/start', {
      model,
      modelProvider: this.scripted ? 'randolph_fixture' : 'openai',
      cwd: this.worktree,
  ...
  async turn(threadId: string, model: string, effort: string, prompt: string): Promise<Json[]> {
    for (const source of this.instructionInventory) {
      if (hash(readFileSync(source.path, 'utf8')) !== source.digest)
        throw new Error('Global instruction source changed during the experiment');
    }
    if (this.scripted)
      this.journal.append('scripted.turn', 'No model inference: local response fixture', { model });
    ...
    const turn = await this.rpc('turn/start', {
      ...
      approvalPolicy: 'on-request',
      approvalsReviewer: 'user',
      sandboxPolicy: workspacePolicy(this.worktree),
      input: [{ type: 'text', text: prompt }],
    });
    let reportAt = Date.now();
    while (Date.now() - started < 60_000) {
      const done = this.notifications
        .slice(offset)
        .find((event) => event.method === 'turn/completed');
      if (done) {
        this.journal.append('turn.completed', 'Native turn settled', {
      ...
      if (this.process?.exitCode !== null || this.process?.signalCode !== null)
        throw new Error('Harness exited during turn');
      if (Date.now() - reportAt >= 5_000) {
        process.stdout.write(
          `Native process live; last reported event ${Math.round((Date.now() - this.lastActivity) / 1000)}s ago\n`,
        );
        reportAt = Date.now();
      }
      await delay(40);
