...
export type CheckpointInput = {
  ...
  metadata: Record<string, unknown>;
  journal: Journal;
  fault?: 'after-contents' | 'before-manifest' | 'after-manifest';
};
...
function realDirectories(path: string): void {
  ...
  for (const part of resolve(path).split('/').filter(Boolean)) {
    ...
    if (stat && (!stat.isDirectory() || stat.isSymbolicLink()))
      throw new Error('Path parent must be a real directory');
  }
}
function validPath(path: unknown): asserts path is string {
  if (
    ...
    path.split('/').some((part) => ['..', '.', '.git', '.worktrees'].includes(part))
  )
...
function regularBytes(path: string): Buffer {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1)
    throw new Error('Checkpoint content must be a regular unlinked file');
...
function inventory(
  worktree: string,
  save?: (hash: string, bytes: Buffer) => void,
): CheckpointEntry[] {
  const entries: CheckpointEntry[] = [];
  const names = [
    ...new Set(
      gitNames(worktree, ['ls-files', '-z', '--cached', '--others', '--exclude-standard']),
    ),
  ].sort();
  for (const path of names) {
    ...
    if (stat.isSymbolicLink())
      entries.push({ path, kind: 'symlink', mode: 0o777, target: readlinkSync(absolute) });
    else if (stat.isFile()) {
...
export async function captureCheckpoint(input: CheckpointInput): Promise<CheckpointManifest> {
  if (!isAbsolute(input.worktree) || !isAbsolute(input.checkpointDir))
    throw new Error('Absolute checkpoint paths required');
  ...
  if (
    ...
    !relative(dir, root).startsWith('..')
  )
    throw new Error('Checkpoint must be a new directory outside source');
  ...
  const entries = inventory(root, (hash, bytes) => {
    ...
    if (statOrMissing(blob)) {
      if (digest(regularBytes(blob)) !== hash) throw new Error('Conflicting checkpoint blob');
  ...
  try {
    ...
    rmSync(objectProof, { recursive: true, force: true });
  }
  syncPath(bundle);
  syncPath(join(dir, 'blobs'));
  if (
    git(root, ['rev-parse', 'HEAD']) !== initialHead ||
    JSON.stringify(inventory(root)) !== JSON.stringify(entries)
  )
    throw new Error('Source changed during checkpoint capture');
  const body = {
    version: 1 as const,
  ...
  input.journal.append('checkpoint.recoverable', 'Checkpoint content and manifest are durable', {
    checkpointDir: dir,
    manifestDigest,
  });
...
export async function verifyCheckpoint(
  ...
  const manifestDigest = digest(raw);
  const event = journal.records.find(
    (record) =>
      record.type === 'checkpoint.recoverable' &&
      record.details.checkpointDir === dir &&
      record.details.manifestDigest === manifestDigest,
  );
  if (!event) throw new Error('Checkpoint is not recoverable');
  const parsed = JSON.parse(raw.toString('utf8')) as CheckpointManifest;
  if (
    parsed.version !== 1 ||
    !/^(?:[0-9a-f]{40}|[0-9a-f]{64})$/.test(parsed.baseOid) ||
    typeof parsed.baseRef !== 'string' ||
    !/^[0-9a-f]{64}$/.test(parsed.bundleDigest) ||
    !Array.isArray(parsed.entries) ||
    !Array.isArray(parsed.deleted) ||
    !parsed.metadata ||
    typeof parsed.metadata !== 'object' ||
    Array.isArray(parsed.metadata) ||
    digest(regularBytes(join(dir, 'base.bundle'))) !== parsed.bundleDigest
  )
  ...
  for (const entry of parsed.entries) {
    ...
    if (!Number.isInteger(entry.mode) || entry.mode < 0 || entry.mode > 0o777)
      throw new Error('Invalid checkpoint mode');
    if (entry.kind === 'file') {
      if (
      ...
      if (digest(bytes) !== entry.digest || bytes.length !== entry.size)
        throw new Error('Corrupt checkpoint blob');
    } else if (
      entry.kind !== 'symlink' ||
...
export async function restoreCheckpoint(
  ...
  destination: string,
  journal: Journal,
): Promise<{ repo: string; worktree: string; manifest: CheckpointManifest }> {
  ...
  try {
    ...
    for (const entry of manifest.entries.filter((item) => item.kind === 'file')) {
      ...
      writeFileSync(path, regularBytes(join(checkpointDir, 'blobs', entry.digest!)), {
        flag: 'wx',
        mode: entry.mode,
      });
    ...
    if (JSON.stringify(inventory(worktree)) !== JSON.stringify(manifest.entries))
      throw new Error('Restored content does not match checkpoint');
    syncPath(worktree);
