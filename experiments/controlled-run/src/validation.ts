...
export function validateEffectivePolicy(thread: Json, worktree: string, version?: string, expectedInstructions: { path: string; digest: string }[] = []): { matches: boolean; evidence: Json } {
  ...
  const rootsMatch = Array.isArray(roots) && ((roots.length === 0 && knownImplicitRoots) ||
    roots.length === 1 && canonical(roots[0]) === expected);
  const runtimeRoots = thread.runtimeWorkspaceRoots;
  const runtimeRootsMatch = Array.isArray(runtimeRoots) && runtimeRoots.length === 1 && canonical(runtimeRoots[0]) === expected;
  const instructionsMatch = Array.isArray(sources) && sources.length === expectedInstructions.length &&
    sources.every((source: unknown, index: number) => {
      ...
      try { return hash(readFileSync(expectedSource.path, 'utf8')) === expectedSource.digest; } catch { return false; }
    });
  const matches = expected !== null && cwd === expected && rootsMatch && runtimeRootsMatch && instructionsMatch &&
  ...
  return { matches, evidence: { type: policy?.type, networkAccess: policy?.networkAccess,
    ...
    rootSemantics: knownImplicitRoots ? 'v0.149.0-project-roots-plus-additional' : 'explicit-only',
    runtimeRootsMatch, runtimeRootCount: Array.isArray(runtimeRoots) ? runtimeRoots.length : null,
    ...
    instructionMode: expectedInstructions.length ? 'inventoried-global-present' : 'no-file-instructions',
