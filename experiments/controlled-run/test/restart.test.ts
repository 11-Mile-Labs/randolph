...
test('Restart requires an exact human decision, including conversation and checkpoint identity', () => {
  ...
  for (const changed of [
    ...
    assert.throws(() =>
      requireRestartApproval(current, { ...valid, ...changed } as RestartApproval),
    );
  }
...
test('explicit Restart restores once into a linked run; read-only reopen and late events never dispatch', async () => {
  ...
  source.append('lifecycle.state', 'Interrupted old work', {
    state: 'interrupted',
    runId: previousRunId,
  });
  await captureCheckpoint({
    ...
    checkpointDir,
    baseRef: 'HEAD',
    metadata: { runId: previousRunId, conversationId, decisions: ['retain'], pending: ['verify'] },
    journal: source,
  });
  ...
  const options = {
    ...
    previousRunId,
    conversationId,
    launch: async (restored: { worktree: string }) => {
      assert.ok(existsSync(join(restored.worktree, 'src/filter.mjs')));
      calls++;
    },
  };
  ...
  const approval: RestartApproval = {
    action: 'restart',
    source: 'human',
    decision: 'approved',
    previousRunId,
    conversationId,
    checkpointDigest: hash(readFileSync(join(checkpointDir, 'manifest.json'), 'utf8')),
  };
  ...
  assert.throws(
    () => appendRunEvent(next, runId, { runId: previousRunId, type: 'completion', details: {} }),
    /different run/,
  );
  await assert.rejects(
    () => restartFromCheckpoint({ ...options, next: new Journal(next.directory), approval }),
    /already used/,
  );
  assert.equal(calls, 1);
});
