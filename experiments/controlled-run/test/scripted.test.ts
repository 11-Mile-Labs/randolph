...
test('scripted response requests the real native shell tool with exact arguments', () => {
  const events = scriptedToolResponse(call)
    .split('\n')
  ...
  assert.deepEqual(JSON.parse(item.arguments), {
...
test('positive control approval requires exact call, command and worktree', () => {
  ...
  for (const change of [
    { itemId: 'other' },
    { command: call.command + '; touch other' },
    { cwd: '/' },
  ...
  assert.equal(
    matchesCommand(`/bin/zsh -lc echo misleading; '${call.command}'`, call.command),
    false,
  );
});
test('scripted launch uses a distinct no-auth provider with no retry or subscription fallback', () => {
  const args = launchArguments('/fixture/work', [], {
    home: '/fixture/home',
...
test('loopback fixture requires the correlated tool output before returning a final response', async () => {
  ...
  try {
    ...
    const post = async (input: unknown[]) =>
      fetch(server.endpoint + '/responses', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ model: 'mock-model', input }),
      });
    ...
    const second = await post([
      { type: 'function_call_output', call_id: call.id, output: 'real tool receipt' },
    ]);
...
test('loopback fixture rejects missing results and authentication headers', async () => {
  ...
  try {
    ...
    const first = await fetch(server.endpoint + '/responses', request);
    await first.text();
    const absent = await fetch(server.endpoint + '/responses', request);
    await absent.text();
    assert.equal(absent.status, 400);
    const auth = await fetch(server.endpoint + '/responses', {
      ...request,
      headers: { authorization: 'synthetic-rejected' },
    });
    await auth.text();
