...
test('child environment excludes inherited control, credentials, shell injection, and Git overrides', () => {
  const env = cleanEnvironment({ PATH: '/bin', HOME: '/example', CODEX_APP_TOOLS_PIPE_PATH: 'private',
    CODEX_PERMISSION_PROFILE: 'broad', OPENAI_API_KEY: 'private', ANTHROPIC_API_KEY: 'private',
    OPENAI_BASE_URL: 'private', NODE_OPTIONS: '--require /private', BASH_ENV: '/private',
    GIT_DIR: '/private', GIT_CONFIG_COUNT: '1', HTTPS_PROXY: 'private' });
  assert.equal(env.PATH, '/bin');
  assert.equal(env.HOME, '/example');
  for (const key of ['CODEX_APP_TOOLS_PIPE_PATH', 'CODEX_PERMISSION_PROFILE', 'OPENAI_API_KEY',
    'ANTHROPIC_API_KEY', 'OPENAI_BASE_URL', 'NODE_OPTIONS', 'BASH_ENV', 'GIT_DIR', 'GIT_CONFIG_COUNT', 'HTTPS_PROXY']) {
...
test('launch and turn policies exclude broad temp roots and automatic approval', () => {
  ...
  assert.deepEqual(workspacePolicy('/fixture/work'), { type: 'workspaceWrite',
    writableRoots: ['/fixture/work'], networkAccess: false,
