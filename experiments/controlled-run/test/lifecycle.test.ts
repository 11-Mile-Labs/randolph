...
test('Stop closes effectful dispatch and declines late native approval without invoking the allow callback', async () => {
  ...
  const client = new CodexClient(journal, directory, 'codex', {
    home: directory,
  ...
  Reflect.get(client, 'receive').call(
    client,
    JSON.stringify({
      id: 8,
      method: 'item/commandExecution/requestApproval',
      params: { itemId: 'late', command: 'late command' },
    }),
  );
...
test('fresh process reopen maps unfinished execution to interrupted and adds no events', () => {
  ...
  const output = execFileSync(
    process.execPath,
    [fileURLToPath(new URL('../src/lifecycle-reopen.js', import.meta.url)), journal.directory],
    { encoding: 'utf8' },
  );
...
test('enumeration failure cannot certify an empty owned tree', () => {
  ...
  const tracker = new ProcessTracker(binary, journal, {
    pid: 123,
    ppid: 1,
    pgid: 123,
    uid: 501,
...
test('controller announces readiness before any supervisor or native dispatch', async () => {
  const controller = fork(
    fileURLToPath(new URL('../src/lifecycle-controller.js', import.meta.url)),
    [],
