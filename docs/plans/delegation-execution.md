...
Codex application tools have a version-gated callback route, negative protocol tests, and a real production-adapter round trip with confirmed owned-process cleanup. The broker derives authority scope from application registration, records proposals without authorization or worker launch, and returns bounded same-run task summaries. Its current task-read contract does not yet expose result artifacts. Normal runtime dispatch remains unchanged: session registration, post-turn source/context capture, current capability and control-generation validation, proposal cards, presets, scheduler admission, and delegated native acceptance are still pending. Storage methods are not a substitute for scheduler admission. The checklist below tracks the complete group rather than treating these prerequisites as finished delegation.
