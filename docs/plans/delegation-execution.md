...
A shared native-session capacity registry covers main, worker, integration, verification, review, and synthesis reservations. Worker/review reservations require their authorized parallel limit. Writer ownership checks both canonical path and directory device/inode so symlinks, renames, and replacements cannot evade an existing reservation. Ordinary discovery, main/setup turns, and review checks now use the shared runtime admission service. Quarantined native occupancy is reconstructed before new work. Delegated discovery, model turns, and checks now enter that same service separately; source preparation runs without native capacity and source identity is rechecked after the model or command queue wait. Runtime-only writer reservations now have a SQLite-backed service shared by ordinary run preparation/execution/checkpoints, verification, delivery, integration, and push.
...
Task attempts now have a durable claim before preparation, exact prepared workspace/source binding, separate native dispatch and task completion, terminal dependency reasons, and one-shot preparation/publication receipts. A completed native turn cannot by itself establish a successful source-producing task. Pause permits an active turn to settle; an explicitly resumed runtime stage can publish its retained output without rerunning that turn. Reopen preserves confirmed native cleanup while requiring explicit recovery for unfinished task processing.

The prepared-task native executor connects those records to a priority queue, shared capacity, exact workspace leases, charged discovery and model execution, frozen native selection, cancellation, and bounded separate worker output. Its durable admission claim prevents another executor from settling or launching the same session. Discovery explicitly reports cleanup; missing evidence quarantines occupancy. Late callbacks cannot mutate completed evidence, and source changes during discovery prevent dispatch. Runtime-only workspaces have a separate lease registry so preparation and capture need not consume native session capacity.

A synthetic two-task proof used the production executor and Codex CLI 0.154.0 through the existing subscription: Luna at low effort completed a read-only worker, runtime published its immutable source, and a new session of the same selected main model synthesized from that source and retained result. Both real native identities and owned-process cleanup were retained, all four native process groups exited, source trees remained unchanged, and reopening launched nothing. The fixture explicitly constructed its authorization and orchestration; it does not establish the desktop proposal-to-approval path, automatic graph scheduler, Code integration/check phases, other harnesses, or owner-loss cleanup. Those remain required before capability enablement.

The next implementation connects a graph coordinator to source preparation, native tasks, explicit conversation-worktree integration, native project checks, review, and main synthesis. A durable coordinator claim prevents duplicate owners, and retained context is checked against the authorized basis. Scripted complete-graph acceptance covers two independent writers, integration, two project checks, review, and read-only synthesis with unchanged parent refs/index. Separate regressions cover duplicate ownership, Pause between checks, budget exhaustion after preparation or applied integration, priority changes before dispatch, and failed raw integration retaining its writer lease.

Multiwriter integration derives each delta from retained source/output snapshots, keeps the declared target distinct from the conversation before-image, and uses isolated Git three-way merges. Before applying raw files, it verifies the authorized conversation basis, candidate evidence, and every destination ancestor. Conflicts retain the processed and pending writer frontier and currently stop visibly; conflict resolution and repair generations remain unfinished. Applying files preserves the live Git index and refs. Only an applied candidate can become the native main integration session's input.

Verification now records one immutable command manifest and distinct command identity/session per check. Commands use the exact authorized harness's verified lifecycle capability without requesting a model turn. Source hashing, manifest detection, discovery, and command execution are charged; capacity waits are excluded. Each check requires current authority, its exact input tree, exit-zero success, and confirmed cleanup. Checked output publication requires every check and exactly the checked input tree. Passed checks survive Pause/Resume, while unknown cleanup quarantines capacity and writer ownership. Startup reconciliation launches nothing.

A native synthetic Code graph then used the production coordinator with Codex CLI 0.154.0 and Luna at low effort through the existing subscription. Two workers wrote separate files, the selected main integrated them in the conversation worktree, two native commands asserted the resulting content, a read-only reviewer inspected the checked tree, and the selected main synthesized the result. All six tasks completed with seven distinct native sessions, confirmed cleanup for fourteen process groups, an unchanged parent repository, and no launch on reopen. Runtime retained 42.838 seconds of union active time. The test supplied explicit synthetic authorization; it does not establish desktop proposal approval or app-wide concurrency acceptance. Initial fixture failures retained honest failure receipts: a package-manager download attempt was denied, generated cache content invalidated checks until declared before the basis, and an ambiguous expected-content instruction was corrected and enforced by assertions. No native boundary was relaxed.

A subsequent native Code graph used the shared admission service with app and per-harness capacity both limited to one, plus SQLite-backed workspace ownership. All six tasks, seven sessions, fourteen native operations, and fourteen process groups completed cleanly; all six workspace reservations retained terminal release evidence. The parent remained unchanged and reopening launched nothing. Runtime retained 52.773 seconds of union active time. This establishes the bounded native integration at capacity one, while full desktop authorization and remaining recovery controls remain separate acceptance requirements.

These components remain behind the production execution gate. Ordinary main/setup/review/discovery admission now shares one runtime capacity service with durable intent, cleanup settlement, and quarantine reconstruction. The delegated executor now joins that exact service, including centralized deadlines that cannot be reversed by late native results. The coordinator accepts the same durable workspace ownership service used in its complete scripted graph proofs. Terminal reservation IDs retain their cleanup evidence and cannot be replayed; explicit reopen reconciliation quarantines unfinished runtime ownership without inspecting old paths or launching work. Ordinary main/setup/check/delivery/push operations now use that authority. Existing root-folder native read-only runs have independent shared read reservations; every writer remains exclusive, and any uncertain reader blocks new access. These live-root reads are not delegated immutable sources. Read-only planning holds a durable reader before Git probing and transactionally replaces it with a fresh exclusive reservation before worktree mutation. Operations retain ownership with parent-to-worktree ownership overlap during preparation, checkpoint export reservations, exact guards after queue waits, and cleanup evidence retained through settlement. Legacy unfinished writers are reconstructed without looking up old paths; each original uncertain witness blocks admission independently. Uncertain process cleanup remains uncertain if a later evidence write also fails. Proposals/presets, visible run controls, result-query surfaces, repair generations, broader explicit owner-loss recovery, and installed desktop end-to-end delegation acceptance remain required before enablement.

## Implementation and acceptance sequence

Schema 5 retains app-level native operations without fabricating runs for catalog discovery. Executable inventory is filesystem-only in both adapters; Grok discovery supports cancellation and explicit owned-process cleanup evidence while Code execution remains disabled. Capacity waits do not launch a process. Frozen inputs and synchronous authority checks are retained across the wait, and review commands recheck their exact tree and directory identity before dispatch. Stop-all and close include pre-run discovery and await its settlement before SQLite closes. Failed cleanup persistence retains capacity; clean discovery cannot reconcile a later unknown turn. Reopen reconstructs conservative legacy occupancy, preserves known no-launch evidence, and never resumes work. The existing explicit later-boot setup reconciliation also settles its corresponding capacity records.
