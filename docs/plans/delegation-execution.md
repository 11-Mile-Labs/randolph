...
A shared native-session capacity registry covers main, worker, integration, verification, review, and synthesis reservations. Worker/review reservations require their authorized parallel limit. Writer ownership checks both canonical path and directory device/inode so symlinks, renames, and replacements cannot evade an existing reservation. Ordinary discovery, main/setup turns, and review checks now use the shared runtime admission service. Quarantined native occupancy is reconstructed before new work; delegated executor wiring and durable runtime-only writer reservations remain scheduler work.
...
These components remain behind the production execution gate. Ordinary main/setup/review/discovery admission now shares one runtime capacity service with durable intent, cleanup settlement, and quarantine reconstruction. The delegated executor must still join that exact service, and durable workspace ownership, proposals/presets, visible run controls, result-query surfaces, repair generations, and installed desktop end-to-end delegation acceptance remain required before enablement.
