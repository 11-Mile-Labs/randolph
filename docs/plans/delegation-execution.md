...
A subsequent native Code graph used the shared admission service with app and per-harness capacity both limited to one, plus SQLite-backed workspace ownership. All six tasks, seven sessions, fourteen native operations, and fourteen process groups completed cleanly; all six workspace reservations retained terminal release evidence. The parent remained unchanged and reopening launched nothing. Runtime retained 52.773 seconds of union active time. This establishes the bounded native integration at capacity one, while ordinary workspace lifecycle wiring and desktop authorization remain separate acceptance requirements.

These components remain behind the production execution gate. Ordinary main/setup/review/discovery admission now shares one runtime capacity service with durable intent, cleanup settlement, and quarantine reconstruction. The delegated executor now joins that exact service, including centralized deadlines that cannot be reversed by late native results. The coordinator accepts the same durable workspace ownership service used in its complete scripted graph proofs. Terminal reservation IDs retain their cleanup evidence and cannot be replayed; explicit reopen reconciliation quarantines unfinished runtime ownership without inspecting old paths or launching work. Ordinary main/setup/check/delivery/push ownership wiring, proposals/presets, visible run controls, result-query surfaces, repair generations, and installed desktop end-to-end delegation acceptance remain required before enablement.
