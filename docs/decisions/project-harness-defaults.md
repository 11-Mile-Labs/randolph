...
At dispatch, the runtime reads current project settings and resolves the conversation override, project default, or initial native choice in that order. Switching a conversation to Code mode already runs a full discovery of the selected CLI; the next send in that conversation within thirty seconds reuses that discovery once instead of starting the CLI again, provided the project, harness, executable, and settings revision are unchanged and the adapter re-checks authentication before every turn, as Codex does. The send still confirms the executable is installed and re-reads settings; a settings revision that changed since the switch forces a fresh discovery, a disabled route rejects the send, and the adapter re-verifies the reported version at launch. Every new run retains its resolved executable path and reported version, exact model, effort, settings source, and project configuration revision in its record, manifest, and initial event. Later settings changes apply to new runs only. Review checks use the executable recorded for their run. Linked checkpoint execution retains the recorded CLI identity when present; a missing executable or changed reported version blocks execution instead of silently switching. Historical records created before executable tracking have no retained CLI identity and use discovery. Paths and reported versions are recorded; this is not a cryptographic binary pin. Viewing settings or reopening history never starts a model turn.
