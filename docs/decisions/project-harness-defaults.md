...
Open **Project settings** from Workspace, the sidebar, or a conversation to select the project's Codex CLI, default model, and reasoning effort. Save writes `config.harness.yaml` in the registered project root. The file is optional, editable outside Randolph, and suitable for version control:
